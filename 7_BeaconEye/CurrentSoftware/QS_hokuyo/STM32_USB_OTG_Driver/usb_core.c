                         ...
                         Uint16 len)
{
  ...
  {
    *(__packed Uint32 *)dest = USB_OTG_READ_REG32(fifo);
                               ...
                               USB_OTG_CORE_ID_TypeDef coreID)
{
  ...
  pdev->regs.HPRT0 = (Uint32 *)(baseAddress + USB_OTG_HOST_PORT_REGS_OFFSET);
  ...
  for (i = 0; i < pdev->cfg.host_channels; i++)
  {
    pdev->regs.DFIFO[i] = (Uint32 *)(baseAddress + USB_OTG_DATA_FIFO_OFFSET +\
      (i * USB_OTG_DATA_FIFO_SIZE));
  }
  pdev->regs.PCGCCTL = (Uint32 *)(baseAddress + USB_OTG_PCGCCTL_OFFSET);
...
* @retval USB_OTG_STS : status
*/
USB_OTG_STS USB_OTG_SetCurrentMode(USB_OTG_CORE_HANDLE *pdev , Uint8 mode)
{
  USB_OTG_STS status = USB_OTG_OK;
  USB_OTG_GUSBCFG_TypeDef  usbcfg;
