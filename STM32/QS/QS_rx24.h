 ...
 *
 * Pour utiliser ce module vous devez avoir configuré vos RX-24+ de la manière suivante :
 * <ul>
 ...
 * <li> Les ID des RX-24+ doivent être différents et se suivre (ID = 0 ... n)
 * </ul>
 */
...
//	#endif
	 ...
	 *
	 * Ce timer sera utilisé pour avoir un temps d'attente maximum lorsqu'un paquet de status est censé être enovoyé par l'RX24.
	 */
...
///@{
	 ...
	 *
	 * Vitesse du port série utilisé pour communiquer avec l'RX24/RX24 en bauds.
...
//	#endif
	 ...
	 *
	 * Un paquet status est envoyé par l'RX24/RX24 après un paquet instruction dans certains cas défini par ce define.
	 * Le paquet de status informera le driver des erreurs rencontrées et pourra contenir des données demandées.
	 * Si #RX24_STATUS_RETURN_NEVER est utilisé, aucun paquet de status ne sera envoyé par le servo,
	 * et aucune des fonctions RX24_get* et RX24_config_get* ne serons utilisable.
	 * Valeurs possibles:
	 * <ul>
	 *  <li> RX24_STATUS_RETURN_NEVER Le servo ne renverra jamais de paquet de status (sauf dans le cas de #RX24_is_ready). La lecture de données n'est possible dans ce cas.
	 *  <li> RX24_STATUS_RETURN_ONREAD Le servo ne reverra des paquet de status seulement lors de lecture de données et pour #RX24_is_ready.
	 *  <li> RX24_STATUS_RETURN_ALWAYS Le servo renverra un paquet de status après chaque instruction envoyé (sauf lorsque l'instruction est envoyé a tous les servo en utilisant #RX24_BROADCAST_ID).
	 * </ul>
...
//	#endif
	 ...
	 *
	 * Ce define défini la taille du buffer d'instruction, cette taille ne prend pas en compte le nombre d'RX24.
	 * Le buffer interne est commun à tous les RX24.
...
///@{
	 ...
	 *
	 * Le servo ne renverra jamais de paquet de status (sauf dans le cas de #RX24_is_ready). La lecture de données n'est possible dans ce cas.
	 */
	#define RX24_STATUS_RETURN_NEVER 0

	/** Renvois de paquet status lors de réponse à une lecture de donnée.
	 *
	 * Le servo ne reverra des paquet de status seulement lors de lecture de données et pour #RX24_is_ready.
	 */
	#define RX24_STATUS_RETURN_ONREAD 1	//last_status sera le résultat d'un ping ou d'une lecture

	/** Toujours renvoyer une paquet de status.
	 *
	 * Le servo renverra un paquet de status après chaque instruction envoyé (sauf lorsque l'instruction est envoyé a tous les servo en utilisant #RX24_BROADCAST_ID).
	 */
	#define RX24_STATUS_RETURN_ALWAYS 2	//last_status sera le résultat de la dernière instruction
///@}
...
#include "QS_all.h"
	...
	/** Structure contenant le dernier status reçu d'un RX24.
	 *
	 * Il y a un status par RX24/RX24 d'enregistré par le driver,
	 * si le driver reçois un nouveau paquet de status, seul l'ancien status de cet RX24/RX24 sera écrasé.
	 * @see RX24_get_last_error
	 */
	typedef struct {
		Uint8 last_instruction_address;
		/** Erreur constituée des flags RX24_ERROR_*. */
		Uint16 error;
		union {
	 ...
	 *
	 * Aucun paquet de status ne sera retourné dans ce cas, sauf pour #RX24_is_ready */
	 ...
	 *
	 * Les limites sont définies par #RX24_config_set_lowest_voltage et #RX24_config_set_highest_voltage. */
	#define RX24_ERROR_VOLTAGE		0x01

	/** L'angle demandé est hors des limites.
	 *
	 * Les limites des angles sont définies par #RX24_config_set_minimal_angle et #RX24_config_set_maximal_angle */
	 ...
	 * Utilisez #RX24_is_ready pour savoir si le servo est connecté et répond correctement.
	 * Si erreur est définie avec RX24_ERROR_RANGE en même temps, c'est que le driver à attendu trop longtemps pour bufferiser la commande et l'a annulé.
	...
	/** Des instructions sont en cours de traitement pour l'RX24/RX24 considéré
	 *
	 * Une ou plusieurs commande on été demandées de manière asynchrone et le résultat renvoyé par l'RX24/RX24 n'a pas encore été reçu.
	 ...
	 * Lors de l'initialisation, le mode de retour est défini à #RX24_STATUS_RETURN_MODE et les angles limites sont défini à 0°-300°.
	 * Le servo est initialement en mode aservissement de position.
	 */
	void RX24_init();

	/**
	 * Retourne la dernière information de status envoyé par l'RX24.
	 *
	...
	/**
	 * Réinitialise la dernière information de status à #RX24_ERROR_OK.
	 *
	 ...
	 *
	 * S'il est plein et qu'une fonction appelée nécessite l'envoi d'un paquet a l'RX24,
	 ...
	 *
	 * Les informations sur son état sont accessible avec #RX24_get_last_error.
	 * @param id_servo numéro du servo à verifier
	 * @return TRUE si le servo répond au ping, sinon FALSE
	 */
	bool_e RX24_is_ready(Uint8 id_servo);

	bool_e RX24_async_is_ready(Uint8 id_servo, bool_e *isReady); //version asynchrone, resultat à tester avec RX24_get_last_error. Return FALSE que si la file d'action du driver RX24 est pleine (cas grave, buffer trop petit ou flood de demande à l'RX24/rx24 ?)
	 ...
	 * Les commandes sont exécuté en même temps après l'appel à #RX24_end_instruction_block.
	 ...
	 * Seule les commandes d'écriture RX24_set* seront préparées. Les autres fonctionneront comme avant l'appel à cette fonction.
	 ...
	 *
	 * Si le servo dépasse cette température, le bit d'erreur #RX24_ERROR_OVERHEATING
	 ...
	 * l'erreur #RX24_ERROR_VOLTAGE sera levée. Suivant la configuration, la LED peut être allumée et/ou le servo désactivé.
	 * @param id_servo numéro du servo à questionner
	 ...
	 * <ul>
	 *  <li> #RX24_STATUS_RETURN_ONREAD Le servo ne renverra un paquet de status lors de lecture de donnée et pour #RX24_is_ready
	 *  <li> #RX24_STATUS_RETURN_ALWAYS Le servo renverra un paquet a tout envoi de paquet d'instruction, sauf si l'ID #RX24_BROADCAST_ID est utilisé
	 * </ul>
	 * @see #RX24_STATUS_RETURN_MODE
	 * @see RX24_config_set_status_return_mode
	 */
	 ...
	 *
	 * Les erreurs sont les flags RX24_ERROR*, utilisez (flags & RX24_ERROR_???) != 0 pour savoir si l'erreur RX24_ERROR_??? est levée.
	 * @param id_servo numéro du servo à questionner
	 * @return Masque d'erreur constitué de RX24_ERROR*
	 ...
	 * le servo se désactive en mettant 0% comme couple maximal, utilisez #RX24_set_torque_limit pour autoriser un couple de sortie non nul.
	 * Les erreurs sont les flags RX24_ERROR*, utilisez (flags & RX24_ERROR_???) != 0 pour savoir si l'erreur RX24_ERROR_??? est levée.
	 * @param id_servo numéro du servo à questionner
	 * @return Masque d'erreur constitué de RX24_ERROR*
	 ...
	 *
	 * Après cet appel, il n'est plus possible d'utiliser les fonctions RX24_config_set* ansi que #RX24_set_punch_torque_percentage
	...
	/**
	 * Change l'ID d'un servo RX24/RX24 branché (à utiliser temporairement, de préférence avec un seul servo de branché sur le bus pour éviter les ambiguïté !)
	 *
	...
	/**
	 * Défini la température maximale avant de lever l'erreur #RX24_ERROR_OVERHEATING.
	 *
	 * @param id_servo numéro du servo à régler
	 ...
	 * ATTENTION ! Cette fonction ne devrait pas être utilisée, changez la valeur de #RX24_STATUS_RETURN_MODE à la place.
	 ...
	 *
	 * Les erreurs sont les flags RX24_ERROR*, utilisez l'operateur OU binaire (a | b par exemple) pour définir plusieurs erreurs
	 * qui devrons allumer la LED.
	 * @param id_servo numéro du servo à régler
	 * @param error_type Masque d'erreurs constitué de RX24_ERROR*
	 * @return TRUE si l'opération s'est bien déroulé, sinon FALSE.
	 * Si la fonction retourne FALSE, aucune conclusion n'est possible quand à
	 * l'état de l'opération demandée, il se peut qu'elle soit quand même exécutée.
	 * @see RX24_config_get_error_before_led
	 ...
	 * se désactive en mettant 0% comme couple maximal, utilisez #RX24_set_torque_limit pour autoriser un couple de sortie
	 * non nul.
	 * @param id_servo numéro du servo à régler
	 * @param error_type Masque d'erreurs constitué de RX24_ERROR*
	 * @return TRUE si l'opération s'est bien déroulé, sinon FALSE.
	 ...
	 *  <li> FALSE si le servo est asservi en position (angle). Les limites d'angle sont définies avec #RX24_config_set_minimal_angle et #RX24_config_set_maximal_angle
	 * </ul>
	 ...
	 *
	 * Lorsqu'une nouvelle position est demandée avec #RX24_set_position,
	 ...
	 * La valeur initiale à la mise sous tension est celle retournée par #RX24_config_set_maximum_torque_percentage.
	 * @param id_servo numéro du servo à contrôler
