...
//-------------------------------- Message ignoré par tout le monde
	...
	switch(msg->sid)
	{
		...
		case DEBUG_PROPULION_HOKUYO_HAS_PLANTED_AND_THAT_IS_NOT_VERY_FUNNY:	print(string, len, "%x DEBUG_PROPULION_HOKUYO_HAS_PLANTED_AND_THAT_IS_NOT_VERY_FUNNY   ", DEBUG_PROPULION_HOKUYO_HAS_PLANTED_AND_THAT_IS_NOT_VERY_FUNNY			);	break;
		case DEBUG_PROPULION_HOKUYO_HAS_RESSUCITED_AND_THAT_IS_VERY_FUNNY:	print(string, len, "%x DEBUG_PROPULION_HOKUYO_HAS_RESSUCITED_AND_THAT_IS_VERY_FUNNY   ", DEBUG_PROPULION_HOKUYO_HAS_RESSUCITED_AND_THAT_IS_VERY_FUNNY			);	break;
...
//Wood
	...
	switch(msg->sid)
	{
		...
		case STRAT_PROP_FOE_DETECTED:			print(string, len, "| x : %d   y : %d   dist : %d   angle : %d   %s   %s\n", u16(0,1), u16(2,3), u8(5)*20, u16(6,7),(u8(4)&0x02)?"hokuyo":"balise", (u8(4)&0x01)?"avec timeout":"");	break;
		...
		case PROP_GO_ANGLE:						print(string, len, "| VaAngle   teta=%d=%d° %s %s %s %d%s %s\n",  angle_rad(1, 2),  angle_deg(1, 2), (u8(0) & 0x20)?"multi":" ", (u8(0) & 0x10)?"pas_now":"now", (u8(0) & 0x01)?"relatif":" ", u8(5),(u8(5)==0x00)?"=rapide":((u8(5)==0x01)?"=lente":""), (u8(6)&0x01)?"avant":((u8(6)&0x10)?"arrière":"")	);						break;
		case PROP_OFFSET_AVOID:					print(string, len, "| Xleft : %d  Xright : %d  Yfront : %d  Yback : %d\n",u16(0,1), u16(2,3), u16(4,5), u16(6,7));		break;
		case PROP_GO_POSITION:					print(string, len, "| VaPos     x=%d y=%d %s %s %s vitesse %d%s %s %s\n", u16(1,2), u16(3,4), (u8(0) & 0x20)?"multi":" ", (u8(0) & 0x10)?"pas_now":"now", (u8(0) & 0x01)?"relatif":" ", u8(5),(u8(5)==0x00)?"=rapide":((u8(5)==0x01)?"=lente":""), (u8(6)&0x02)?"avant":((u8(6)&0x01)?"arrière":"any_way"), ((u8(0)&0x02)?"mode bordure":""));	break;
