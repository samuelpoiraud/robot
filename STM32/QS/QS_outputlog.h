...
/** ----------------  Defines possibles  --------------------
 *	OUTPUTLOG_DEFAULT_MAX_LOG_LEVEL : Niveau d'affichage des printfs à définir avec une valeur de l'énumération log_level_e
 *
 *			!!!!! Définir avant l'inclusion de ce header !!!!!
 ...
 *
 * ----------------  Choses à savoir  --------------------
 * Ce header ne doit jamais être inclue dans un .h, seulement dans les .c et précédé des defines ci-dessus au besoin.
 *	Un exemple :
 *				#define LOG_PREFIX "detection: "
 *				#define LOG_COMPONENT OUTPUT_LOG_COMPONENT_DETECTION
 *				#include "QS/QS_outputlog.h"
 *
 *				(Avec OUTPUT_LOG_COMPONENT_DETECTION définit à LOG_PRINT_On ou LOG_PRINT_Off dans config/config_debug.h)
 *
 *	VERBOSE_MODE				: Désactive l'écriture de l'ensemble des printfs
 */
	...
	#define display_bit(nbBit,x)	{Uint8 var_dec = nbBit-1, var_i;			\
									for(var_i=0;var_i<nbBit;var_i++){			\
