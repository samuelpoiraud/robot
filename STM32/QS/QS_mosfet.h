 ...
 * Il est également possible d'activer plusieurs mosfets à la fois en définissant, de la même façon que pour un mosfet
 * seul, un actionneur ACT_MOSFET_MULTI (pour les mosfets pilotés par l'actionneur) et/ou STRAT_MOSFET_MULTI (pour
 * les mosfets pilotés par la stratégie). L'ordre à éxécuter est du type :  ACT_push_order(ACT_MOSFET_MULTI, arg) où
 * arg vaut une valeur binaire (bit de poid fort -> MOSFET_8, bit de poid faible -> MOSFET_1).
