...
static bool_e AX12_instruction_wait(Uint8 id_servo) {
	...
	//debug_printf("+1\n");
	while(!AX12_instruction_queue_is_empty() && global.absolute_time - local_t < 7)	//si on a attendu 7 ms, on stop, on a attendu trop longtemps (au moins 6,5ms, mais ce bout de code ne fait qu'une instruction)
		i++;
