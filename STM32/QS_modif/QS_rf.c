 ...
 *
 *  $Id: QS_rf.c 3766 2015-08-03 19:41:39Z aguilmet $
 *
...
#include "QS_rf.h"
	...
	static CAN_msg_t canmsg_pending_buffer[RF_CAN_BUF_SIZE];
	static volatile Uint8 canmsg_index_write;
	static volatile bool_e canmsg_available;
	...
	static void RF_can_received(CAN_msg_t *msg);
	...
	void RF_init(RF_module_e module, RF_onReceive_ptr onReceiveCallback, RF_onCanMsg_ptr onCanMsgCallback) {
		...
		canmsg_index_write = 0;
		canmsg_available = FALSE;
	...
	static void RF_can_received(CAN_msg_t *msg){
		canmsg_pending_buffer[canmsg_index_write] = *msg;
		canmsg_index_write++;
		canmsg_index_write %= RF_CAN_BUF_SIZE;
		canmsg_available = TRUE;
	}

	bool_e RF_can_data_ready(){
		return canmsg_available;
	}
	...
	CAN_msg_t RF_can_get_next_msg(){
		...
		NVIC_DisableIRQ(RF_UART_IRQ);
		if (next_to_read != canmsg_index_write)
		{

			// récupérer le prochain message à lire
			msgToReturn = &(canmsg_pending_buffer[next_to_read++]);
			next_to_read %= CAN_BUF_SIZE;

			// si on lit le dernier message, abaisser le drapeau.
			if (canmsg_index_write == next_to_read)
				canmsg_available = FALSE;
