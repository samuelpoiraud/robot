...
static void PROP_goto_until_break (stack_id_e stack_id, bool_e init)
{
	...
	{
		if (env.prop.ended || env.prop.freine)
		{
...
static void PROP_goto_multi_point (stack_id_e stack_id, bool_e init)
{
	...
	if (init)
	{
		env.pos.nb_points_reached = 0;
		timeout = env.match_time + 2000;
		...
		for( ; STACKS_get_action(PROP,STACKS_get_top(stack_id)) == &PROP_goto_multi_point ; STACKS_set_top(stack_id,STACKS_get_top(stack_id)-1))
		{
			...
			order.size=8;
			CAN_send(&order);
			//distance=GEOMETRY_distance((GEOMETRY_point_t){env.pos.x,env.pos.y},(GEOMETRY_point_t){args->x,args->y});
	...
	else
	{
		if (env.prop.ended)
		{
			...
			env.pos.nb_points_reached = save_stack_top - save_stack_bottom + 1;
			// Par sécurité, on met le pointeur de pile à la fin des multi-points
			STACKS_set_top(stack_id,save_stack_bottom);
			STACKS_pull(PROP);
		}
		else if (env.prop.freine)
		{
			env.pos.nb_points_reached++;
...
static void PROP_relative_goangle_multi_point (stack_id_e stack_id, bool_e init)
{
	...
	{
		if (env.prop.ended/* || (env.prop.freine && PROP_near_destination_angle())*/)
		{
...
static void PROP_goangle (stack_id_e stack_id, bool_e init)
{
	...
	else
	{
		...
		{
			if ((env.match_time - STACKS_get_action_initial_time(stack_id,STACKS_get_top(PROP)) >= (GOANGLE_TIMEOUT_TIME)))
			{
...
//		order.data[1]=prop_args[STACKS_get_top(stack_id)].speed;
	...
	{
		if (env.prop.ended || env.prop.erreur)
		{
		...
		{
			if ((env.match_time - STACKS_get_action_initial_time(stack_id,STACKS_get_top(stack_id)) >= (RUSH_TIMEOUT_TIME)))
			{
...
static void PROP_stop(stack_id_e stack_id, bool_e init) {
	if(init) {
		...
		{
			if ((env.match_time - STACKS_get_action_initial_time(stack_id,STACKS_get_top(PROP)) >= (STOP_TIMEOUT_TIME)))
			{
