...
#define LOG_PREFIX "pompe_left : "
#define LOG_COMPONENT OUTPUT_LOG_COMPONENT_POMPE_LEFT
#include "../QS/QS_outputlog.h"

static void POMPE_LEFT_command(ACT_order_e command);
...
bool_e POMPE_LEFT_CAN_process_msg(CAN_msg_t* msg) {
	if(msg->sid == ACT_POMPE_LEFT) {
		switch(msg->data.act_msg.order) {
			case ACT_POMPE_LEFT_NORMAL:
			case ACT_POMPE_LEFT_STOP:
				POMPE_LEFT_command(msg->data.act_msg.order);
				break;
...
static void POMPE_LEFT_command(ACT_order_e command) {
	...
	switch(command) {
		case ACT_POMPE_LEFT_NORMAL:
			GPIO_SetBits(POMPE_LEFT_PIN);
			break;

		case ACT_POMPE_LEFT_STOP:
			GPIO_ResetBits(POMPE_LEFT_PIN);
			return;

		default: {
			error_printf("Invalid POMPE_LEFT command: %u, code is broken !\n", command);
			return;
		}
	}
}
