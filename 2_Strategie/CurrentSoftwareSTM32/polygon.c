 ...
 */
	...
	*/
	...
	static void POLYGON_create_opponent_polygon(polygon_t* opponent_polygon)
	{
		...
		x_opponent = global.foe.x;
		y_opponent = global.foe.y;
	...
	static void POLYGON_generate_list(polygon_list_t* polygons_list, polygon_elements_type_e type_elements, bool_e use_opponent)
	{
		...
		{
			case ALL_ELEMENTS:
				for(i=0; i<global.nb_elements; i++)
				{
					if(global.nb_elements)
					POLYGON_create_polygon(&((*polygons_list).polygon[(*polygons_list).polygonsNumber]),global.elements_list[i].x, global.elements_list[i].y);	//poly2
					(*polygons_list).polygonsNumber++;
				}
				break;
