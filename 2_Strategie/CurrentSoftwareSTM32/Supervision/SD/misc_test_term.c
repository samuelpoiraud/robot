...
const Uint16 VirtAddVarTab[NumbOfVar] = { 1000, 1001, 1002 };
...
volatile bool_e overrun;
volatile bool_e sampling;
#define RECORD_SIZE (2*sizeof(Uint32))
...
Uint32 pre_allocation_size = 1024*1024*200; /* 200 MBytes */
...
#define CCR1_VAL      ((Uint16)144)
#define BASEFREQUENCY ((Uint32)100000) /* Hz */
#define FREQUENCY_TO_SOFTPRESCALE( freq ) ((BASEFREQUENCY/freq)-1)
#define SOFTPRESCALE_TO_FREQUENCY( sps )  (BASEFREQUENCY/(sps+1))
static volatile Uint32 soft_prescale = FREQUENCY_TO_SOFTPRESCALE( 1 );
...
{
	static Uint32 soft_prescale_cnt;

	static Uint8  record[RECORD_SIZE];
	static Uint32 rec_count;
	Uint32 val1, val2;

	if (TIM2->SR & TIM_IT_CC1) {
		TIM2->SR = (Uint16)~TIM_IT_CC1;
		TIM2->CCR1 += CCR1_VAL;
...
/*
	...
	v = *((volatile Uint32*)0x1FFFF7E0);
	xprintf("Flash size %ld kBytes\n", v & 0xFFFF);
	ph = (volatile Uint16*)0x1FFFF7E8;
	pw = (volatile Uint32*)(0x1FFFF7E8+4);
...
static bool_e flash_is_locked(void)
{
	return (FLASH->CR & (1<<7) /* Bit 7 LOCK */) ? TRUE : FALSE;
}

static bool_e my_FLASH_Unlock(void)
{
	if ( flash_is_locked() ) {
		FLASH_Unlock();
...
static bool_e my_FLASH_Lock(void)
{
	if ( !flash_is_locked() ) {
		FLASH_Lock();
	}
	...
	switch (*ptr++)
	{
		...
		case 's':
			...
			case 'i': // si - sampling-buffers state
				if (sampling) {
					for (i = 0; i < NBUFFERS; i++) {
						debug_printf("Buffer#%u count=%lu\n", i, (volatile Uint32) log_buffers[i].cnt);
					}
			...
			case 'd' : // sd <n> - dump n records
				...
				for ( i = 0; i < (unsigned)p1; i++ ) {
					...
					if ( s2 != RECORD_SIZE ) {
						...
						Uint32 ts = BYTEARRAY_TO_DWORD( log_buffers[0].data );
						Uint32 pl = BYTEARRAY_TO_DWORD( log_buffers[0].data+4 );
						if ( first ) {
							debug_printf("t:%6lu  l:%6lu\n", ts, pl);
							first = FALSE;
						} else {
							Uint32 delta = (Uint16)((Uint16)ts-(Uint16)prev);
							Uint32 delta_us = delta * (1000000UL/BASEFREQUENCY) / CCR1_VAL;
			...
			switch (*ptr++) {
				...
				case 'w': // ew <idx> <val> - write to virtual EEPROM
					...
					if (xatoi(&ptr, &p1) && xatoi(&ptr, &p2)) {
						if ( p1 < NumbOfVar ) {
							p1 = VirtAddVarTab[p1];
							debug_printf("write EEMUL[%d] := %d ", (Uint16)p1, (Uint16)p2);
							if ( my_FLASH_Unlock() ) {
				...
				case 'r': { // er [<idx>] - read from virtual EEPROM
					...
					if (xatoi(&ptr, &p1)) {
						if ( p1 < NumbOfVar ) {
							p1 = VirtAddVarTab[p1];
							/*if (EE_ReadVariable((Uint16)p1, &r) == 0) {
