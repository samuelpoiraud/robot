...
int term_cmd_goto(int argc, const char *argv[]) {
	...
	if(x >= 200 && x <= 1800 && y >= 200 && y <= 2800){
			if(global.env.pos.x < 250 || global.env.pos.x > 1750 || global.env.pos.y < 250 || global.env.pos.y > 2750)
				PROP_push_goto(x, y, FAST, ANY_WAY, 0, AVOID_DISABLED, END_AT_BREAK, FALSE);
			else
				PROP_push_goto(x, y, FAST, ANY_WAY, 0, AVOID_DISABLED, END_AT_BREAK, TRUE);
	}else
		return EINVAL;

	if(global.env.pos.x < 250)
		PROP_push_goto(250, global.env.pos.y, FAST, ANY_WAY, 0, AVOID_DISABLED, END_AT_BREAK, TRUE);
	else if(global.env.pos.x > 1750)
		PROP_push_goto(1750, global.env.pos.y, FAST, ANY_WAY, 0, AVOID_DISABLED, END_AT_BREAK, TRUE);

	if(global.env.pos.y < 250)
		PROP_push_goto(global.env.pos.x, 250, FAST, ANY_WAY, 0, AVOID_DISABLED, END_AT_BREAK, TRUE);
	else if(global.env.pos.y > 2750)
		PROP_push_goto(global.env.pos.x, 2750, FAST, ANY_WAY, 0, AVOID_DISABLED, END_AT_BREAK, TRUE);
