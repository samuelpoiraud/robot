...
void SYNCHRO_init() {
	...
	offset = 0x0FFF;
	last_received_reply = env.absolute_time;
...
void SYNCHRO_process_main()
{
	...
	if(REPLY_REQ == FALSE && RF_get_module_id() == RF_SMALL && last_received_reply + LAST_REPLY_TIMEOUT <= env.absolute_time) {
		REPLY_REQ = TRUE;
...
static void rf_packet_received_callback(bool_e for_me, RF_header_t header, Uint8 *data, Uint8 size) {
	...
	if(header.sender_id < RF_MODULE_COUNT)
		last_activity_time[header.sender_id] = env.absolute_time;
}
...
static void update_rfmodule_here()
{
	...
	for(module_id = 0; module_id < RF_MODULE_COUNT; module_id++)
	{
		...
		{
			if(last_activity_time[module_id] + LAST_SYNCHRO_TIMEOUT >= env.absolute_time)
			{
	...
	if(warner_low_battery_on_foe1 && (env.absolute_time > low_battery_on_foe1_printed_time + PERIOD_PRINT_LOW_BAT_ON_BEACON))
	{
		low_battery_on_foe1_printed_time = env.absolute_time;
		debug_printf("Beacon 1 has low bat : pwm=%d", pwm_on_foe1);
	}
	if(warner_low_battery_on_foe2 && (env.absolute_time > low_battery_on_foe2_printed_time + PERIOD_PRINT_LOW_BAT_ON_BEACON))
	{
		low_battery_on_foe2_printed_time = env.absolute_time;
