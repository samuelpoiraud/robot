...
error_e sub_harry_depose_modules_centre(Uint8 drop_place, moduleStockLocation_e robot_side, way_e way){
	...
	switch(state){
		...
		case DEPOSE_MODULE:
			...
			if(robot_side == MODULE_STOCK_LEFT){
				state = check_sub_action_result(sub_act_harry_mae_dispose_modules(MODULE_STOCK_LEFT, ((STOCKS_getNbModules(robot_side) > 1 && (6-MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)) > 1) ? ARG_DISPOSE_ONE_CYLINDER_FOLLOW_BY_ANOTHER :ARG_DISPOSE_ONE_CYLINDER_AND_FINISH)), state, TIGHT_BACKWARD, GET_OUT_WITH_ERROR);
			}
			else if(robot_side == MODULE_STOCK_RIGHT){
				state = check_sub_action_result(sub_act_harry_mae_dispose_modules(MODULE_STOCK_RIGHT, ((STOCKS_getNbModules(robot_side) > 1  && (6-MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)) > 1) ? ARG_DISPOSE_ONE_CYLINDER_FOLLOW_BY_ANOTHER :ARG_DISPOSE_ONE_CYLINDER_AND_FINISH)), state, TIGHT_BACKWARD, GET_OUT_WITH_ERROR);
			}
			else{
				state = DONE;
			}
			...
			/*if(drop_place == POS_1){
				...
				if(way == FORWARD){
					if(global.color == BLUE){
						state = try_going((MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)>3)?(1715-(MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)-3)*7):(1715),\
								COLOR_Y((MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)>3)?(865-(MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)-3)*7):(865)), state, UP_PUSHER_LEFT, ERROR, FAST, FORWARD, NO_DODGE_AND_WAIT, END_AT_LAST_POINT);
					}else{
						state = try_going((MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)>3)?(1715-(MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)-3)*7):(1715),\
									COLOR_Y((MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)>3)?(865-(MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)-3)*7):(865)), state, UP_PUSHER_RIGHT, ERROR,  FAST, FORWARD, NO_DODGE_AND_WAIT, END_AT_LAST_POINT);
					}
				}
				else{
					if(global.color == BLUE){
						state = try_going((MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)>3)?(1690-(MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)-3)*7):(1690),\
								COLOR_Y((MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)>3)?(830-(MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)-3)*7):(830)), state, UP_PUSHER_LEFT, ERROR,  FAST, BACKWARD, NO_DODGE_AND_WAIT, END_AT_LAST_POINT);
					}else{
						state = try_going((MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)>3)?(1690-(MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)-3)*7):(1690),\
									COLOR_Y((MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)>3)?(830-(MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)-3)*7):(830)), state, UP_PUSHER_RIGHT, ERROR,  FAST, BACKWARD, NO_DODGE_AND_WAIT, END_AT_LAST_POINT);
					}
				}
			}
