				...
				if(way == FORWARD){
					...
					if(global.color == BLUE){
						state = try_going((MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)>3)?(1690-(MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)-3)*7):(1690),\
								COLOR_Y((MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)>3)?(830-(MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)-3)*7):(830)), state, UP_PUSHER_LEFT, ERROR,  FAST, FORWARD, NO_DODGE_AND_WAIT, END_AT_LAST_POINT);
					}else{
						state = try_going((MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)>3)?(1690-(MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)-3)*7):(1690),\
									COLOR_Y((MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)>3)?(830-(MOONBASES_getNbModules(MODULE_MOONBASE_OUR_CENTER)-3)*7):(830)), state, UP_PUSHER_RIGHT, ERROR,  FAST, FORWARD, NO_DODGE_AND_WAIT, END_AT_LAST_POINT);
					}
				}
