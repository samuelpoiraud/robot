...
pathfind_node_id_t PATHFIND_closestNodeWithoutAdversary(Sint16 x, Sint16 y, Uint32 filteredNodes){
	...
	for(i=0;i<MAX_NB_FOES;i++){
		for(n = 0; n < 4; n++){
			if(closestNodes[i] != NOT_IN_NODE){
				dist = dist_point_to_point(nodes[n].x, nodes[n].y, env.foe[i].x, env.foe[i].y);
				if(dist < DISTANCE_CONSIDERE_ADVERSARY)
					nodeValue[n] = dist_point_to_point(nodes[n].x, nodes[n].y, env.foe[i].x, env.foe[i].y);
			}
		}
	}
...
static Uint16 Pathfind_cost(pathfind_node_id_t from, pathfind_node_id_t to, bool_e handleOpponent)
{
	 ...
	 */
		...
		for(i=0; i<MAX_NB_FOES; i++)
		{
			if(env.foe[i].enable	&& env.foe[i].dist < DISTANCE_CONSIDERE_ADVERSARY)	//Si l'adversaire est proche de nous...
			{
				dist = dist_point_to_point(nodes[from].x, nodes[from].y, env.foe[i].x, env.foe[i].y);
...
static error_e PATHFIND_compute(displacement_curve_t * displacements, Uint8 * p_nb_displacements, Sint16 xFrom, Sint16 yFrom, pathfind_node_id_t to)
{
	...
	for(i=0;i<MAX_NB_FOES;i++)
	{
		if(env.foe[i].enable && env.foe[i].dist < DISTANCE_CONSIDERE_ADVERSARY)	//Pour chaque adversaire situé proche de nous...
		{
			...
			{	//Si l'adversaire en question est proche du noeud : on ajoute le noeud dans la liste des noeuds innaccessibles.
				...
				if(PATHFIND_manhattan_dist(nodes[n].x, nodes[n].y, env.foe[i].x, env.foe[i].y)<MANHATTAN_DIST_NODE_BLOQUED_BY_ADVERSARY)
				{
...
Uint8 PATHFIND_try_going(pathfind_node_id_t node_wanted, Uint8 in_progress, Uint8 success_state, Uint8 fail_state, PROP_speed_e speed, way_e way, avoidance_type_e avoidance, PROP_end_condition_e end_condition)
{
	...
	switch(state)
	{
		...
		case COMPUTE:
			...
			if(PATHFIND_compute(displacements, &nb_displacements, env.pos.x, env.pos.y, node_wanted) != END_OK)
			{
				SD_printf("I cannot find any path. x=%d y=%d to node %d\n",env.pos.x, env.pos.y, node_wanted);
				state = FAIL;
			}
			else
			{
...
pathfind_node_id_t min_node_dist(pathfind_node_id_t n1,pathfind_node_id_t n2){
	if(PATHFIND_manhattan_dist(PATHFIND_get_node_x(n1),PATHFIND_get_node_y(n1),env.pos.x,env.pos.y)
			< PATHFIND_manhattan_dist(PATHFIND_get_node_x(n2),PATHFIND_get_node_y(n2),env.pos.x,env.pos.y))
		return n1;
	else
		return n2;
}
...
void PATHFIND_updateOpponentPosition(Uint8 foe_id)
{
	...
	pathfind_node_id_t node = PATHFIND_closestNode(env.foe[foe_id].x, env.foe[foe_id].y, 0);
	//si on est trop loin
	int dist = GEOMETRY_squared_distance((GEOMETRY_point_t){nodes[node].x,nodes[node].y},(GEOMETRY_point_t){env.foe[foe_id].x,env.foe[foe_id].y});
	if(dist>220)
	{
