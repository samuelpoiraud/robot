	...
	switch(state) {
		...
		case DP_PROCESS_PLATE: {
			...
			if(entrance) {
				...
				if((plate_number_in_stock >= LAUNCH_EVERY_NUM_PLATE || TRUE) && !doing_special_plate_3)	//On doit jeter l'assiette si on fait la 3 coté bleu pour pouvoir prendre la 4 !
					keep_plate = TRUE;
	...
	switch(state) {
		...
		case GP_CATCH_PLATE:
		{
			...
			state = try_going_multipoint((displacement_t[]){
				...
				ANY_WAY, NO_AVOIDANCE, END_AT_LAST_POINT		//ANY_WAY si jamais le premier point est derrière nous ...
				);
