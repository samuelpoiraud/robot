...
*/
	...
	switch(state)
	{
		...
		case GET_OUT_IF_NO_CALIBRATION:
			...
			state = try_going(250, COLOR_Y(150), GET_OUT_IF_NO_CALIBRATION, TURN_IF_NO_CALIBRATION, TURN_IF_NO_CALIBRATION, FAST, ANY_WAY, NO_AVOIDANCE);
		break;
		...
		case BP:
			...
			state = try_going(1000,COLOR_Y(1800), BP, WAIT_UNTIL_60SEC, WAIT_UNTIL_60SEC, FAST, ANY_WAY, NO_DODGE_AND_WAIT);
		break;
...
void STRAT_TINY_test_avoidance_goto_pos_no_dodge_and_wait(void)
{
	...
	switch(state){
		...
		break;
		case GOTO_A:
			state = try_going(500, COLOR_Y(500), GOTO_A, GOTO_B, GOTO_D,FAST, FORWARD,NO_DODGE_AND_WAIT);
		break;
		case GOTO_B:
			state = try_going(1000, COLOR_Y(500), GOTO_B, GOTO_C, GOTO_A,FAST, FORWARD,NO_DODGE_AND_WAIT);
		break;
		case GOTO_C:
			state = try_going(1000, COLOR_Y(1500), GOTO_C, GOTO_D, GOTO_B,FAST, BACKWARD,NO_DODGE_AND_WAIT);
		break;
		case GOTO_D:
			state = try_going(500, COLOR_Y(1500), GOTO_D, GOTO_A, GOTO_C,FAST, BACKWARD,NO_DODGE_AND_WAIT);
		break;
...
void STRAT_TINY_all_candles(void)
{
	...
	switch(state)
	{
		...
		case GET_OUT:
			...
			state = try_going(250, COLOR_Y(200), GET_OUT, TURN,TURN,FAST,ANY_WAY,NO_DODGE_AND_WAIT);
		break;
		...
		case TINY_RUSH_IW:
			state = try_going(1350, COLOR_Y(200), TINY_RUSH_IW, DONE,DONE,FAST,FORWARD,NO_DODGE_AND_WAIT);
		break;
...
void STRAT_TINY_test_moisson(void){
	...
	switch(state){
		...
		case P_1:
			//									  in_progress	success	failed
			state = try_going(300, COLOR_Y(2000), P_1, P_2, GO_HOME,FAST,FORWARD,NO_DODGE_AND_WAIT);
			previousState = P_1;
			break;
		case P_2:
			if(count < 2){
				count++;
				ACT_plier_open();
				state = try_going(600, COLOR_Y(2000), P_2, GO_HOME, P_4,FAST,FORWARD,NO_DODGE_AND_WAIT);
			}else{
				state = DONE;
			}
			previousState = P_2;
		...
		case P_3:
			...
			state = try_going(900, COLOR_Y(2000), P_3, P_4, P_2,FAST,FORWARD,NO_DODGE_AND_WAIT);
			previousState = P_3;
			break;
		...
		case GO_HOME:
			...
			switch(previousState){
				case P_1:
					...
					break;
				case P_2:
					state = try_going(300, COLOR_Y(300), GO_HOME,P_3 ,DONE,FAST,FORWARD,NO_DODGE_AND_WAIT);
					break;
				case P_3:
					//non
					state = DONE;
