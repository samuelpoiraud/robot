...
/**
  * @brief  Displays a maximum of 20 char on the LCD.
  * @param  colon : set the first char of the string
  * 		Line: the Line where to display the character shape .
  *   This parameter can be one of the following values:
  *     @arg Linex: where x can be 0..9
  * @param  *ptr: pointer to string to display on LCD.
  * @retval None
  */
void LCD_DisplayStringLineColon(uint16_t colon, uint16_t Line, char *ptr)
{
  uint16_t refcolumn = colon;
...
/**
  * @brief  Displays ur bitmap picture.
  * @param  BmpAddress : tableau rvg 8bits
  * @param size : taille du tableau
  * @retval None
  */
void LCD_WriteBMP_2(uint8_t * tab, uint32_t size)
{
	uint16_t temp;
  uint32_t index = 0;
  /* Set GRAM write direction and BGR = 1 */
