...
void SEQUENCES_calibrate(way_e way, calibration_square_e calibration_square)
{
	...
	if(QS_WHO_AM_I_get() == PIERRE){
		...
		if(color == RED){
			ROADMAP_add_order(TRAJECTORY_ROTATION, 0, 0,PI4096, RELATIVE, NOT_NOW, ANY_WAY, NOT_BORDER_MODE, NO_MULTIPOINT, SLOW, NO_ACKNOWLEDGE, CORRECTOR_ENABLE);
			ROADMAP_add_order(TRAJECTORY_TRANSLATION, BIG_RED_CALIBRATION_X - BIG_CALIBRATION_BACKWARD_BORDER_DISTANCE - 80, BIG_RED_CALIBRATION_Y - BIG_CALIBRATION_BACKWARD_BORDER_DISTANCE - 80,0, RELATIVE, NOT_NOW, ANY_WAY, NOT_BORDER_MODE, NO_MULTIPOINT, SLOW, NO_ACKNOWLEDGE, CORRECTOR_ENABLE);
			ROADMAP_add_order(TRAJECTORY_ROTATION, 0, 0,BIG_RED_CALIBRATION_TETA, RELATIVE, NOT_NOW, ANY_WAY, NOT_BORDER_MODE, NO_MULTIPOINT, SLOW, NO_ACKNOWLEDGE, CORRECTOR_ENABLE);
		}else{
			ROADMAP_add_order(TRAJECTORY_TRANSLATION, BIG_YELLOW_CALIBRATION_X - BIG_CALIBRATION_BACKWARD_BORDER_DISTANCE - 80, BIG_YELLOW_CALIBRATION_Y - 3000 + BIG_CALIBRATION_BACKWARD_BORDER_DISTANCE + 80,0, RELATIVE, NOT_NOW, ANY_WAY, NOT_BORDER_MODE, NO_MULTIPOINT, SLOW, NO_ACKNOWLEDGE, CORRECTOR_ENABLE);
			ROADMAP_add_order(TRAJECTORY_ROTATION, 0, 0,BIG_YELLOW_CALIBRATION_TETA, RELATIVE, NOT_NOW, ANY_WAY, NOT_BORDER_MODE, NO_MULTIPOINT, SLOW, NO_ACKNOWLEDGE, CORRECTOR_ENABLE);
		}
	}else{// GUY
		if(color == RED)
			y = 100;
