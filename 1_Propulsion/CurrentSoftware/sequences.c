...
static bool_e start_rotation = FALSE;
void START_ROTATION_canMsg(CAN_msg_t *msg){
	start_rotation = msg->data[0];
	SEQUENCES_calibrate();
}
...
/*
	...
	*/
		if(color == BOT_COLOR)
			...
			ODOMETRY_set(BIG_TOP_COLOR_CALIBRATION_X, BIG_TOP_COLOR_CALIBRATION_Y,BIG_TOP_COLOR_CALIBRATION_TETA);
		ROADMAP_add_order(TRAJECTORY_NONE,0,0,0,NOT_RELATIVE,NOT_NOW,ANY_WAY,NOT_BORDER_MODE,NO_MULTIPOINT,SLOW,ACKNOWLEDGE_CALIBRATION,CORRECTOR_ENABLE,AVOID_DISABLED);

	}else{// SMALL ROBOT

		//J'avance de 400mm pour sortir de la zone de départ... (ma position initiale est contre le bac de dépose).
		//y = (color==BOT_COLOR)?(+400):(-400);
		...
		//Translation relative en 1400... donc +400...
		ROADMAP_add_order(TRAJECTORY_TRANSLATION, 400, 0, 0, RELATIVE, NOT_NOW, ANY_WAY, NOT_BORDER_MODE, NO_MULTIPOINT, FAST, NO_ACKNOWLEDGE, CORRECTOR_ENABLE, AVOID_DISABLED);

		//On s'approche du mur rapidement...
		y = (color==BOT_COLOR)?(-400):400;
		ROADMAP_add_order(TRAJECTORY_TRANSLATION, 0, y, 0, RELATIVE, NOT_NOW, FORWARD, NOT_BORDER_MODE, NO_MULTIPOINT, FAST, NO_ACKNOWLEDGE, CORRECTOR_ENABLE, AVOID_DISABLED);
		...
		if(color == BOT_COLOR)
			y = SMALL_BOT_COLOR_CALIBRATION_Y-SMALL_CALIBRATION_FORWARD_BORDER_DISTANCE;
		else
			y = SMALL_CALIBRATION_FORWARD_BORDER_DISTANCE-SMALL_BOT_COLOR_CALIBRATION_Y;

		//Eloignement de la bordure pour calage en X
		ROADMAP_add_order(TRAJECTORY_TRANSLATION, 0, y, 0, RELATIVE, NOT_NOW, ANY_WAY, NOT_BORDER_MODE, NO_MULTIPOINT, FAST, NO_ACKNOWLEDGE, CORRECTOR_ENABLE, AVOID_DISABLED);

		//On s'approche du mur rapidement...
		ROADMAP_add_order(TRAJECTORY_TRANSLATION, 450,0, 0, RELATIVE, NOT_NOW, FORWARD, NOT_BORDER_MODE, NO_MULTIPOINT, FAST, NO_ACKNOWLEDGE, CORRECTOR_ENABLE, AVOID_DISABLED);
		...
		if(color == BOT_COLOR){
			ROADMAP_add_order(TRAJECTORY_TRANSLATION, SMALL_BOT_COLOR_CALIBRATION_X, SMALL_BOT_COLOR_CALIBRATION_Y,0, NOT_RELATIVE, NOT_NOW, BACKWARD, NOT_BORDER_MODE, NO_MULTIPOINT, FAST, NO_ACKNOWLEDGE, CORRECTOR_ENABLE, AVOID_DISABLED);
			ROADMAP_add_order(TRAJECTORY_ROTATION,0,0,(!start_rotation)?SMALL_BOT_COLOR_CALIBRATION_TETA:(-PI4096/2), NOT_RELATIVE, NOT_NOW, BACKWARD, NOT_BORDER_MODE, NO_MULTIPOINT, SLOW, ACKNOWLEDGE_CALIBRATION, CORRECTOR_ENABLE, AVOID_DISABLED);
		}else{
			ROADMAP_add_order(TRAJECTORY_TRANSLATION, SMALL_TOP_COLOR_CALIBRATION_X, SMALL_TOP_COLOR_CALIBRATION_Y,0, NOT_RELATIVE, NOT_NOW, BACKWARD, NOT_BORDER_MODE, NO_MULTIPOINT, FAST, NO_ACKNOWLEDGE, CORRECTOR_ENABLE, AVOID_DISABLED);
			ROADMAP_add_order(TRAJECTORY_ROTATION,0,0,(!start_rotation)?SMALL_TOP_COLOR_CALIBRATION_TETA:(PI4096/2), NOT_RELATIVE, NOT_NOW, BACKWARD, NOT_BORDER_MODE, NO_MULTIPOINT, SLOW, ACKNOWLEDGE_CALIBRATION, CORRECTOR_ENABLE, AVOID_DISABLED);
		}
	}
#endif
