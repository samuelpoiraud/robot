...
static void  USBH_ParseHIDDesc (USBH_HIDDesc_TypeDef *desc, Uint8 *buf);
...
static void  USBH_ParseHIDDesc (USBH_HIDDesc_TypeDef *desc, Uint8 *buf)
{
  ...
  desc->bLength                  = *(Uint8  *) (buf + 0);
  desc->bDescriptorType          = *(Uint8  *) (buf + 1);
  desc->bcdHID                   =  LE16  (buf + 2);
  desc->bCountryCode             = *(Uint8  *) (buf + 4);
  desc->bNumDescriptors          = *(Uint8  *) (buf + 5);
  desc->bReportDescriptorType    = *(Uint8  *) (buf + 6);
