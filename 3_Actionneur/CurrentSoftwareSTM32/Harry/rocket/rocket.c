...
// ROCKET d'un actionneur standart avec AX12
...
//		Formatage : QUEUE_ACT_AX12_ROCKET
...
// 5) Ajout du pilotage via terminal dans le fichier term_io.c dans le tableau terminal_motor du type : ACT_DECLARE(ROCKET)
// 6) Un #define ROCKET_AX12_ID doit avoir été ajouté au fichier config_big/config_pin.h ou config_small/config_pin.h
...
#define LOG_PREFIX "rocket.c : "
#define LOG_COMPONENT OUTPUT_LOG_COMPONENT_ROCKET
#include "../QS/QS_outputlog.h"

// Les fonctions internes au fonctionnement de l'actionneur
static void ROCKET_command_run(queue_id_t queueId);
static void ROCKET_initAX12();
static void ROCKET_command_init(queue_id_t queueId);
static void ROCKET_set_config(CAN_msg_t* msg);
static void ROCKET_get_position_config(ACT_order_e *pOrder, Uint16 *pPos);
static void ROCKET_get_config(CAN_msg_t *incoming_msg);
static void ROCKET_set_warner(CAN_msg_t *msg);
static void ROCKET_check_warner(Uint16 pos);
...
static void ROCKET_initAX12() {
    if(ax12_is_initialized == FALSE && AX12_is_ready(ROCKET_AX12_ID) == TRUE) {
		ax12_is_initialized = TRUE;
        AX12_config_set_lowest_voltage(ROCKET_AX12_ID, AX12_MIN_VOLTAGE);
        AX12_config_set_highest_voltage(ROCKET_AX12_ID, AX12_MAX_VOLTAGE);
        AX12_set_torque_limit(ROCKET_AX12_ID, ROCKET_AX12_MAX_TORQUE_PERCENT);
        AX12_config_set_temperature_limit(ROCKET_AX12_ID, ROCKET_AX12_MAX_TEMPERATURE);
...
void ROCKET_set_config(CAN_msg_t* msg){
	switch(msg->data.act_msg.act_data.act_config.sub_act_id){
		case DEFAULT_MONO_ACT : // Premier élement de l'actionneur
            ACTMGR_config_AX12(ROCKET_AX12_ID, msg);
			break;
...
static void ROCKET_get_config(CAN_msg_t *incoming_msg){
	...
	msg.data.act_get_config_answer.sid = 0xFF & ACT_ROCKET;
	...
	switch(incoming_msg->data.act_msg.act_data.config){
		case POSITION_CONFIG:
			ROCKET_get_position_config(&order, &pos);
			...
			break;
		case TORQUE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.torque = AX12_get_speed_percentage(ROCKET_AX12_ID);
			break;
		case TEMPERATURE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.temperature = AX12_get_temperature(ROCKET_AX12_ID);
			break;
		case LOAD_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.load = AX12_get_load_percentage(ROCKET_AX12_ID);
			break;
		default:
			error = TRUE;
...
static void ROCKET_get_position_config(ACT_order_e *pOrder, Uint16 *pPos){
	ACT_order_e order = ACT_ROCKET_STOP;
	Uint16 position = AX12_get_position(ROCKET_AX12_ID);
	Uint16 epsilon = ROCKET_AX12_ASSER_POS_EPSILON;

	if(position > ROCKET_AX12_LAUNCH_POS - epsilon && position < ROCKET_AX12_LAUNCH_POS + epsilon){
		order = ACT_ROCKET_LAUNCH;
	}else if(position > ROCKET_AX12_PREPARE_LAUNCH_POS - epsilon && position < ROCKET_AX12_PREPARE_LAUNCH_POS + epsilon){
		order = ACT_ROCKET_PREPARE_LAUNCH;
	}else if(position > ROCKET_AX12_IDLE_POS - epsilon && position < ROCKET_AX12_IDLE_POS + epsilon){
		order = ACT_ROCKET_IDLE;
	}
...
void ROCKET_init_pos(){
    ...
    if(!AX12_set_position(ROCKET_AX12_ID, ROCKET_AX12_INIT_POS))
...
void ROCKET_stop(){
    AX12_set_torque_enabled(ROCKET_AX12_ID, FALSE); //Stopper l'asservissement de l'AX12
}

// fonction appellée à la réception d'un message CAN (via ActManager)
bool_e ROCKET_CAN_process_msg(CAN_msg_t* msg) {
    if(msg->sid == ACT_ROCKET){
        ROCKET_initAX12();
		switch(msg->data.act_msg.order) {
			// Listing de toutes les positions de l'actionneur possible
            case ACT_ROCKET_IDLE :
            case ACT_ROCKET_LAUNCH :
            case ACT_ROCKET_PREPARE_LAUNCH:
            case ACT_ROCKET_STOP :
            	run_now = msg->data.act_msg.act_data.act_order.run_now;
                ACTQ_push_operation_from_msg(msg, QUEUE_ACT_AX12_ROCKET, &ROCKET_run_command, 0,TRUE);
				break;

            case ACT_WARNER:
				ROCKET_set_warner(msg);
				break;

			case ACT_GET_CONFIG:
				ROCKET_get_config(msg);
				break;

			case ACT_SET_CONFIG :
                ROCKET_set_config(msg);
				break;
...
void ROCKET_run_command(queue_id_t queueId, bool_e init) {
    ...
    if(QUEUE_get_act(queueId) == QUEUE_ACT_AX12_ROCKET) {    // Gestion des mouvements de ROCKET
		if(init)
            ROCKET_command_init(queueId);
		else
            ROCKET_command_run(queueId);
	}
}
...
static void ROCKET_command_init(queue_id_t queueId) {
	...
	*ax12_goalPosition = 0xFFFF;
	...
	switch(command) {
        ...
        case ACT_ROCKET_IDLE : *ax12_goalPosition = ROCKET_AX12_IDLE_POS; break;
        case ACT_ROCKET_LAUNCH : *ax12_goalPosition = ROCKET_AX12_LAUNCH_POS; break;
        case ACT_ROCKET_PREPARE_LAUNCH : *ax12_goalPosition = ROCKET_AX12_PREPARE_LAUNCH_POS; break;

        case ACT_ROCKET_STOP :
            AX12_set_torque_enabled(ROCKET_AX12_ID, FALSE); //Stopper l'asservissement de l'AX12
            QUEUE_next(queueId, ACT_ROCKET, ACT_RESULT_DONE, ACT_RESULT_ERROR_OK, __LINE__);
			return;

		default: {
            error_printf("Invalid ROCKET command: %u, code is broken !\n", command);
            QUEUE_next(queueId, ACT_ROCKET, ACT_RESULT_NOT_HANDLED, ACT_RESULT_ERROR_LOGIC, __LINE__);
			return;
		}
	}
	...
	if(ax12_is_initialized == FALSE){
        ...
        QUEUE_next(queueId, ACT_ROCKET, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
    ...
    AX12_reset_last_error(ROCKET_AX12_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
    if(!AX12_set_position(ROCKET_AX12_ID, *ax12_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que l'AX12 ne répond pas a cet envoi, on l'indique à la carte stratégie
        error_printf("AX12_set_position error: 0x%x\n", AX12_get_last_error(ROCKET_AX12_ID).error);
        QUEUE_next(queueId, ACT_ROCKET, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	//La commande a été envoyée et l'AX12 l'a bien reçu
    debug_printf("Move ROCKET ax12 to %d\n", *ax12_goalPosition);
}
...
static void ROCKET_command_run(queue_id_t queueId) {
	...
	Uint16 pos = AX12_get_position(ROCKET_AX12_ID);

	if(!run_now){
		if(ACTQ_check_status_ax12(queueId, ROCKET_AX12_ID, QUEUE_get_arg(queueId)->param, pos, ROCKET_AX12_ASSER_POS_EPSILON, ROCKET_AX12_ASSER_TIMEOUT, ROCKET_AX12_ASSER_POS_LARGE_EPSILON, &result, &errorCode, &line))
			QUEUE_next(queueId, ACT_ROCKET, result, errorCode, line);
	}else{
		// Lorsqu'on vient de recevoir un run_now, on passe directement à l'ordre reçu.
		ACTQ_flush_queue_to_run_now(queueId, ACT_ROCKET);
	}

    // On ne surveille le warner que si il est activé
   	if(warner.activated)
   		ROCKET_check_warner(pos);
}

static void ROCKET_set_warner(CAN_msg_t *msg){
	warner.activated = TRUE;
	warner.last_pos = AX12_get_position(ROCKET_AX12_ID);

	switch(msg->data.act_msg.act_data.warner_pos){
		/*case ACT_ROCKET_WARNER:
			warner.warner_pos = ROCKET_AX12_WARNER_POS;
			break;*/
		default:{
...
static void ROCKET_check_warner(Uint16 pos){
	...
	if( (warner.last_pos < pos && warner.last_pos < warner.warner_pos && pos > warner.warner_pos)
	 ...
	 || (warner.last_pos < AX12_MIN_WARNER && pos > AX12_MAX_WARNER	&& (warner.last_pos > warner.warner_pos || pos < warner.warner_pos))
		...
		msg.data.act_warner_answer.sid = 0xFF & ACT_ROCKET;
		CAN_send(&msg);
