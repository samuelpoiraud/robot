...
static void ROCKET_get_config(CAN_msg_t *incoming_msg){
	...
	switch(incoming_msg->data.act_msg.act_data.config){
		case POSITION_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.pos = ROCKET_get_position_config();
			CAN_send(&msg); // Envoi du message CAN
			break;
		case SPEED_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.speed = AX12_get_speed_percentage(ROCKET_AX12_ID);
			CAN_send(&msg); // Envoi du message CAN
			break;
		default:
...
static ACT_order_e ROCKET_get_position_config(){
	ACT_order_e current_state = ACT_ROCKET_STOP;
...
static void ROCKET_command_run(queue_id_t queueId) {
    ...
    if(ACTQ_check_status_ax12(queueId, ROCKET_AX12_ID, QUEUE_get_arg(queueId)->param, ROCKET_AX12_ASSER_POS_EPSILON, ROCKET_AX12_ASSER_TIMEOUT, ROCKET_AX12_ASSER_POS_LARGE_EPSILON, &result, &errorCode, &line))
