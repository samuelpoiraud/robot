...
// 6) Un #define ORE_GUN_RX24_ID doit avoir été ajouté au fichier config_big/config_pin.h ou config_small/config_pin.h
...
static void ORE_GUN_set_config(CAN_msg_t* msg);
static void ORE_GUN_get_position_config(ACT_order_e *pOrder, Uint16 *pPos);
static void ORE_GUN_get_config(CAN_msg_t *incoming_msg);
static void ORE_GUN_set_warner(CAN_msg_t *msg);
static void ORE_GUN_check_warner(Uint16 pos);
...
static void ORE_GUN_initRX24() {
	if(rx24_is_initialized == FALSE && RX24_is_ready(ORE_GUN_RX24_ID) == TRUE) {
		...
		RX24_set_move_to_position_speed(ORE_GUN_RX24_ID, ORE_GUN_RX24_SPEED);
...
void ORE_GUN_set_config(CAN_msg_t* msg){
...
static void ORE_GUN_get_config(CAN_msg_t *incoming_msg){
	...
	msg.data.act_get_config_answer.sid = 0xFF & ACT_ORE_GUN;
	...
	switch(incoming_msg->data.act_msg.act_data.config){
		case POSITION_CONFIG:
			ORE_GUN_get_position_config(&order, &pos);
			...
			break;
		case TORQUE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.torque = RX24_get_speed_percentage(ORE_GUN_RX24_ID);
			break;
		case TEMPERATURE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.temperature = RX24_get_temperature(ORE_GUN_RX24_ID);
			break;
		case LOAD_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.load = RX24_get_load_percentage(ORE_GUN_RX24_ID);
			break;
		default:
			error = TRUE;
...
static void ORE_GUN_get_position_config(ACT_order_e *pOrder, Uint16 *pPos){
	ACT_order_e order = ACT_ORE_GUN_STOP;
	Uint16 position = RX24_get_position(ORE_GUN_RX24_ID);
	Uint16 epsilon = ORE_GUN_RX24_ASSER_POS_EPSILON;

	if(position > ORE_GUN_RX24_DOWN_POS - epsilon && position < ORE_GUN_RX24_DOWN_POS + epsilon){
		order = ACT_ORE_GUN_DOWN;
	}else if(position > ORE_GUN_RX24_UP_POS - epsilon && position < ORE_GUN_RX24_UP_POS + epsilon){
		order = ACT_ORE_GUN_UP;
	}else if(position > ORE_GUN_RX24_IDLE_POS - epsilon && position < ORE_GUN_RX24_IDLE_POS + epsilon){
		order = ACT_ORE_GUN_IDLE;
	}
...
bool_e ORE_GUN_CAN_process_msg(CAN_msg_t* msg) {
	if(msg->sid == ACT_ORE_GUN){
		...
		switch(msg->data.act_msg.order) {
			...
			case ACT_WARNER:
				ORE_GUN_set_warner(msg);
				break;

			case ACT_GET_CONFIG:
				ORE_GUN_get_config(msg);
				break;

			case ACT_SET_CONFIG :
				ORE_GUN_set_config(msg);
				break;
...
static void ORE_GUN_command_init(queue_id_t queueId) {
	...
	*rx24_goalPosition = 0xFFFF;
	...
	RX24_reset_last_error(ORE_GUN_RX24_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
	if(!RX24_set_position(ORE_GUN_RX24_ID, *rx24_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que le RX24 ne répond pas a cet envoi, on l'indique à la carte stratégie
...
static void ORE_GUN_command_run(queue_id_t queueId) {
	...
	if(warner.activated)
		ORE_GUN_check_warner(pos);
}

static void ORE_GUN_set_warner(CAN_msg_t *msg){
	warner.activated = TRUE;
	warner.last_pos = RX24_get_position(ORE_GUN_RX24_ID);

	switch(msg->data.act_msg.act_data.warner_pos){
		/*case ACT_ORE_GUN_WARNER:
			warner.warner_pos = ORE_GUN_RX24_WARNER_POS;
			break;*/
		default:{
...
static void ORE_GUN_check_warner(Uint16 pos){
	...
	if( (warner.last_pos < pos && warner.last_pos < warner.warner_pos && pos > warner.warner_pos)
	 ...
	 || (warner.last_pos < RX24_MIN_WARNER && pos > RX24_MAX_WARNER	&& (warner.last_pos > warner.warner_pos || pos < warner.warner_pos))
		...
		msg.data.act_warner_answer.sid = 0xFF & ACT_ORE_GUN;
		CAN_send(&msg);
