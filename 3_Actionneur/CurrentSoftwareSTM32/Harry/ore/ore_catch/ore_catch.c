...
// ORE_CATCH d'un actionneur standart avec AX12
...
//		Formatage : QUEUE_ACT_AX12_ORE_CATCH
...
// 5) Ajout du pilotage via terminal dans le fichier term_io.c dans le tableau terminal_motor du type : ACT_DECLARE(ORE_CATCH)
// 6) Un #define ORE_CATCH_AX12_ID doit avoir été ajouté au fichier config_big/config_pin.h ou config_small/config_pin.h
...
#include "ore_catch_config.h"

// Les différents define pour le verbose sur uart
#define LOG_PREFIX "ORE_CATCH.c : "
#define LOG_COMPONENT OUTPUT_LOG_COMPONENT_ORE_CATCH
#include "../QS/QS_outputlog.h"

// Les fonctions internes au fonctionnement de l'actionneur
static void ORE_CATCH_command_run(queue_id_t queueId);
static void ORE_CATCH_initAX12();
static void ORE_CATCH_command_init(queue_id_t queueId);
static void ORE_CATCH_config(CAN_msg_t* msg);
...
static void ORE_CATCH_initAX12() {
	if(ax12_is_initialized == FALSE && AX12_is_ready(ORE_CATCH_AX12_ID) == TRUE) {
		ax12_is_initialized = TRUE;
		AX12_config_set_lowest_voltage(ORE_CATCH_AX12_ID, AX12_MIN_VOLTAGE);
		AX12_config_set_highest_voltage(ORE_CATCH_AX12_ID, AX12_MAX_VOLTAGE);
		AX12_set_torque_limit(ORE_CATCH_AX12_ID, ORE_CATCH_AX12_MAX_TORQUE_PERCENT);
		AX12_config_set_temperature_limit(ORE_CATCH_AX12_ID, ORE_CATCH_AX12_MAX_TEMPERATURE);

		AX12_config_set_maximal_angle(ORE_CATCH_AX12_ID, ORE_CATCH_AX12_MAX_VALUE);
		AX12_config_set_minimal_angle(ORE_CATCH_AX12_ID, ORE_CATCH_AX12_MIN_VALUE);

		AX12_config_set_error_before_led(ORE_CATCH_AX12_ID, AX12_BEFORE_LED);
		AX12_config_set_error_before_shutdown(ORE_CATCH_AX12_ID, AX12_BEFORE_SHUTDOWN);
...
void ORE_CATCH_config(CAN_msg_t* msg){
	switch(msg->data.act_msg.act_data.act_config.sub_act_id){
		case DEFAULT_MONO_ACT : // Premier élement de l'actionneur
			ACTMGR_config_AX12(ORE_CATCH_AX12_ID, msg);
			break;
...
void ORE_CATCH_init_pos(){
	...
	if(!AX12_set_position(ORE_CATCH_AX12_ID, ORE_CATCH_AX12_INIT_POS))
...
void ORE_CATCH_stop(){
	AX12_set_torque_enabled(ORE_CATCH_AX12_ID, FALSE); //Stopper l'asservissement de l'AX12
}

// fonction appellée à la réception d'un message CAN (via ActManager)
bool_e ORE_CATCH_CAN_process_msg(CAN_msg_t* msg) {
	if(msg->sid == ACT_ORE_CATCH){
		ORE_CATCH_initAX12();
		switch(msg->data.act_msg.order) {
			// Listing de toutes les positions de l'actionneur possible
			case ACT_ORE_CATCH_IDLE :
			case ACT_ORE_CATCH_LOCK :
			case ACT_ORE_CATCH_UNLOCK :
			case ACT_ORE_CATCH_STOP :
				ACTQ_push_operation_from_msg(msg, QUEUE_ACT_AX12_ORE_CATCH, &ORE_CATCH_run_command, 0,TRUE);
				break;

			case ACT_CONFIG :
				ORE_CATCH_config(msg);
				break;
...
void ORE_CATCH_run_command(queue_id_t queueId, bool_e init) {
	...
	if(QUEUE_get_act(queueId) == QUEUE_ACT_AX12_ORE_CATCH) {    // Gestion des mouvements de ORE_CATCH
		if(init)
			ORE_CATCH_command_init(queueId);
		else
			ORE_CATCH_command_run(queueId);
	}
}
...
static void ORE_CATCH_command_init(queue_id_t queueId) {
	...
	*ax12_goalPosition = 0xFFFF;
	...
	switch(command) {
		...
		case ACT_ORE_CATCH_IDLE : *ax12_goalPosition = ORE_CATCH_AX12_IDLE_POS; break;
		case ACT_ORE_CATCH_LOCK : *ax12_goalPosition = ORE_CATCH_AX12_LOCK_POS; break;
		case ACT_ORE_CATCH_UNLOCK : *ax12_goalPosition = ORE_CATCH_AX12_UNLOCK_POS; break;

		case ACT_ORE_CATCH_STOP :
			AX12_set_torque_enabled(ORE_CATCH_AX12_ID, FALSE); //Stopper l'asservissement de l'AX12
			QUEUE_next(queueId, ACT_ORE_CATCH, ACT_RESULT_DONE, ACT_RESULT_ERROR_OK, __LINE__);
			return;

		default: {
			error_printf("Invalid ORE_CATCH command: %u, code is broken !\n", command);
			QUEUE_next(queueId, ACT_ORE_CATCH, ACT_RESULT_NOT_HANDLED, ACT_RESULT_ERROR_LOGIC, __LINE__);
			return;
		}
	}
	...
	if(ax12_is_initialized == FALSE){
		...
		QUEUE_next(queueId, ACT_ORE_CATCH, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	...
	AX12_reset_last_error(ORE_CATCH_AX12_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
	if(!AX12_set_position(ORE_CATCH_AX12_ID, *ax12_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que l'AX12 ne répond pas a cet envoi, on l'indique à la carte stratégie
		error_printf("AX12_set_position error: 0x%x\n", AX12_get_last_error(ORE_CATCH_AX12_ID).error);
		QUEUE_next(queueId, ACT_ORE_CATCH, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	//La commande a été envoyée et l'AX12 l'a bien reçu
	debug_printf("Move ORE_CATCH ax12 to %d\n", *ax12_goalPosition);
}
...
static void ORE_CATCH_command_run(queue_id_t queueId) {
	...
	if(ACTQ_check_status_ax12(queueId, ORE_CATCH_AX12_ID, QUEUE_get_arg(queueId)->param, ORE_CATCH_AX12_ASSER_POS_EPSILON, ORE_CATCH_AX12_ASSER_TIMEOUT, ORE_CATCH_AX12_ASSER_POS_LARGE_EPSILON, &result, &errorCode, &line))
		QUEUE_next(queueId, ACT_ORE_CATCH, result, errorCode, line);
}
#endif
