...
// 6) Un #define ORE_WALL_RX24_ID doit avoir été ajouté au fichier config_big/config_pin.h ou config_small/config_pin.h
...
static void ORE_WALL_set_config(CAN_msg_t* msg);
static void ORE_WALL_get_position_config(ACT_order_e *pOrder, Uint16 *pPos);
static void ORE_WALL_get_config(CAN_msg_t *incoming_msg);
static void ORE_WALL_set_warner(CAN_msg_t *msg);
static void ORE_WALL_check_warner(Uint16 pos);
...
static void ORE_WALL_initRX24() {
	if(rx24_is_initialized == FALSE && RX24_is_ready(ORE_WALL_RX24_ID) == TRUE) {
		...
		RX24_set_move_to_position_speed(ORE_WALL_RX24_ID, ORE_WALL_RX24_SPEED);
...
void ORE_WALL_set_config(CAN_msg_t* msg){
...
static void ORE_WALL_get_config(CAN_msg_t *incoming_msg){
	...
	msg.data.act_get_config_answer.sid = 0xFF & ACT_ORE_WALL;
	...
	switch(incoming_msg->data.act_msg.act_data.config){
		case POSITION_CONFIG:
			ORE_WALL_get_position_config(&order, &pos);
			...
			break;
		case TORQUE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.torque = RX24_get_speed_percentage(ORE_WALL_RX24_ID);
			break;
		case TEMPERATURE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.temperature = RX24_get_temperature(ORE_WALL_RX24_ID);
			break;
		case LOAD_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.load = RX24_get_load_percentage(ORE_WALL_RX24_ID);
			break;
		default:
			error = TRUE;
...
static void ORE_WALL_get_position_config(ACT_order_e *pOrder, Uint16 *pPos){
	ACT_order_e order = ACT_ORE_WALL_STOP;
	Uint16 position = RX24_get_position(ORE_WALL_RX24_ID);
	Uint16 epsilon = ORE_WALL_RX24_ASSER_POS_EPSILON;
...
bool_e ORE_WALL_CAN_process_msg(CAN_msg_t* msg) {
	if(msg->sid == ACT_ORE_WALL){
		...
		switch(msg->data.act_msg.order) {
			...
			case ACT_WARNER:
				ORE_WALL_set_warner(msg);
				break;

			case ACT_GET_CONFIG:
				ORE_WALL_get_config(msg);
				break;

			case ACT_SET_CONFIG :
				ORE_WALL_set_config(msg);
				break;
...
static void ORE_WALL_command_init(queue_id_t queueId) {
	...
	*rx24_goalPosition = 0xFFFF;
	...
	RX24_reset_last_error(ORE_WALL_RX24_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
	if(!RX24_set_position(ORE_WALL_RX24_ID, *rx24_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que le RX24 ne répond pas a cet envoi, on l'indique à la carte stratégie
...
static void ORE_WALL_command_run(queue_id_t queueId) {
	...
	Uint16 pos = RX24_get_position(ORE_WALL_RX24_ID);

	if(ACTQ_check_status_rx24(queueId, ORE_WALL_RX24_ID, QUEUE_get_arg(queueId)->param, pos, ORE_WALL_RX24_ASSER_POS_EPSILON, ORE_WALL_RX24_ASSER_TIMEOUT, ORE_WALL_RX24_ASSER_POS_LARGE_EPSILON, &result, &errorCode, &line))
		QUEUE_next(queueId, ACT_ORE_WALL, result, errorCode, line);

    // On ne surveille le warner que si il est activé
	if(warner.activated)
		ORE_WALL_check_warner(pos);
}

static void ORE_WALL_set_warner(CAN_msg_t *msg){
	warner.activated = TRUE;
	warner.last_pos = RX24_get_position(ORE_WALL_RX24_ID);

	switch(msg->data.act_msg.act_data.warner_pos){
		/*case ACT_ORE_WALL_WARNER:
			warner.warner_pos = ORE_WALL_RX24_WARNER_POS;
			break;*/
		default:{
...
static void ORE_WALL_check_warner(Uint16 pos){
	...
	if( (warner.last_pos < pos && warner.last_pos < warner.warner_pos && pos > warner.warner_pos)
	 ...
	 || (warner.last_pos < RX24_MIN_WARNER && pos > RX24_MAX_WARNER	&& (warner.last_pos > warner.warner_pos || pos < warner.warner_pos))
		...
		msg.data.act_warner_answer.sid = 0xFF & ACT_ORE_WALL;
		CAN_send(&msg);
