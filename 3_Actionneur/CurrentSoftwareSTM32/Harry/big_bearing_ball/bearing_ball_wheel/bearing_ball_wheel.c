...
// 6) Un #define BEARING_BALL_WHEEL_AX12_ID doit avoir été ajouté au fichier config_big/config_pin.h ou config_small/config_pin.h
...
static void BEARING_BALL_WHEEL_config(CAN_msg_t* msg);
...
void BEARING_BALL_WHEEL_config(CAN_msg_t* msg){
...
bool_e BEARING_BALL_WHEEL_CAN_process_msg(CAN_msg_t* msg) {
    if(msg->sid == ACT_BEARING_BALL_WHEEL){
		...
		switch(msg->data.act_msg.order) {
            ...
            case ACT_BEARING_BALL_WHEEL_LOCK :
            case ACT_BEARING_BALL_WHEEL_UNLOCK :
...
static void BEARING_BALL_WHEEL_command_init(queue_id_t queueId) {
	...
	*ax12_goalPosition = 0xFFFF;
	...
	switch(command) {
        ...
        case ACT_BEARING_BALL_WHEEL_LOCK : *ax12_goalPosition = BEARING_BALL_WHEEL_AX12_LOCK_POS; break;
        case ACT_BEARING_BALL_WHEEL_UNLOCK : *ax12_goalPosition = BEARING_BALL_WHEEL_AX12_UNLOCK_POS; break;
    ...
    AX12_reset_last_error(BEARING_BALL_WHEEL_AX12_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
    if(!AX12_set_position(BEARING_BALL_WHEEL_AX12_ID, *ax12_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que l'AX12 ne répond pas a cet envoi, on l'indique à la carte stratégie
