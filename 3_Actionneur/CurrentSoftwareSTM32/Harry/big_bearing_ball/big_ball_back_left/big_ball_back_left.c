...
static void BIG_BALL_BACK_LEFT_get_config(CAN_msg_t *incoming_msg){
	...
	switch(incoming_msg->data.act_msg.act_data.config){
		case POSITION_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.pos = BIG_BALL_BACK_LEFT_get_position_config();
			CAN_send(&msg); // Envoi du message CAN
			break;
		case SPEED_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.speed = AX12_get_speed_percentage(BIG_BALL_BACK_LEFT_AX12_ID);
			CAN_send(&msg); // Envoi du message CAN
			break;
		default:
...
static ACT_order_e BIG_BALL_BACK_LEFT_get_position_config(){
	ACT_order_e current_state = ACT_BIG_BALL_BACK_LEFT_STOP;
...
static void BIG_BALL_BACK_LEFT_command_run(queue_id_t queueId) {
    ...
    if(ACTQ_check_status_ax12(queueId, BIG_BALL_BACK_LEFT_AX12_ID, QUEUE_get_arg(queueId)->param, BIG_BALL_BACK_LEFT_AX12_ASSER_POS_EPSILON, BIG_BALL_BACK_LEFT_AX12_ASSER_TIMEOUT, BIG_BALL_BACK_LEFT_AX12_ASSER_POS_LARGE_EPSILON, &result, &errorCode, &line))
        QUEUE_next(queueId, ACT_BIG_BALL_BACK_LEFT, result, errorCode, line);

    // On ne surveille le warner que si il est activé
	if(warner.activated)
		BIG_BALL_BACK_LEFT_check_warner();
}
