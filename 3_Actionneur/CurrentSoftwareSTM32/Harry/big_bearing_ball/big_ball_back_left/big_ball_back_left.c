 ...
 *
 *	Fichier : BIG_BALL_BACK_LEFT_ax12.c
...
#include "big_ball_back_left.h"


// BIG_BALL_BACK_LEFT d'un actionneur standart avec AX12
...
//		Formatage : QUEUE_ACT_AX12_BIG_BALL_BACK_LEFT
...
// 5) Ajout du pilotage via terminal dans le fichier term_io.c dans le tableau terminal_motor du type : ACT_DECLARE(BIG_BALL_BACK_LEFT)
// 6) Un #define BIG_BALL_BACK_LEFT_AX12_ID doit avoir été ajouté au fichier config_big/config_pin.h ou config_small/config_pin.h
...
#include "big_ball_back_left_config.h"

// Les différents define pour le verbose sur uart
#define LOG_PREFIX "big_ball_back_left.c : "
#define LOG_COMPONENT OUTPUT_LOG_COMPONENT_BIG_BALL_BACK_LEFT
#include "../QS/QS_outputlog.h"

// Les fonctions internes au fonctionnement de l'actionneur
static void BIG_BALL_BACK_LEFT_command_run(queue_id_t queueId);
static void BIG_BALL_BACK_LEFT_initAX12();
static void BIG_BALL_BACK_LEFT_command_init(queue_id_t queueId);
static void BIG_BALL_BACK_LEFT_set_config(CAN_msg_t* msg);
static void BIG_BALL_BACK_LEFT_get_position_config(ACT_order_e *pOrder, Uint16 *pPos);
static void BIG_BALL_BACK_LEFT_get_config(CAN_msg_t *incoming_msg);
static void BIG_BALL_BACK_LEFT_set_warner(CAN_msg_t *msg);
static void BIG_BALL_BACK_LEFT_check_warner(Uint16 pos);
...
static void BIG_BALL_BACK_LEFT_initAX12() {
    if(ax12_is_initialized == FALSE && AX12_is_ready(BIG_BALL_BACK_LEFT_AX12_ID) == TRUE) {
		ax12_is_initialized = TRUE;
        AX12_config_set_lowest_voltage(BIG_BALL_BACK_LEFT_AX12_ID, AX12_MIN_VOLTAGE);
        AX12_config_set_highest_voltage(BIG_BALL_BACK_LEFT_AX12_ID, AX12_MAX_VOLTAGE);
        AX12_set_torque_limit(BIG_BALL_BACK_LEFT_AX12_ID, BIG_BALL_BACK_LEFT_AX12_MAX_TORQUE_PERCENT);
        AX12_config_set_temperature_limit(BIG_BALL_BACK_LEFT_AX12_ID, BIG_BALL_BACK_LEFT_AX12_MAX_TEMPERATURE);

        AX12_config_set_maximal_angle(BIG_BALL_BACK_LEFT_AX12_ID, BIG_BALL_BACK_LEFT_AX12_MAX_VALUE);
        AX12_config_set_minimal_angle(BIG_BALL_BACK_LEFT_AX12_ID, BIG_BALL_BACK_LEFT_AX12_MIN_VALUE);
        AX12_set_move_to_position_speed(BIG_BALL_BACK_LEFT_AX12_ID, BIG_BALL_BACK_LEFT_AX12_SPEED);

        AX12_config_set_error_before_led(BIG_BALL_BACK_LEFT_AX12_ID, AX12_BEFORE_LED);
        AX12_config_set_error_before_shutdown(BIG_BALL_BACK_LEFT_AX12_ID, AX12_BEFORE_SHUTDOWN);
...
void BIG_BALL_BACK_LEFT_set_config(CAN_msg_t* msg){
	switch(msg->data.act_msg.act_data.act_config.sub_act_id){
		case DEFAULT_MONO_ACT : // Premier élement de l'actionneur
            ACTMGR_config_AX12(BIG_BALL_BACK_LEFT_AX12_ID, msg);
			break;
...
static void BIG_BALL_BACK_LEFT_get_config(CAN_msg_t *incoming_msg){
	...
	msg.data.act_get_config_answer.sid = 0xFF & ACT_BIG_BALL_BACK_LEFT;
	...
	switch(incoming_msg->data.act_msg.act_data.config){
		case POSITION_CONFIG:
			BIG_BALL_BACK_LEFT_get_position_config(&order, &pos);
			...
			break;
		case TORQUE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.torque = AX12_get_speed_percentage(BIG_BALL_BACK_LEFT_AX12_ID);
			break;
		case TEMPERATURE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.temperature = AX12_get_temperature(BIG_BALL_BACK_LEFT_AX12_ID);
			break;
		case LOAD_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.load = AX12_get_load_percentage(BIG_BALL_BACK_LEFT_AX12_ID);
			break;
		default:
			error = TRUE;
...
static void BIG_BALL_BACK_LEFT_get_position_config(ACT_order_e *pOrder, Uint16 *pPos){
	ACT_order_e order = ACT_BIG_BALL_BACK_LEFT_STOP;
	Uint16 position = AX12_get_position(BIG_BALL_BACK_LEFT_AX12_ID);
	Uint16 epsilon = BIG_BALL_BACK_LEFT_AX12_ASSER_POS_EPSILON;

	if(position > BIG_BALL_BACK_LEFT_AX12_UP_POS - epsilon && position < BIG_BALL_BACK_LEFT_AX12_UP_POS + epsilon){
		order = ACT_BIG_BALL_BACK_LEFT_UP;
	}else if(position > BIG_BALL_BACK_LEFT_AX12_DOWN_POS - epsilon && position < BIG_BALL_BACK_LEFT_AX12_DOWN_POS + epsilon){
		order = ACT_BIG_BALL_BACK_LEFT_DOWN;
	}else if(position > BIG_BALL_BACK_LEFT_AX12_IDLE_POS - epsilon && position < BIG_BALL_BACK_LEFT_AX12_IDLE_POS + epsilon){
		order = ACT_BIG_BALL_BACK_LEFT_IDLE;
	}
...
void BIG_BALL_BACK_LEFT_init_pos(){
    ...
    if(!AX12_set_position(BIG_BALL_BACK_LEFT_AX12_ID, BIG_BALL_BACK_LEFT_AX12_INIT_POS))
        debug_printf("   L'AX12 n°%d n'est pas là\n", BIG_BALL_BACK_LEFT_AX12_ID);
	else
...
bool_e BIG_BALL_BACK_LEFT_CAN_process_msg(CAN_msg_t* msg) {
    if(msg->sid == ACT_BIG_BALL_BACK_LEFT){
        BIG_BALL_BACK_LEFT_initAX12();
		switch(msg->data.act_msg.order) {
            ...
            case ACT_WARNER:
				BIG_BALL_BACK_LEFT_set_warner(msg);
				break;

			case ACT_GET_CONFIG:
				BIG_BALL_BACK_LEFT_get_config(msg);
				break;

			case ACT_SET_CONFIG :
                BIG_BALL_BACK_LEFT_set_config(msg);
				break;
...
void BIG_BALL_BACK_LEFT_run_command(queue_id_t queueId, bool_e init) {
    ...
    if(QUEUE_get_act(queueId) == QUEUE_ACT_AX12_BIG_BALL_BACK_LEFT) {    // Gestion des mouvements de BIG_BALL_BACK_LEFT
		if(init)
            BIG_BALL_BACK_LEFT_command_init(queueId);
		else
            BIG_BALL_BACK_LEFT_command_run(queueId);
	}
}
...
static void BIG_BALL_BACK_LEFT_command_init(queue_id_t queueId) {
	...
	*ax12_goalPosition = 0xFFFF;
	...
	switch(command) {
        ...
        case ACT_BIG_BALL_BACK_LEFT_STOP :
            AX12_set_torque_enabled(BIG_BALL_BACK_LEFT_AX12_ID, FALSE); //Stopper l'asservissement de l'AX12
            QUEUE_next(queueId, ACT_BIG_BALL_BACK_LEFT, ACT_RESULT_DONE, ACT_RESULT_ERROR_OK, __LINE__);
			return;

		default: {
            error_printf("Invalid BIG_BALL_BACK_LEFT command: %u, code is broken !\n", command);
            QUEUE_next(queueId, ACT_BIG_BALL_BACK_LEFT, ACT_RESULT_NOT_HANDLED, ACT_RESULT_ERROR_LOGIC, __LINE__);
			return;
		}
	}
	...
	if(ax12_is_initialized == FALSE){
        ...
        QUEUE_next(queueId, ACT_BIG_BALL_BACK_LEFT, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
    ...
    AX12_reset_last_error(BIG_BALL_BACK_LEFT_AX12_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
    if(!AX12_set_position(BIG_BALL_BACK_LEFT_AX12_ID, *ax12_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que l'AX12 ne répond pas a cet envoi, on l'indique à la carte stratégie
        error_printf("AX12_set_position error: 0x%x\n", AX12_get_last_error(BIG_BALL_BACK_LEFT_AX12_ID).error);
        QUEUE_next(queueId, ACT_BIG_BALL_BACK_LEFT, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	//La commande a été envoyée et l'AX12 l'a bien reçu
    debug_printf("Move BIG_BALL_BACK_LEFT ax12 to %d\n", *ax12_goalPosition);
}
...
static void BIG_BALL_BACK_LEFT_command_run(queue_id_t queueId) {
	...
	if(warner.activated)
		BIG_BALL_BACK_LEFT_check_warner(pos);
}

static void BIG_BALL_BACK_LEFT_set_warner(CAN_msg_t *msg){
	warner.activated = TRUE;
	warner.last_pos = AX12_get_position(BIG_BALL_BACK_LEFT_AX12_ID);

	switch(msg->data.act_msg.act_data.warner_pos){
		/*case ACT_BIG_BALL_BACK_LEFT_WARNER:
			warner.warner_pos = BIG_BALL_BACK_LEFT_AX12_WARNER_POS;
			break;*/
		default:{
...
static void BIG_BALL_BACK_LEFT_check_warner(Uint16 pos){
	...
	if( (warner.last_pos < pos && warner.last_pos < warner.warner_pos && pos > warner.warner_pos)
	 ...
	 || (warner.last_pos < AX12_MIN_WARNER && pos > AX12_MAX_WARNER	&& (warner.last_pos > warner.warner_pos || pos < warner.warner_pos))
		...
		msg.data.act_warner_answer.sid = 0xFF & ACT_BIG_BALL_BACK_LEFT;
		CAN_send(&msg);
