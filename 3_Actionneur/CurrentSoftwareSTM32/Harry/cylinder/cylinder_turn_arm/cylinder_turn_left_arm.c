...
// 6) Un #define CYLINDER_TURN_LEFT_ARM_RX24_ID doit avoir été ajouté au fichier config_big/config_pin.h ou config_small/config_pin.h
...
static void CYLINDER_TURN_LEFT_ARM_set_config(CAN_msg_t* msg);
static ACT_order_e CYLINDER_TURN_LEFT_ARM_get_position_config();
static void CYLINDER_TURN_LEFT_ARM_get_config(CAN_msg_t *incoming_msg);
static void CYLINDER_TURN_LEFT_ARM_set_warner(CAN_msg_t *msg);
static void CYLINDER_TURN_LEFT_ARM_check_warner();
...
void CYLINDER_TURN_LEFT_ARM_set_config(CAN_msg_t* msg){
...
static void CYLINDER_TURN_LEFT_ARM_get_config(CAN_msg_t *incoming_msg){
	...
	msg.data.act_get_config_answer.sid = 0xFF & ACT_CYLINDER_TURN_LEFT_ARM;
	...
	switch(incoming_msg->data.act_msg.act_data.config){
		case POSITION_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.pos = CYLINDER_TURN_LEFT_ARM_get_position_config();
			CAN_send(&msg); // Envoi du message CAN
			break;
		case SPEED_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.speed = RX24_get_speed_percentage(CYLINDER_TURN_LEFT_ARM_RX24_ID);
			CAN_send(&msg); // Envoi du message CAN
			break;
		default:
...
static ACT_order_e CYLINDER_TURN_LEFT_ARM_get_position_config(){
	ACT_order_e current_state = ACT_CYLINDER_TURN_LEFT_ARM_STOP;
	Uint16 position = RX24_get_position(CYLINDER_TURN_LEFT_ARM_RX24_ID);
	Uint16 epsilon = CYLINDER_TURN_LEFT_ARM_RX24_ASSER_POS_EPSILON;

	if(position > CYLINDER_TURN_LEFT_ARM_RX24_LOCK_POS - epsilon && position < CYLINDER_TURN_LEFT_ARM_RX24_LOCK_POS + epsilon){
		current_state = ACT_CYLINDER_TURN_LEFT_ARM_LOCK;
	}else if(position > CYLINDER_TURN_LEFT_ARM_RX24_UNLOCK_POS - epsilon && position < CYLINDER_TURN_LEFT_ARM_RX24_UNLOCK_POS + epsilon){
		current_state = ACT_CYLINDER_TURN_LEFT_ARM_UNLOCK;
	}else if(position > CYLINDER_TURN_LEFT_ARM_RX24_IDLE_POS - epsilon && position < CYLINDER_TURN_LEFT_ARM_RX24_IDLE_POS + epsilon){
		current_state = ACT_CYLINDER_TURN_LEFT_ARM_IDLE;
	}
...
bool_e CYLINDER_TURN_LEFT_ARM_CAN_process_msg(CAN_msg_t* msg) {
    if(msg->sid == ACT_CYLINDER_TURN_LEFT_ARM){
		...
		switch(msg->data.act_msg.order) {
            ...
            case ACT_WARNER:
				CYLINDER_TURN_LEFT_ARM_set_warner(msg);
				break;

			case ACT_GET_CONFIG:
				CYLINDER_TURN_LEFT_ARM_get_config(msg);
				break;

			case ACT_SET_CONFIG :
                CYLINDER_TURN_LEFT_ARM_set_config(msg);
				break;
...
static void CYLINDER_TURN_LEFT_ARM_command_init(queue_id_t queueId) {
	...
	*rx24_goalPosition = 0xFFFF;
    ...
    RX24_reset_last_error(CYLINDER_TURN_LEFT_ARM_RX24_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
    if(!RX24_set_position(CYLINDER_TURN_LEFT_ARM_RX24_ID, *rx24_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que le RX24 ne répond pas a cet envoi, on l'indique à la carte stratégie
...
static void CYLINDER_TURN_LEFT_ARM_command_run(queue_id_t queueId) {
	...
	if(warner.activated)
		CYLINDER_TURN_LEFT_ARM_check_warner();
}

static void CYLINDER_TURN_LEFT_ARM_set_warner(CAN_msg_t *msg){
	warner.activated = TRUE;
	warner.last_pos = RX24_get_position(CYLINDER_TURN_LEFT_ARM_RX24_ID);

	switch(msg->data.act_msg.act_data.warner_pos){
		/*case ACT_CYLINDER_TURN_LEFT_ARM_WARNER:
			warner.warner_pos = CYLINDER_TURN_LEFT_ARM_RX24_WARNER_POS;
			break;*/
		default:{
...
static void CYLINDER_TURN_LEFT_ARM_check_warner(){
	...
	Uint16 pos = RX24_get_position(CYLINDER_TURN_LEFT_ARM_RX24_ID);
	...
	if( (warner.last_pos < pos && warner.last_pos < warner.warner_pos && pos > warner.warner_pos)
	 ...
	 || (warner.last_pos < RX24_MIN_WARNER && pos > RX24_MAX_WARNER	&& (warner.last_pos > warner.warner_pos || pos < warner.warner_pos))
		...
		msg.data.act_warner_answer.sid = 0xFF & ACT_CYLINDER_TURN_LEFT_ARM;
		CAN_send(&msg);
