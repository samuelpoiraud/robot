...
// 6) Un #define CYLINDER_ARM_RIGHT_RX24_ID doit avoir été ajouté au fichier config_big/config_pin.h ou config_small/config_pin.h
...
static void CYLINDER_ARM_RIGHT_command_init(queue_id_t queueId) {
	...
	*rx24_goalPosition = 0xFFFF;
    ...
    RX24_reset_last_error(CYLINDER_ARM_RIGHT_RX24_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
    if(!RX24_set_position(CYLINDER_ARM_RIGHT_RX24_ID, *rx24_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que le RX24 ne répond pas a cet envoi, on l'indique à la carte stratégie
...
static void CYLINDER_ARM_RIGHT_command_run(queue_id_t queueId) {
	...
	if(!run_now){
		...
		ACTQ_flush_queue_to_run_now(queueId, ACT_CYLINDER_ARM_RIGHT);
		run_now = FALSE;
	}
