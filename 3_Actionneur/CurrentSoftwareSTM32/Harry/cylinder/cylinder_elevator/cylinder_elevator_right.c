...
*
*	Fichier : CYLINDER_ELEVATOR_RIGHT_rx24.c
...
#include "cylinder_elevator_right.h"


// CYLINDER_ELEVATOR_RIGHT d'un actionneur standart avec RX24
...
//		Formatage : QUEUE_ACT_RX24_CYLINDER_ELEVATOR_RIGHT
...
// 5) Ajout du pilotage via terminal dans le fichier term_io.c dans le tableau terminal_motor du type : ACT_DECLARE(CYLINDER_ELEVATOR_RIGHT)
// 6) Un #define CYLINDER_ELEVATOR_RIGHT_RX24_ID doit avoir été ajouté au fichier config_big/config_pin.h ou config_small/config_pin.h
...
static void CYLINDER_ELEVATOR_RIGHT_command_run(queue_id_t queueId);
...
void CYLINDER_ELEVATOR_RIGHT_init() {
	...
	RX24_init();
    CYLINDER_ELEVATOR_RIGHT_initRX24();
}
...
static void CYLINDER_ELEVATOR_RIGHT_initRX24() {
    if(rx24_is_initialized == FALSE && RX24_is_ready(CYLINDER_ELEVATOR_RIGHT_RX24_ID) == TRUE) {
		rx24_is_initialized = TRUE;
        RX24_config_set_lowest_voltage(CYLINDER_ELEVATOR_RIGHT_RX24_ID, RX24_MIN_VOLTAGE);
        RX24_config_set_highest_voltage(CYLINDER_ELEVATOR_RIGHT_RX24_ID, RX24_MAX_VOLTAGE);
        RX24_set_torque_limit(CYLINDER_ELEVATOR_RIGHT_RX24_ID, CYLINDER_ELEVATOR_RIGHT_RX24_MAX_TORQUE_PERCENT);
        RX24_config_set_temperature_limit(CYLINDER_ELEVATOR_RIGHT_RX24_ID, CYLINDER_ELEVATOR_RIGHT_RX24_MAX_TEMPERATURE);

        RX24_config_set_maximal_angle(CYLINDER_ELEVATOR_RIGHT_RX24_ID, CYLINDER_ELEVATOR_RIGHT_RX24_MAX_VALUE);
        RX24_config_set_minimal_angle(CYLINDER_ELEVATOR_RIGHT_RX24_ID, CYLINDER_ELEVATOR_RIGHT_RX24_MIN_VALUE);
        RX24_set_move_to_position_speed(CYLINDER_ELEVATOR_RIGHT_RX24_ID, CYLINDER_ELEVATOR_RIGHT_RX24_SPEED);

        RX24_config_set_error_before_led(CYLINDER_ELEVATOR_RIGHT_RX24_ID, RX24_BEFORE_LED);
        RX24_config_set_error_before_shutdown(CYLINDER_ELEVATOR_RIGHT_RX24_ID, RX24_BEFORE_SHUTDOWN);
...
void CYLINDER_ELEVATOR_RIGHT_set_config(CAN_msg_t* msg){
	switch(msg->data.act_msg.act_data.act_config.sub_act_id){
		case DEFAULT_MONO_ACT : // Premier élement de l'actionneur
            ACTMGR_config_RX24(CYLINDER_ELEVATOR_RIGHT_RX24_ID, msg);
			break;
...
static void CYLINDER_ELEVATOR_RIGHT_get_config(CAN_msg_t *incoming_msg){
	...
	msg.data.act_get_config_answer.sid = 0xFF & ACT_CYLINDER_ELEVATOR_RIGHT;
...
void CYLINDER_ELEVATOR_RIGHT_init_pos(){
    ...
    if(!RX24_set_position(CYLINDER_ELEVATOR_RIGHT_RX24_ID, CYLINDER_ELEVATOR_RIGHT_RX24_INIT_POS))
        debug_printf("   Le RX24 n°%d n'est pas là\n", CYLINDER_ELEVATOR_RIGHT_RX24_ID);
	else
...
void CYLINDER_ELEVATOR_RIGHT_stop(){
    RX24_set_torque_enabled(CYLINDER_ELEVATOR_RIGHT_RX24_ID, FALSE); //Stopper l'asservissement du RX24
}
...
bool_e CYLINDER_ELEVATOR_RIGHT_CAN_process_msg(CAN_msg_t* msg) {
    if(msg->sid == ACT_CYLINDER_ELEVATOR_RIGHT){
        CYLINDER_ELEVATOR_RIGHT_initRX24();
		switch(msg->data.act_msg.order) {
			// Listing de toutes les positions de l'actionneur possible
            case ACT_CYLINDER_ELEVATOR_RIGHT_IDLE :
            case ACT_CYLINDER_ELEVATOR_RIGHT_TOP :
            case ACT_CYLINDER_ELEVATOR_RIGHT_BOTTOM :
            case ACT_CYLINDER_ELEVATOR_RIGHT_LOCK_WITH_CYLINDER:
            case ACT_CYLINDER_ELEVATOR_RIGHT_STOP :
            	run_now = msg->data.act_msg.act_data.act_order.run_now;
                ACTQ_push_operation_from_msg(msg, QUEUE_ACT_RX24_CYLINDER_ELEVATOR_RIGHT, &CYLINDER_ELEVATOR_RIGHT_run_command, 0,TRUE);
				break;

            case ACT_WARNER:
				CYLINDER_ELEVATOR_RIGHT_set_warner(msg);
				break;

			case ACT_GET_CONFIG:
				CYLINDER_ELEVATOR_RIGHT_get_config(msg);
				break;

			case ACT_SET_CONFIG :
                CYLINDER_ELEVATOR_RIGHT_set_config(msg);
				break;
...
void CYLINDER_ELEVATOR_RIGHT_run_command(queue_id_t queueId, bool_e init) {
    ...
    if(QUEUE_get_act(queueId) == QUEUE_ACT_RX24_CYLINDER_ELEVATOR_RIGHT) {    // Gestion des mouvements de CYLINDER_ELEVATOR_RIGHT
		if(init)
            CYLINDER_ELEVATOR_RIGHT_command_init(queueId);
		else
            CYLINDER_ELEVATOR_RIGHT_command_run(queueId);
	}
}
...
static void CYLINDER_ELEVATOR_RIGHT_command_init(queue_id_t queueId) {
	...
	*rx24_goalPosition = 0xFFFF;
	...
	switch(command) {
        ...
        case ACT_CYLINDER_ELEVATOR_RIGHT_IDLE : *rx24_goalPosition = CYLINDER_ELEVATOR_RIGHT_RX24_IDLE_POS; break;
        case ACT_CYLINDER_ELEVATOR_RIGHT_TOP : *rx24_goalPosition = CYLINDER_ELEVATOR_RIGHT_RX24_TOP_POS; break;
        case ACT_CYLINDER_ELEVATOR_RIGHT_BOTTOM : *rx24_goalPosition = CYLINDER_ELEVATOR_RIGHT_RX24_BOTTOM_POS; break;
        case ACT_CYLINDER_ELEVATOR_RIGHT_LOCK_WITH_CYLINDER : *rx24_goalPosition = CYLINDER_ELEVATOR_RIGHT_RX24_LOCK_WITH_CYLINDER_POS; break;

        case ACT_CYLINDER_ELEVATOR_RIGHT_STOP :
            RX24_set_torque_enabled(CYLINDER_ELEVATOR_RIGHT_RX24_ID, FALSE); //Stopper l'asservissement du RX24
            QUEUE_next(queueId, ACT_CYLINDER_ELEVATOR_RIGHT, ACT_RESULT_DONE, ACT_RESULT_ERROR_OK, __LINE__);
			return;

		default: {
            error_printf("Invalid CYLINDER_ELEVATOR_RIGHT command: %u, code is broken !\n", command);
            QUEUE_next(queueId, ACT_CYLINDER_ELEVATOR_RIGHT, ACT_RESULT_NOT_HANDLED, ACT_RESULT_ERROR_LOGIC, __LINE__);
			return;
		}
	}
	...
	if(rx24_is_initialized == FALSE){
        ...
        QUEUE_next(queueId, ACT_CYLINDER_ELEVATOR_RIGHT, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
    ...
    RX24_reset_last_error(CYLINDER_ELEVATOR_RIGHT_RX24_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
    if(!RX24_set_position(CYLINDER_ELEVATOR_RIGHT_RX24_ID, *rx24_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que le RX24 ne répond pas a cet envoi, on l'indique à la carte stratégie
        error_printf("RX24_set_position error: 0x%x\n", RX24_get_last_error(CYLINDER_ELEVATOR_RIGHT_RX24_ID).error);
        QUEUE_next(queueId, ACT_CYLINDER_ELEVATOR_RIGHT, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	//La commande a été envoyée et le RX24 l'a bien reçu
    debug_printf("Move CYLINDER_ELEVATOR_RIGHT rx24 to %d\n", *rx24_goalPosition);
}
...
static void CYLINDER_ELEVATOR_RIGHT_command_run(queue_id_t queueId) {
	...
	if(!run_now){
		...
		ACTQ_flush_queue_to_run_now(queueId, ACT_CYLINDER_ELEVATOR_RIGHT);
		run_now = FALSE;
	}
...
static void CYLINDER_ELEVATOR_RIGHT_set_warner(CAN_msg_t *msg){
	...
	warner.last_pos = RX24_get_position(CYLINDER_ELEVATOR_RIGHT_RX24_ID);

	switch(msg->data.act_msg.act_data.warner_pos){
		case ACT_CYLINDER_ELEVATOR_RIGHT_WARNER:
			warner.warner_pos = CYLINDER_ELEVATOR_RIGHT_RX24_WARNER_POS;
			break;
		default:{
...
static void CYLINDER_ELEVATOR_RIGHT_check_warner(Uint16 pos){
	...
	if( (warner.last_pos < pos && warner.last_pos < warner.warner_pos && pos > warner.warner_pos)
	 ...
	 || (warner.last_pos < RX24_MIN_WARNER && pos > RX24_MAX_WARNER	&& (warner.last_pos > warner.warner_pos || pos < warner.warner_pos))
		...
		msg.data.act_warner_answer.sid = 0xFF & ACT_CYLINDER_ELEVATOR_RIGHT;
		CAN_send(&msg);
