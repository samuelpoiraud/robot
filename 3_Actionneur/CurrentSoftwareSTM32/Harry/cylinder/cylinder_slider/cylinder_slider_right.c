...
*
*	Fichier : CYLINDER_SLIDER_RIGHT_rx24.c
...
#include "cylinder_slider_right.h"


// CYLINDER_SLIDER_RIGHT d'un actionneur standart avec RX24
...
//		Formatage : QUEUE_ACT_RX24_CYLINDER_SLIDER_RIGHT
...
// 5) Ajout du pilotage via terminal dans le fichier term_io.c dans le tableau terminal_motor du type : ACT_DECLARE(CYLINDER_SLIDER_RIGHT)
// 6) Un #define CYLINDER_SLIDER_RIGHT_RX24_ID doit avoir été ajouté au fichier config_big/config_pin.h ou config_small/config_pin.h
...
#include "cylinder_slider_right_config.h"

// Les différents define pour le verbose sur uart
#define LOG_PREFIX "cylinder_slider_right.c : "
#define LOG_COMPONENT OUTPUT_LOG_COMPONENT_CYLINDER_SLIDER_RIGHT
...
static void CYLINDER_SLIDER_RIGHT_get_position_config(ACT_order_e *pOrder, Uint16 *pPos);
...
static void CYLINDER_SLIDER_RIGHT_check_warner(Uint16 pos);
...
void CYLINDER_SLIDER_RIGHT_init() {
	...
	RX24_init();
    CYLINDER_SLIDER_RIGHT_initRX24();
}
...
void CYLINDER_SLIDER_RIGHT_reset_config(){
...
static void CYLINDER_SLIDER_RIGHT_initRX24() {
    if(rx24_is_initialized == FALSE && RX24_is_ready(CYLINDER_SLIDER_RIGHT_RX24_ID) == TRUE) {
		rx24_is_initialized = TRUE;
        RX24_config_set_lowest_voltage(CYLINDER_SLIDER_RIGHT_RX24_ID, RX24_MIN_VOLTAGE);
        RX24_config_set_highest_voltage(CYLINDER_SLIDER_RIGHT_RX24_ID, RX24_MAX_VOLTAGE);
        RX24_set_torque_limit(CYLINDER_SLIDER_RIGHT_RX24_ID, CYLINDER_SLIDER_RIGHT_RX24_MAX_TORQUE_PERCENT);
        RX24_config_set_temperature_limit(CYLINDER_SLIDER_RIGHT_RX24_ID, CYLINDER_SLIDER_RIGHT_RX24_MAX_TEMPERATURE);

        RX24_config_set_maximal_angle(CYLINDER_SLIDER_RIGHT_RX24_ID, CYLINDER_SLIDER_RIGHT_RX24_MAX_VALUE);
        RX24_config_set_minimal_angle(CYLINDER_SLIDER_RIGHT_RX24_ID, CYLINDER_SLIDER_RIGHT_RX24_MIN_VALUE);
        RX24_set_move_to_position_speed(CYLINDER_SLIDER_RIGHT_RX24_ID, CYLINDER_SLIDER_RIGHT_RX24_SPEED);

        RX24_config_set_error_before_led(CYLINDER_SLIDER_RIGHT_RX24_ID, RX24_BEFORE_LED);
        RX24_config_set_error_before_shutdown(CYLINDER_SLIDER_RIGHT_RX24_ID, RX24_BEFORE_SHUTDOWN);
...
static void CYLINDER_SLIDER_RIGHT_get_config(CAN_msg_t *incoming_msg){
	...
	switch(incoming_msg->data.act_msg.act_data.config){
		case POSITION_CONFIG:
			CYLINDER_SLIDER_RIGHT_get_position_config(&order, &pos);
			...
			break;
		case TORQUE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.torque = RX24_get_speed_percentage(CYLINDER_SLIDER_RIGHT_RX24_ID);
			break;
		case TEMPERATURE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.temperature = RX24_get_temperature(CYLINDER_SLIDER_RIGHT_RX24_ID);
			break;
		case LOAD_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.load = RX24_get_load_percentage(CYLINDER_SLIDER_RIGHT_RX24_ID);
			break;
		default:
			error = TRUE;
...
static void CYLINDER_SLIDER_RIGHT_get_position_config(ACT_order_e *pOrder, Uint16 *pPos){
	ACT_order_e order = ACT_CYLINDER_SLIDER_RIGHT_STOP;
	...
	if(position > CYLINDER_SLIDER_RIGHT_RX24_IN_POS - epsilon && position < CYLINDER_SLIDER_RIGHT_RX24_IN_POS + epsilon){
		...
		order = ACT_CYLINDER_SLIDER_RIGHT_ALMOST_OUT;
...
void CYLINDER_SLIDER_RIGHT_init_pos(){
    CYLINDER_SLIDER_RIGHT_initRX24();
    ...
    if(!RX24_set_position(CYLINDER_SLIDER_RIGHT_RX24_ID, CYLINDER_SLIDER_RIGHT_RX24_INIT_POS))
        debug_printf("   Le RX24 n°%d n'est pas là\n", CYLINDER_SLIDER_RIGHT_RX24_ID);
	else
        debug_printf("   Le RX24 n°%d a été initialisé en position\n", CYLINDER_SLIDER_RIGHT_RX24_ID);
}

// Fonction appellée à la fin du match (via ActManager)
void CYLINDER_SLIDER_RIGHT_stop(){
...
bool_e CYLINDER_SLIDER_RIGHT_CAN_process_msg(CAN_msg_t* msg) {
    if(msg->sid == ACT_CYLINDER_SLIDER_RIGHT){
        CYLINDER_SLIDER_RIGHT_initRX24();
		switch(msg->data.act_msg.order) {
			// Listing de toutes les positions de l'actionneur possible
            case ACT_CYLINDER_SLIDER_RIGHT_IDLE :
            case ACT_CYLINDER_SLIDER_RIGHT_IN :
...
void CYLINDER_SLIDER_RIGHT_run_command(queue_id_t queueId, bool_e init) {
	if(QUEUE_has_error(queueId)) {
		QUEUE_behead(queueId);
		return;
	}

    if(QUEUE_get_act(queueId) == QUEUE_ACT_RX24_CYLINDER_SLIDER_RIGHT) {    // Gestion des mouvements de CYLINDER_SLIDER_RIGHT
		if(init)
            CYLINDER_SLIDER_RIGHT_command_init(queueId);
		else
            CYLINDER_SLIDER_RIGHT_command_run(queueId);
	}
}
...
static void CYLINDER_SLIDER_RIGHT_command_init(queue_id_t queueId) {
	...
	*rx24_goalPosition = 0xFFFF;
	...
	switch(command) {
        ...
        case ACT_CYLINDER_SLIDER_RIGHT_IDLE : *rx24_goalPosition = CYLINDER_SLIDER_RIGHT_RX24_IDLE_POS; break;
        case ACT_CYLINDER_SLIDER_RIGHT_IN : *rx24_goalPosition = CYLINDER_SLIDER_RIGHT_RX24_IN_POS; break;
        ...
        case ACT_CYLINDER_SLIDER_RIGHT_STOP :
            RX24_set_torque_enabled(CYLINDER_SLIDER_RIGHT_RX24_ID, FALSE); //Stopper l'asservissement du RX24
            QUEUE_next(queueId, ACT_CYLINDER_SLIDER_RIGHT, ACT_RESULT_DONE, ACT_RESULT_ERROR_OK, __LINE__);
			return;

		default: {
            error_printf("Invalid CYLINDER_SLIDER_RIGHT command: %u, code is broken !\n", command);
            QUEUE_next(queueId, ACT_CYLINDER_SLIDER_RIGHT, ACT_RESULT_NOT_HANDLED, ACT_RESULT_ERROR_LOGIC, __LINE__);
			return;
		}
	}
	...
	if(rx24_is_initialized == FALSE){
        ...
        QUEUE_next(queueId, ACT_CYLINDER_SLIDER_RIGHT, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
    ...
    RX24_reset_last_error(CYLINDER_SLIDER_RIGHT_RX24_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
    if(!RX24_set_position(CYLINDER_SLIDER_RIGHT_RX24_ID, *rx24_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que le RX24 ne répond pas a cet envoi, on l'indique à la carte stratégie
        error_printf("RX24_set_position error: 0x%x\n", RX24_get_last_error(CYLINDER_SLIDER_RIGHT_RX24_ID).error);
        QUEUE_next(queueId, ACT_CYLINDER_SLIDER_RIGHT, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	//La commande a été envoyée et le RX24 l'a bien reçu
    debug_printf("Move CYLINDER_SLIDER_RIGHT rx24 to %d\n", *rx24_goalPosition);
}
...
static void CYLINDER_SLIDER_RIGHT_command_run(queue_id_t queueId) {
	...
	if(!run_now){
		if(ACTQ_check_status_rx24(queueId, CYLINDER_SLIDER_RIGHT_RX24_ID, QUEUE_get_arg(queueId)->param, pos, CYLINDER_SLIDER_RIGHT_RX24_ASSER_POS_EPSILON, CYLINDER_SLIDER_RIGHT_RX24_ASSER_TIMEOUT, CYLINDER_SLIDER_RIGHT_RX24_ASSER_POS_LARGE_EPSILON, &result, &errorCode, &line))
		...
		ACTQ_flush_queue_to_run_now(queueId, ACT_CYLINDER_SLIDER_RIGHT);
	}

    // On ne surveille le warner que si il est activé
	if(warner.activated)
		CYLINDER_SLIDER_RIGHT_check_warner(pos);
}
