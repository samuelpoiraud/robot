...
static ACT_order_e CYLINDER_SLIDER_LEFT_get_position_config();
...
static void CYLINDER_SLIDER_LEFT_check_warner();
...
static void CYLINDER_SLIDER_LEFT_get_config(CAN_msg_t *incoming_msg){
	...
	switch(incoming_msg->data.act_msg.act_data.config){
		case POSITION_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.pos = CYLINDER_SLIDER_LEFT_get_position_config();
			CAN_send(&msg); // Envoi du message CAN
			break;
		case SPEED_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.speed = RX24_get_speed_percentage(CYLINDER_SLIDER_LEFT_RX24_ID);
			CAN_send(&msg); // Envoi du message CAN
			break;
		default:
...
static ACT_order_e CYLINDER_SLIDER_LEFT_get_position_config(){
	ACT_order_e current_state = ACT_CYLINDER_SLIDER_LEFT_STOP;
	...
	if(position > CYLINDER_SLIDER_LEFT_RX24_IN_POS - epsilon && position < CYLINDER_SLIDER_LEFT_RX24_IN_POS + epsilon){
		...
		current_state = ACT_CYLINDER_SLIDER_LEFT_ALMOST_OUT;
...
static void CYLINDER_SLIDER_LEFT_command_run(queue_id_t queueId) {
	...
	if(warner.activated)
		CYLINDER_SLIDER_LEFT_check_warner();
}
