...
static void CYLINDER_SLIDER_LEFT_set_config(CAN_msg_t* msg);
static void CYLINDER_SLIDER_LEFT_get_position_config(ACT_order_e *pOrder, Uint16 *pPos);
static void CYLINDER_SLIDER_LEFT_get_config(CAN_msg_t *incoming_msg);
static void CYLINDER_SLIDER_LEFT_set_warner(CAN_msg_t *msg);
static void CYLINDER_SLIDER_LEFT_check_warner(Uint16 pos);
...
static void CYLINDER_SLIDER_LEFT_initRX24() {
    if(rx24_is_initialized == FALSE && RX24_is_ready(CYLINDER_SLIDER_LEFT_RX24_ID) == TRUE) {
        ...
        RX24_set_move_to_position_speed(CYLINDER_SLIDER_LEFT_RX24_ID, CYLINDER_SLIDER_LEFT_RX24_SPEED);
...
void CYLINDER_SLIDER_LEFT_set_config(CAN_msg_t* msg){
...
static void CYLINDER_SLIDER_LEFT_get_config(CAN_msg_t *incoming_msg){
	...
	msg.data.act_get_config_answer.sid = 0xFF & ACT_CYLINDER_SLIDER_LEFT;
	...
	switch(incoming_msg->data.act_msg.act_data.config){
		case POSITION_CONFIG:
			CYLINDER_SLIDER_LEFT_get_position_config(&order, &pos);
			...
			break;
		case TORQUE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.torque = RX24_get_speed_percentage(CYLINDER_SLIDER_LEFT_RX24_ID);
			break;
		case TEMPERATURE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.temperature = RX24_get_temperature(CYLINDER_SLIDER_LEFT_RX24_ID);
			break;
		case LOAD_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.load = RX24_get_load_percentage(CYLINDER_SLIDER_LEFT_RX24_ID);
			break;
		default:
			error = TRUE;
...
static void CYLINDER_SLIDER_LEFT_get_position_config(ACT_order_e *pOrder, Uint16 *pPos){
	ACT_order_e order = ACT_CYLINDER_SLIDER_LEFT_STOP;
	Uint16 position = RX24_get_position(CYLINDER_SLIDER_LEFT_RX24_ID);
	Uint16 epsilon = CYLINDER_SLIDER_LEFT_RX24_ASSER_POS_EPSILON;

	if(position > CYLINDER_SLIDER_LEFT_RX24_IN_POS - epsilon && position < CYLINDER_SLIDER_LEFT_RX24_IN_POS + epsilon){
		order = ACT_CYLINDER_SLIDER_LEFT_IN;
	}else if(position > CYLINDER_SLIDER_LEFT_RX24_OUT_POS - epsilon && position < CYLINDER_SLIDER_LEFT_RX24_OUT_POS + epsilon){
		order = ACT_CYLINDER_SLIDER_LEFT_OUT;
	}else if(position > CYLINDER_SLIDER_LEFT_RX24_ALMOST_OUT_POS - epsilon && position < CYLINDER_SLIDER_LEFT_RX24_ALMOST_OUT_POS + epsilon){
		order = ACT_CYLINDER_SLIDER_LEFT_ALMOST_OUT;
	}else if(position > CYLINDER_SLIDER_LEFT_RX24_ALMOST_OUT_WITH_CYLINDER_POS - epsilon && position < CYLINDER_SLIDER_LEFT_RX24_ALMOST_OUT_WITH_CYLINDER_POS + epsilon){
		order = ACT_CYLINDER_SLIDER_LEFT_ALMOST_OUT_WITH_CYLINDER;
	}else if(position > CYLINDER_SLIDER_LEFT_RX24_IDLE_POS - epsilon && position < CYLINDER_SLIDER_LEFT_RX24_IDLE_POS + epsilon){
		order = ACT_CYLINDER_SLIDER_LEFT_IDLE;
	}else if(position > CYLINDER_SLIDER_LEFT_RX24_HARVEST_POS - epsilon && position < CYLINDER_SLIDER_LEFT_RX24_HARVEST_POS + epsilon){
		order = ACT_CYLINDER_SLIDER_LEFT_HARVEST;
	}
...
bool_e CYLINDER_SLIDER_LEFT_CAN_process_msg(CAN_msg_t* msg) {
    if(msg->sid == ACT_CYLINDER_SLIDER_LEFT){
		...
		switch(msg->data.act_msg.order) {
            ...
            case ACT_CYLINDER_SLIDER_LEFT_HARVEST:
            ...
            case ACT_WARNER:
				CYLINDER_SLIDER_LEFT_set_warner(msg);
				break;

			case ACT_GET_CONFIG:
				CYLINDER_SLIDER_LEFT_get_config(msg);
				break;

			case ACT_SET_CONFIG :
                CYLINDER_SLIDER_LEFT_set_config(msg);
				break;
...
static void CYLINDER_SLIDER_LEFT_command_init(queue_id_t queueId) {
	...
	*rx24_goalPosition = 0xFFFF;
	...
	switch(command) {
        ...
        case ACT_CYLINDER_SLIDER_LEFT_HARVEST : *rx24_goalPosition = CYLINDER_SLIDER_LEFT_RX24_HARVEST_POS; break;
...
static void CYLINDER_SLIDER_LEFT_command_run(queue_id_t queueId) {
	...
	Uint16 pos = RX24_get_position(CYLINDER_SLIDER_LEFT_RX24_ID);

	if(!run_now){
		if(ACTQ_check_status_rx24(queueId, CYLINDER_SLIDER_LEFT_RX24_ID, QUEUE_get_arg(queueId)->param, pos, CYLINDER_SLIDER_LEFT_RX24_ASSER_POS_EPSILON, CYLINDER_SLIDER_LEFT_RX24_ASSER_TIMEOUT, CYLINDER_SLIDER_LEFT_RX24_ASSER_POS_LARGE_EPSILON, &result, &errorCode, &line))
		...
		ACTQ_flush_queue_to_run_now(queueId, ACT_CYLINDER_SLIDER_LEFT);
	}

    // On ne surveille le warner que si il est activé
	if(warner.activated)
		CYLINDER_SLIDER_LEFT_check_warner(pos);
}
...
static void CYLINDER_SLIDER_LEFT_check_warner(Uint16 pos){
	...
	if( (warner.last_pos < pos && warner.last_pos < warner.warner_pos && pos > warner.warner_pos)
	 ...
	 || (warner.last_pos < RX24_MIN_WARNER && pos > RX24_MAX_WARNER	&& (warner.last_pos > warner.warner_pos || pos < warner.warner_pos))
		...
		msg.data.act_warner_answer.sid = 0xFF & ACT_CYLINDER_SLIDER_LEFT;
		CAN_send(&msg);
