...
static ACT_order_e CYLINDER_COLOR_RIGHT_get_position_config();
static void CYLINDER_COLOR_RIGHT_set_warner(CAN_msg_t *msg);
static void CYLINDER_COLOR_RIGHT_check_warner();
...
static void CYLINDER_COLOR_RIGHT_initRX24() {
	if(rx24_is_initialized == FALSE && RX24_is_ready(CYLINDER_COLOR_RIGHT_RX24_ID) == TRUE) {
		...
		RX24_config_set_maximal_angle(CYLINDER_COLOR_RIGHT_RX24_ID, CYLINDER_COLOR_RIGHT_RX24_MAX_VALUE);
		RX24_config_set_minimal_angle(CYLINDER_COLOR_RIGHT_RX24_ID, CYLINDER_COLOR_RIGHT_RX24_MIN_VALUE);
...
static void CYLINDER_COLOR_RIGHT_get_config(CAN_msg_t *incoming_msg){
	...
	switch(incoming_msg->data.act_msg.act_data.config){
		case POSITION_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.pos = CYLINDER_COLOR_RIGHT_get_position_config();
			CAN_send(&msg); // Envoi du message CAN
			break;
		case SPEED_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.speed = RX24_get_speed_percentage(CYLINDER_COLOR_RIGHT_RX24_ID);
			CAN_send(&msg); // Envoi du message CAN
			break;
		default:
...
static ACT_order_e CYLINDER_COLOR_RIGHT_get_position_config(){
	ACT_order_e current_state = ACT_CYLINDER_COLOR_RIGHT_STOP;
...
static void CYLINDER_COLOR_RIGHT_command_run(queue_id_t queueId) {
	...
	if(ACTQ_check_status_rx24(queueId, CYLINDER_COLOR_RIGHT_RX24_ID, QUEUE_get_arg(queueId)->param, CYLINDER_COLOR_RIGHT_RX24_ASSER_POS_EPSILON, CYLINDER_COLOR_RIGHT_RX24_ASSER_TIMEOUT, CYLINDER_COLOR_RIGHT_RX24_ASSER_POS_LARGE_EPSILON, &result, &errorCode, &line))
		QUEUE_next(queueId, ACT_CYLINDER_COLOR_RIGHT, result, errorCode, line);

    // On ne surveille le warner que si il est activé
	if(warner.activated)
		CYLINDER_COLOR_RIGHT_check_warner();
}
