 ...
 *
 *	Fichier : CYLINDER_PUSHER_LEFT_ax12.c
...
#include "cylinder_pusher_left.h"


// CYLINDER_PUSHER_LEFT d'un actionneur standart avec AX12
...
//		Formatage : QUEUE_ACT_AX12_CYLINDER_PUSHER_LEFT
...
// 5) Ajout du pilotage via terminal dans le fichier term_io.c dans le tableau terminal_motor du type : ACT_DECLARE(CYLINDER_PUSHER_LEFT)
// 6) Un #define CYLINDER_PUSHER_LEFT_AX12_ID doit avoir été ajouté au fichier config_big/config_pin.h ou config_small/config_pin.h
...
#include "cylinder_pusher_left_config.h"

// Les différents define pour le verbose sur uart
#define LOG_PREFIX "cylinder_pusher_left.c : "
#define LOG_COMPONENT OUTPUT_LOG_COMPONENT_CYLINDER_PUSHER_LEFT
#include "../QS/QS_outputlog.h"

// Les fonctions internes au fonctionnement de l'actionneur
static void CYLINDER_PUSHER_LEFT_command_run(queue_id_t queueId);
static void CYLINDER_PUSHER_LEFT_initAX12();
static void CYLINDER_PUSHER_LEFT_command_init(queue_id_t queueId);
static void CYLINDER_PUSHER_LEFT_set_config(CAN_msg_t* msg);
static void CYLINDER_PUSHER_LEFT_get_position_config(ACT_order_e *pOrder, Uint16 *pPos);;
static void CYLINDER_PUSHER_LEFT_get_config(CAN_msg_t *incoming_msg);
static void CYLINDER_PUSHER_LEFT_set_warner(CAN_msg_t *msg);
static void CYLINDER_PUSHER_LEFT_check_warner(Uint16 pos);
...
static void CYLINDER_PUSHER_LEFT_initAX12() {
    if(ax12_is_initialized == FALSE && AX12_is_ready(CYLINDER_PUSHER_LEFT_AX12_ID) == TRUE) {
		ax12_is_initialized = TRUE;
        AX12_config_set_lowest_voltage(CYLINDER_PUSHER_LEFT_AX12_ID, AX12_MIN_VOLTAGE);
        AX12_config_set_highest_voltage(CYLINDER_PUSHER_LEFT_AX12_ID, AX12_MAX_VOLTAGE);
        AX12_set_torque_limit(CYLINDER_PUSHER_LEFT_AX12_ID, CYLINDER_PUSHER_LEFT_AX12_MAX_TORQUE_PERCENT);
        AX12_config_set_temperature_limit(CYLINDER_PUSHER_LEFT_AX12_ID, CYLINDER_PUSHER_LEFT_AX12_MAX_TEMPERATURE);

        AX12_config_set_maximal_angle(CYLINDER_PUSHER_LEFT_AX12_ID, CYLINDER_PUSHER_LEFT_AX12_MAX_VALUE);
        AX12_config_set_minimal_angle(CYLINDER_PUSHER_LEFT_AX12_ID, CYLINDER_PUSHER_LEFT_AX12_MIN_VALUE);
        AX12_set_move_to_position_speed(CYLINDER_PUSHER_LEFT_AX12_ID, CYLINDER_PUSHER_LEFT_AX12_SPEED);

        AX12_config_set_error_before_led(CYLINDER_PUSHER_LEFT_AX12_ID, AX12_BEFORE_LED);
        AX12_config_set_error_before_shutdown(CYLINDER_PUSHER_LEFT_AX12_ID, AX12_BEFORE_SHUTDOWN);
...
void CYLINDER_PUSHER_LEFT_set_config(CAN_msg_t* msg){
	switch(msg->data.act_msg.act_data.act_config.sub_act_id){
		case DEFAULT_MONO_ACT : // Premier élement de l'actionneur
            ACTMGR_config_AX12(CYLINDER_PUSHER_LEFT_AX12_ID, msg);
			break;
...
static void CYLINDER_PUSHER_LEFT_get_config(CAN_msg_t *incoming_msg){
	...
	msg.data.act_get_config_answer.sid = 0xFF & ACT_CYLINDER_PUSHER_LEFT;
	...
	switch(incoming_msg->data.act_msg.act_data.config){
		case POSITION_CONFIG:
			CYLINDER_PUSHER_LEFT_get_position_config(&order, &pos);
			...
			break;
		case TORQUE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.torque = AX12_get_speed_percentage(CYLINDER_PUSHER_LEFT_AX12_ID);
			break;
		case TEMPERATURE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.temperature = AX12_get_temperature(CYLINDER_PUSHER_LEFT_AX12_ID);
			break;
		case LOAD_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.load = AX12_get_load_percentage(CYLINDER_PUSHER_LEFT_AX12_ID);
			break;
		default:
			error = TRUE;
...
static void CYLINDER_PUSHER_LEFT_get_position_config(ACT_order_e *pOrder, Uint16 *pPos){
	ACT_order_e order = ACT_CYLINDER_PUSHER_LEFT_STOP;
	Uint16 position = AX12_get_position(CYLINDER_PUSHER_LEFT_AX12_ID);
	Uint16 epsilon = CYLINDER_PUSHER_LEFT_AX12_ASSER_POS_EPSILON;

	if(position > CYLINDER_PUSHER_LEFT_AX12_IN_POS - epsilon && position < CYLINDER_PUSHER_LEFT_AX12_IN_POS + epsilon){
		order = ACT_CYLINDER_PUSHER_LEFT_IN;
	}else if(position > CYLINDER_PUSHER_LEFT_AX12_OUT_POS - epsilon && position < CYLINDER_PUSHER_LEFT_AX12_OUT_POS + epsilon){
	...
	}else if(position > CYLINDER_PUSHER_LEFT_AX12_HIT_POS - epsilon && position < CYLINDER_PUSHER_LEFT_AX12_HIT_POS + epsilon){
		order = ACT_CYLINDER_PUSHER_LEFT_HIT;
	}else if(position > CYLINDER_PUSHER_LEFT_AX12_PREVENT_DEPOSE_POS - epsilon && position < CYLINDER_PUSHER_LEFT_AX12_PREVENT_DEPOSE_POS + epsilon){
		order = ACT_CYLINDER_PUSHER_LEFT_PREVENT_DEPOSE;
	}
...
void CYLINDER_PUSHER_LEFT_init_pos(){
    ...
    if(!AX12_set_position(CYLINDER_PUSHER_LEFT_AX12_ID, CYLINDER_PUSHER_LEFT_AX12_INIT_POS))
        debug_printf("   L'AX12 n°%d n'est pas là\n", CYLINDER_PUSHER_LEFT_AX12_ID);
	else
...
bool_e CYLINDER_PUSHER_LEFT_CAN_process_msg(CAN_msg_t* msg) {
    if(msg->sid == ACT_CYLINDER_PUSHER_LEFT){
        CYLINDER_PUSHER_LEFT_initAX12();
		switch(msg->data.act_msg.order) {
            ...
            case ACT_CYLINDER_PUSHER_LEFT_HIT :
            case ACT_CYLINDER_PUSHER_LEFT_PREVENT_DEPOSE :
            ...
            case ACT_WARNER:
				CYLINDER_PUSHER_LEFT_set_warner(msg);
				break;

			case ACT_GET_CONFIG:
				CYLINDER_PUSHER_LEFT_get_config(msg);
				break;

			case ACT_SET_CONFIG :
                CYLINDER_PUSHER_LEFT_set_config(msg);
				break;
...
void CYLINDER_PUSHER_LEFT_run_command(queue_id_t queueId, bool_e init) {
    ...
    if(QUEUE_get_act(queueId) == QUEUE_ACT_AX12_CYLINDER_PUSHER_LEFT) {    // Gestion des mouvements de CYLINDER_PUSHER_LEFT
		if(init)
            CYLINDER_PUSHER_LEFT_command_init(queueId);
		else
            CYLINDER_PUSHER_LEFT_command_run(queueId);
	}
}
...
static void CYLINDER_PUSHER_LEFT_command_init(queue_id_t queueId) {
	...
	*ax12_goalPosition = 0xFFFF;
	...
	switch(command) {
        ...
        case ACT_CYLINDER_PUSHER_LEFT_HIT : *ax12_goalPosition = CYLINDER_PUSHER_LEFT_AX12_HIT_POS; break;
        case ACT_CYLINDER_PUSHER_LEFT_PREVENT_DEPOSE : *ax12_goalPosition = CYLINDER_PUSHER_LEFT_AX12_PREVENT_DEPOSE_POS; break;
        case ACT_CYLINDER_PUSHER_LEFT_STOP :
            AX12_set_torque_enabled(CYLINDER_PUSHER_LEFT_AX12_ID, FALSE); //Stopper l'asservissement de l'AX12
		...
		default: {
            error_printf("Invalid CYLINDER_PUSHER_LEFT command: %u, code is broken !\n", command);
            QUEUE_next(queueId, ACT_CYLINDER_PUSHER_LEFT, ACT_RESULT_NOT_HANDLED, ACT_RESULT_ERROR_LOGIC, __LINE__);
			return;
		}
	}
	...
	if(ax12_is_initialized == FALSE){
        ...
        QUEUE_next(queueId, ACT_CYLINDER_PUSHER_LEFT, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
    ...
    AX12_reset_last_error(CYLINDER_PUSHER_LEFT_AX12_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
    if(!AX12_set_position(CYLINDER_PUSHER_LEFT_AX12_ID, *ax12_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que l'AX12 ne répond pas a cet envoi, on l'indique à la carte stratégie
        error_printf("AX12_set_position error: 0x%x\n", AX12_get_last_error(CYLINDER_PUSHER_LEFT_AX12_ID).error);
        QUEUE_next(queueId, ACT_CYLINDER_PUSHER_LEFT, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	//La commande a été envoyée et l'AX12 l'a bien reçu
    debug_printf("Move CYLINDER_PUSHER_LEFT ax12 to %d\n", *ax12_goalPosition);
}
...
static void CYLINDER_PUSHER_LEFT_command_run(queue_id_t queueId) {
	...
	Uint16 pos = AX12_get_position(CYLINDER_PUSHER_LEFT_AX12_ID);

	if(!run_now){
		if(ACTQ_check_status_ax12(queueId, CYLINDER_PUSHER_LEFT_AX12_ID, QUEUE_get_arg(queueId)->param, pos, CYLINDER_PUSHER_LEFT_AX12_ASSER_POS_EPSILON, CYLINDER_PUSHER_LEFT_AX12_ASSER_TIMEOUT, CYLINDER_PUSHER_LEFT_AX12_ASSER_POS_LARGE_EPSILON, &result, &errorCode, &line))
			QUEUE_next(queueId, ACT_CYLINDER_PUSHER_LEFT, result, errorCode, line);
	}else{
		// Lorsqu'on vient de recevoir un run_now, on passe directement à l'ordre reçu.
		ACTQ_flush_queue_to_run_now(queueId, ACT_CYLINDER_PUSHER_LEFT);
		run_now = FALSE;
	}

    // On ne surveille le warner que si il est activé
	if(warner.activated)
		CYLINDER_PUSHER_LEFT_check_warner(pos);
}

static void CYLINDER_PUSHER_LEFT_set_warner(CAN_msg_t *msg){
	warner.activated = TRUE;
	warner.last_pos = AX12_get_position(CYLINDER_PUSHER_LEFT_AX12_ID);

	switch(msg->data.act_msg.act_data.warner_pos){
		/*case ACT_CYLINDER_PUSHER_LEFT_WARNER:
			warner.warner_pos = CYLINDER_PUSHER_LEFT_AX12_WARNER_POS;
			break;*/
		default:{
...
static void CYLINDER_PUSHER_LEFT_check_warner(Uint16 pos){
	...
	if( (warner.last_pos < pos && warner.last_pos < warner.warner_pos && pos > warner.warner_pos)
	 ...
	 || (warner.last_pos < AX12_MIN_WARNER && pos > AX12_MAX_WARNER	&& (warner.last_pos > warner.warner_pos || pos < warner.warner_pos))
		...
		msg.data.act_warner_answer.sid = 0xFF & ACT_CYLINDER_PUSHER_LEFT;
		CAN_send(&msg);
