...
/*static void CUP_NIPPER_initAX12() {
	if(ax12_is_initialized == FALSE && AX12_is_ready(CUP_NIPPER_AX12_ID) == TRUE) {
		...
		AX12_config_set_highest_voltage(CUP_NIPPER_AX12_ID, AX12_MAX_VOLTAGE);
		AX12_set_torque_limit(CUP_NIPPER_AX12_ID, CUP_NIPPER_AX12_MAX_TORQUE_PERCENT);
		AX12_config_set_temperature_limit(CUP_NIPPER_AX12_ID, CUP_NIPPER_AX12_MAX_TEMPERATURE);
		...
		AX12_config_set_error_before_led(CUP_NIPPER_AX12_ID, AX12_BEFORE_LED);
		AX12_config_set_error_before_shutdown(CUP_NIPPER_AX12_ID, AX12_BEFORE_SHUTDOWN);
