 ...
 *  Robot : HOLLY
 */
#include "carpet_launcher_left.h"
...
#include "carpet_launcher_left_config.h"

// Les différents define pour le verbose sur uart
#define LOG_PREFIX "carpet_launcher_left : "
#define LOG_COMPONENT OUTPUT_LOG_COMPONENT_CARPET_LAUNCHER_LEFT
#include "../QS/QS_outputlog.h"

// Les fonctions internes au fonctionnement de l'actionneur
static void CARPET_LAUNCHER_LEFT_command_run(queue_id_t queueId);
static void CARPET_LAUNCHER_LEFT_initAX12();
static void CARPET_LAUNCHER_LEFT_command_init(queue_id_t queueId);
static void CARPET_LAUNCHER_LEFT_config(CAN_msg_t* msg);
...
static void CARPET_LAUNCHER_LEFT_initAX12() {
	if(ax12_is_initialized == FALSE && AX12_is_ready(CARPET_LAUNCHER_LEFT_AX12_ID) == TRUE) {
		...
		AX12_config_set_temperature_limit(CARPET_LAUNCHER_LEFT_AX12_ID, CARPET_LAUNCHER_LEFT_AX12_MAX_TEMPERATURE);
...
void CARPET_LAUNCHER_LEFT_config(CAN_msg_t* msg){
	switch(msg->data[1]){
		case 0 : // Premier élement de l'actionneur
			ACTMGR_config_AX12(CARPET_LAUNCHER_LEFT_AX12_ID, msg);
			break;
...
void CARPET_LAUNCHER_LEFT_init_pos(){
	...
	if(!AX12_set_position(CARPET_LAUNCHER_LEFT_AX12_ID, CARPET_LAUNCHER_LEFT_AX12_INIT_POS))
		debug_printf("   L'AX12 n°%d n'est pas là\n", CARPET_LAUNCHER_LEFT_AX12_ID);
	else
...
void CARPET_LAUNCHER_LEFT_stop(){
	AX12_set_torque_enabled(CARPET_LAUNCHER_LEFT_AX12_ID, FALSE); //Stopper l'asservissement de l'AX12
}
...
bool_e CARPET_LAUNCHER_LEFT_CAN_process_msg(CAN_msg_t* msg) {
	if(msg->sid == ACT_CARPET_LAUNCHER_LEFT){
		CARPET_LAUNCHER_LEFT_initAX12();
		switch(msg->data[0]) {
			...
			case ACT_CONFIG :
				CARPET_LAUNCHER_LEFT_config(msg);
				break;
		...
		SELFTEST_set_actions(&CARPET_LAUNCHER_LEFT_run_command, 4, 3, (SELFTEST_action_t[]){
								 {ACT_CARPET_LAUNCHER_LEFT_IDLE,		0,  QUEUE_ACT_AX12_CARPET_LAUNCHER_LEFT},
								 {ACT_CARPET_LAUNCHER_LEFT_LOADING,		0,  QUEUE_ACT_AX12_CARPET_LAUNCHER_LEFT},
								 {ACT_CARPET_LAUNCHER_LEFT_IDLE,		0,  QUEUE_ACT_AX12_CARPET_LAUNCHER_LEFT}
							 });
	}
	return FALSE;
...
void CARPET_LAUNCHER_LEFT_run_command(queue_id_t queueId, bool_e init) {
	...
	if(QUEUE_get_act(queueId) == QUEUE_ACT_AX12_CARPET_LAUNCHER_LEFT) {    // Gestion des mouvements de CARPET_LAUNCHER_LEFT
		if(init)
			CARPET_LAUNCHER_LEFT_command_init(queueId);
		else
			CARPET_LAUNCHER_LEFT_command_run(queueId);
	}
}
...
static void CARPET_LAUNCHER_LEFT_command_init(queue_id_t queueId) {
	...
	*ax12_goalPosition = 0xFFFF;
	...
	switch(command) {
		...
		case ACT_CARPET_LAUNCHER_LEFT_STOP :
			AX12_set_torque_enabled(CARPET_LAUNCHER_LEFT_AX12_ID, FALSE); //Stopper l'asservissement de l'AX12
			QUEUE_next(queueId, ACT_CARPET_LAUNCHER_LEFT, ACT_RESULT_DONE, ACT_RESULT_ERROR_OK, __LINE__);
			return;

		default: {
			error_printf("Invalid exemple command: %u, code is broken !\n", command);
			QUEUE_next(queueId, ACT_CARPET_LAUNCHER_LEFT, ACT_RESULT_NOT_HANDLED, ACT_RESULT_ERROR_LOGIC, __LINE__);
			return;
		}
	}
	...
	if(ax12_is_initialized == FALSE){
		...
		QUEUE_next(queueId, ACT_CARPET_LAUNCHER_LEFT, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	...
	AX12_reset_last_error(CARPET_LAUNCHER_LEFT_AX12_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
	if(!AX12_set_position(CARPET_LAUNCHER_LEFT_AX12_ID, *ax12_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que l'AX12 ne répond pas a cet envoi, on l'indique à la carte stratégie
		error_printf("AX12_set_position error: 0x%x\n", AX12_get_last_error(CARPET_LAUNCHER_LEFT_AX12_ID).error);
		QUEUE_next(queueId, ACT_CARPET_LAUNCHER_LEFT, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	//La commande a été envoyée et l'AX12 l'a bien reçu
	debug_printf("Move CARPET_LAUNCHER_LEFT ax12 to %d\n", *ax12_goalPosition);
}
...
static void CARPET_LAUNCHER_LEFT_command_run(queue_id_t queueId) {
	...
	if(ACTQ_check_status_ax12(queueId, CARPET_LAUNCHER_LEFT_AX12_ID, QUEUE_get_arg(queueId)->param, CARPET_LAUNCHER_LEFT_AX12_ASSER_POS_EPSILON, CARPET_LAUNCHER_LEFT_AX12_ASSER_TIMEOUT, CARPET_LAUNCHER_LEFT_AX12_ASSER_POS_LARGE_EPSILON, &result, &errorCode, &line))
		QUEUE_next(queueId, ACT_CARPET_LAUNCHER_LEFT, result, errorCode, line);
}
