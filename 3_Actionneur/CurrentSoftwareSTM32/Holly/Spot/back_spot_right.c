...
static void BACK_SPOT_RIGHT_initAX12() {
	if(ax12_is_initialized == FALSE && AX12_is_ready(BACK_SPOT_RIGHT_AX12_ID) == TRUE) {
		...
		AX12_config_set_error_before_led(BACK_SPOT_RIGHT_AX12_ID, AX12_ERROR_ANGLE | AX12_ERROR_CHECKSUM | AX12_ERROR_INSTRUCTION | AX12_ERROR_OVERHEATING | AX12_ERROR_OVERLOAD | AX12_ERROR_RANGE);
		AX12_config_set_error_before_shutdown(BACK_SPOT_RIGHT_AX12_ID, AX12_ERROR_OVERHEATING);
...
bool_e BACK_SPOT_RIGHT_CAN_process_msg(CAN_msg_t* msg) {
	if(msg->sid == ACT_BACK_SPOT_RIGHT){
		...
		SELFTEST_set_actions(&BACK_SPOT_RIGHT_run_command, 36, 3, (SELFTEST_action_t[]){
