...
static void POP_DROP_LEFT_initAX12() {
	if(ax12_is_initialized == FALSE && AX12_is_ready(POP_DROP_LEFT_AX12_ID) == TRUE) {
		...
		AX12_config_set_highest_voltage(POP_DROP_LEFT_AX12_ID, 136);
		AX12_config_set_lowest_voltage(POP_DROP_LEFT_AX12_ID, 70);
		AX12_config_set_maximum_torque_percentage(POP_DROP_LEFT_AX12_ID, POP_DROP_LEFT_AX12_MAX_TORQUE_PERCENT);
...
bool_e POP_DROP_LEFT_CAN_process_msg(CAN_msg_t* msg) {
	if(msg->sid == ACT_POP_DROP_LEFT){
		...
		SELFTEST_set_actions(&POP_DROP_LEFT_run_command, 27, 3, (SELFTEST_action_t[]){
