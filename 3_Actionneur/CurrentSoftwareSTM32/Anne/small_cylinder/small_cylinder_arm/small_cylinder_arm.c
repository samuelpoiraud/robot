...
//		Formatage : QUEUE_ACT_RX24_SMALL_CYLINDER_ARM
...
// 5) Ajout du pilotage via terminal dans le fichier term_io.c dans le tableau terminal_motor du type : ACT_DECLARE(SMALL_CYLINDER_ARM)
// 6) Un #define SMALL_CYLINDER_ARM_RX24_ID doit avoir été ajouté au fichier config_big/config_pin.h ou config_small/config_pin.h
...
static void SMALL_CYLINDER_ARM_command_run(queue_id_t queueId);
static void SMALL_CYLINDER_ARM_initRX24();
static void SMALL_CYLINDER_ARM_command_init(queue_id_t queueId);
static void SMALL_CYLINDER_ARM_set_config(CAN_msg_t* msg);
static void SMALL_CYLINDER_ARM_get_position_config(ACT_order_e *pOrder, Uint16 *pPos);
static void SMALL_CYLINDER_ARM_get_config(CAN_msg_t *incoming_msg);
static void SMALL_CYLINDER_ARM_set_warner(CAN_msg_t *msg);
static void SMALL_CYLINDER_ARM_check_warner(Uint16 pos);
...
static void SMALL_CYLINDER_ARM_initRX24() {
    if(rx24_is_initialized == FALSE && RX24_is_ready(SMALL_CYLINDER_ARM_RX24_ID) == TRUE) {
		rx24_is_initialized = TRUE;
        RX24_config_set_lowest_voltage(SMALL_CYLINDER_ARM_RX24_ID, RX24_MIN_VOLTAGE);
        RX24_config_set_highest_voltage(SMALL_CYLINDER_ARM_RX24_ID, RX24_MAX_VOLTAGE);
        RX24_set_torque_limit(SMALL_CYLINDER_ARM_RX24_ID, SMALL_CYLINDER_ARM_RX24_MAX_TORQUE_PERCENT);
        RX24_config_set_temperature_limit(SMALL_CYLINDER_ARM_RX24_ID, SMALL_CYLINDER_ARM_RX24_MAX_TEMPERATURE);

        RX24_config_set_maximal_angle(SMALL_CYLINDER_ARM_RX24_ID, SMALL_CYLINDER_ARM_RX24_MAX_VALUE);
        RX24_config_set_minimal_angle(SMALL_CYLINDER_ARM_RX24_ID, SMALL_CYLINDER_ARM_RX24_MIN_VALUE);
        RX24_set_move_to_position_speed(SMALL_CYLINDER_ARM_RX24_ID, SMALL_CYLINDER_ARM_RX24_SPEED);

        RX24_config_set_error_before_led(SMALL_CYLINDER_ARM_RX24_ID, RX24_BEFORE_LED);
        RX24_config_set_error_before_shutdown(SMALL_CYLINDER_ARM_RX24_ID, RX24_BEFORE_SHUTDOWN);
...
void SMALL_CYLINDER_ARM_set_config(CAN_msg_t* msg){
	switch(msg->data.act_msg.act_data.act_config.sub_act_id){
		case DEFAULT_MONO_ACT : // Premier élement de l'actionneur
            ACTMGR_config_RX24(SMALL_CYLINDER_ARM_RX24_ID, msg);
			break;
...
static void SMALL_CYLINDER_ARM_get_config(CAN_msg_t *incoming_msg){
	...
	msg.data.act_get_config_answer.sid = 0xFF & ACT_SMALL_CYLINDER_ARM;
	...
	switch(incoming_msg->data.act_msg.act_data.config){
		case POSITION_CONFIG:
			SMALL_CYLINDER_ARM_get_position_config(&order, &pos);
			...
			break;
		case TORQUE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.torque = RX24_get_speed_percentage(SMALL_CYLINDER_ARM_RX24_ID);
			break;
		case TEMPERATURE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.temperature = RX24_get_temperature(SMALL_CYLINDER_ARM_RX24_ID);
			break;
		case LOAD_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.load = RX24_get_load_percentage(SMALL_CYLINDER_ARM_RX24_ID);
			break;
		default:
			error = TRUE;
...
static void SMALL_CYLINDER_ARM_get_position_config(ACT_order_e *pOrder, Uint16 *pPos){
	ACT_order_e order = ACT_SMALL_CYLINDER_ARM_STOP;
	Uint16 position = RX24_get_position(SMALL_CYLINDER_ARM_RX24_ID);
	Uint16 epsilon = SMALL_CYLINDER_ARM_RX24_ASSER_POS_EPSILON;

	if(position > SMALL_CYLINDER_ARM_RX24_IN_POS - epsilon && position < SMALL_CYLINDER_ARM_RX24_IN_POS + epsilon){
		order = ACT_SMALL_CYLINDER_ARM_IN;
	}else if(position > SMALL_CYLINDER_ARM_RX24_PREPARE_TO_TAKE_POS - epsilon && position < SMALL_CYLINDER_ARM_RX24_PREPARE_TO_TAKE_POS + epsilon){
		order = ACT_SMALL_CYLINDER_ARM_PREPARE_TO_TAKE;
	}else if(position > SMALL_CYLINDER_ARM_RX24_TAKE_POS - epsilon && position < SMALL_CYLINDER_ARM_RX24_TAKE_POS + epsilon){
		order = ACT_SMALL_CYLINDER_ARM_TAKE;
	}else if(position > SMALL_CYLINDER_ARM_RX24_DISPOSE_POS - epsilon && position < SMALL_CYLINDER_ARM_RX24_DISPOSE_POS + epsilon){
	...
	}else if(position > SMALL_CYLINDER_ARM_RX24_PROTECT_WHEN_DISPOSE - epsilon && position < SMALL_CYLINDER_ARM_RX24_PROTECT_WHEN_DISPOSE + epsilon){
		order = ACT_SMALL_CYLINDER_ARM_PROTECT_WHEN_DISPOSE;
...
void SMALL_CYLINDER_ARM_init_pos(){
    ...
    if(!RX24_set_position(SMALL_CYLINDER_ARM_RX24_ID, SMALL_CYLINDER_ARM_RX24_INIT_POS))
        debug_printf("   Le RX24 n°%d n'est pas là\n", SMALL_CYLINDER_ARM_RX24_ID);
	else
...
bool_e SMALL_CYLINDER_ARM_CAN_process_msg(CAN_msg_t* msg) {
    if(msg->sid == ACT_SMALL_CYLINDER_ARM){
        SMALL_CYLINDER_ARM_initRX24();
		switch(msg->data.act_msg.order) {
			// Listing de toutes les positions de l'actionneur possible
            case ACT_SMALL_CYLINDER_ARM_IDLE :
            case ACT_SMALL_CYLINDER_ARM_IN :
            case ACT_SMALL_CYLINDER_ARM_PREPARE_TO_TAKE :
            ...
            case ACT_SMALL_CYLINDER_ARM_PROTECT_WHEN_DISPOSE:
            ...
            case ACT_WARNER:
				SMALL_CYLINDER_ARM_set_warner(msg);
				break;

			case ACT_GET_CONFIG:
				SMALL_CYLINDER_ARM_get_config(msg);
				break;

			case ACT_SET_CONFIG :
                SMALL_CYLINDER_ARM_set_config(msg);
				break;
...
void SMALL_CYLINDER_ARM_run_command(queue_id_t queueId, bool_e init) {
    ...
    if(QUEUE_get_act(queueId) == QUEUE_ACT_RX24_SMALL_CYLINDER_ARM) {    // Gestion des mouvements de SMALL_CYLINDER_ARM
		if(init)
            SMALL_CYLINDER_ARM_command_init(queueId);
		else
            SMALL_CYLINDER_ARM_command_run(queueId);
	}
}
...
static void SMALL_CYLINDER_ARM_command_init(queue_id_t queueId) {
	...
	*rx24_goalPosition = 0xFFFF;
	...
	switch(command) {
        ...
        case ACT_SMALL_CYLINDER_ARM_IDLE : *rx24_goalPosition = SMALL_CYLINDER_ARM_RX24_IDLE_POS; break;
        case ACT_SMALL_CYLINDER_ARM_IN : *rx24_goalPosition = SMALL_CYLINDER_ARM_RX24_IN_POS; break;
        case ACT_SMALL_CYLINDER_ARM_PREPARE_TO_TAKE : *rx24_goalPosition = SMALL_CYLINDER_ARM_RX24_PREPARE_TO_TAKE_POS; break;
        ...
        case ACT_SMALL_CYLINDER_ARM_PROTECT_WHEN_DISPOSE : *rx24_goalPosition = SMALL_CYLINDER_ARM_RX24_PROTECT_WHEN_DISPOSE; break;

        case ACT_SMALL_CYLINDER_ARM_STOP :
            RX24_set_torque_enabled(SMALL_CYLINDER_ARM_RX24_ID, FALSE); //Stopper l'asservissement du RX24
            QUEUE_next(queueId, ACT_SMALL_CYLINDER_ARM, ACT_RESULT_DONE, ACT_RESULT_ERROR_OK, __LINE__);
			return;

		default: {
            error_printf("Invalid SMALL_CYLINDER_ARM command: %u, code is broken !\n", command);
            QUEUE_next(queueId, ACT_SMALL_CYLINDER_ARM, ACT_RESULT_NOT_HANDLED, ACT_RESULT_ERROR_LOGIC, __LINE__);
			return;
		}
	}
	...
	if(rx24_is_initialized == FALSE){
        ...
        QUEUE_next(queueId, ACT_SMALL_CYLINDER_ARM, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
    ...
    RX24_reset_last_error(SMALL_CYLINDER_ARM_RX24_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
    if(!RX24_set_position(SMALL_CYLINDER_ARM_RX24_ID, *rx24_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que le RX24 ne répond pas a cet envoi, on l'indique à la carte stratégie
        error_printf("RX24_set_position error: 0x%x\n", RX24_get_last_error(SMALL_CYLINDER_ARM_RX24_ID).error);
        QUEUE_next(queueId, ACT_SMALL_CYLINDER_ARM, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	//La commande a été envoyée et le RX24 l'a bien reçu
    debug_printf("Move SMALL_CYLINDER_ARM rx24 to %d\n", *rx24_goalPosition);
}
...
static void SMALL_CYLINDER_ARM_command_run(queue_id_t queueId) {
	...
	Uint16 pos = RX24_get_position(SMALL_CYLINDER_ARM_RX24_ID);

	if(!run_now){
		if(ACTQ_check_status_rx24(queueId, SMALL_CYLINDER_ARM_RX24_ID, QUEUE_get_arg(queueId)->param, pos, SMALL_CYLINDER_ARM_RX24_ASSER_POS_EPSILON, SMALL_CYLINDER_ARM_RX24_ASSER_TIMEOUT, SMALL_CYLINDER_ARM_RX24_ASSER_POS_LARGE_EPSILON, &result, &errorCode, &line))
			QUEUE_next(queueId, ACT_SMALL_CYLINDER_ARM, result, errorCode, line);
	}else{
		// Lorsqu'on vient de recevoir un run_now, on passe directement à l'ordre reçu.
		ACTQ_flush_queue_to_run_now(queueId, ACT_SMALL_CYLINDER_ARM);
		run_now = FALSE;
	}

    // On ne surveille le warner que si il est activé
	if(warner.activated)
		SMALL_CYLINDER_ARM_check_warner(pos);
}

static void SMALL_CYLINDER_ARM_set_warner(CAN_msg_t *msg){
	warner.activated = TRUE;
	warner.last_pos = RX24_get_position(SMALL_CYLINDER_ARM_RX24_ID);

	switch(msg->data.act_msg.act_data.warner_pos){
		/*case ACT_SMALL_CYLINDER_ARM_WARNER:
			warner.warner_pos = SMALL_CYLINDER_ARM_RX24_WARNER_POS;
			break;*/
		default:{
...
static void SMALL_CYLINDER_ARM_check_warner(Uint16 pos){
	...
	if( (warner.last_pos < pos && warner.last_pos < warner.warner_pos && pos > warner.warner_pos)
	 ...
	 || (warner.last_pos < RX24_MIN_WARNER && pos > RX24_MAX_WARNER	&& (warner.last_pos > warner.warner_pos || pos < warner.warner_pos))
		...
		msg.data.act_warner_answer.sid = 0xFF & ACT_SMALL_CYLINDER_ARM;
		CAN_send(&msg);
