...
// 6) Un #define SMALL_CYLINDER_COLOR_RX24_ID doit avoir été ajouté au fichier config_big/config_pin.h ou config_small/config_pin.h
...
static void SMALL_CYLINDER_COLOR_command_init(queue_id_t queueId) {
	...
	*rx24_goalPosition = 0xFFFF;
	...
	RX24_reset_last_error(SMALL_CYLINDER_COLOR_RX24_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
	if(!RX24_set_speed_percentage(SMALL_CYLINDER_COLOR_RX24_ID, *rx24_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que le RX24 ne répond pas a cet envoi, on l'indique à la carte stratégie
...
static void SMALL_CYLINDER_COLOR_command_run(queue_id_t queueId) {
	...
	if(!run_now){
		...
		ACTQ_flush_queue_to_run_now(queueId, ACT_SMALL_CYLINDER_COLOR);
		run_now = FALSE;
	}
