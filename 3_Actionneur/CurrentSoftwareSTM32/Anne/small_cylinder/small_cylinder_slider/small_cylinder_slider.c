 ...
 *
 *	Fichier : SMALL_CYLINDER_SLIDER_ax12.c
...
// SMALL_CYLINDER_SLIDER d'un actionneur standart avec AX12
...
//		Formatage : QUEUE_ACT_AX12_SMALL_CYLINDER_SLIDER
...
// 6) Un #define SMALL_CYLINDER_SLIDER_AX12_ID doit avoir été ajouté au fichier config_big/config_pin.h ou config_small/config_pin.h
...
static void SMALL_CYLINDER_SLIDER_initAX12();
...
void SMALL_CYLINDER_SLIDER_init() {
	...
	AX12_init();
	SMALL_CYLINDER_SLIDER_initAX12();
}
...
static void SMALL_CYLINDER_SLIDER_initAX12() {
	if(ax12_is_initialized == FALSE && AX12_is_ready(SMALL_CYLINDER_SLIDER_AX12_ID) == TRUE) {
		ax12_is_initialized = TRUE;
		AX12_config_set_lowest_voltage(SMALL_CYLINDER_SLIDER_AX12_ID, AX12_MIN_VOLTAGE);
		AX12_config_set_highest_voltage(SMALL_CYLINDER_SLIDER_AX12_ID, AX12_MAX_VOLTAGE);
		AX12_set_torque_limit(SMALL_CYLINDER_SLIDER_AX12_ID, SMALL_CYLINDER_SLIDER_AX12_MAX_TORQUE_PERCENT);
		AX12_config_set_temperature_limit(SMALL_CYLINDER_SLIDER_AX12_ID, SMALL_CYLINDER_SLIDER_AX12_MAX_TEMPERATURE);

		AX12_config_set_maximal_angle(SMALL_CYLINDER_SLIDER_AX12_ID, SMALL_CYLINDER_SLIDER_AX12_MAX_VALUE);
		AX12_config_set_minimal_angle(SMALL_CYLINDER_SLIDER_AX12_ID, SMALL_CYLINDER_SLIDER_AX12_MIN_VALUE);
		AX12_set_move_to_position_speed(SMALL_CYLINDER_SLIDER_AX12_ID, SMALL_CYLINDER_SLIDER_AX12_SPEED);

		AX12_config_set_error_before_led(SMALL_CYLINDER_SLIDER_AX12_ID, AX12_BEFORE_LED);
		AX12_config_set_error_before_shutdown(SMALL_CYLINDER_SLIDER_AX12_ID, AX12_BEFORE_SHUTDOWN);
...
void SMALL_CYLINDER_SLIDER_set_config(CAN_msg_t* msg){
	switch(msg->data.act_msg.act_data.act_config.sub_act_id){
		case DEFAULT_MONO_ACT : // Premier élement de l'actionneur
			ACTMGR_config_AX12(SMALL_CYLINDER_SLIDER_AX12_ID, msg);
			break;
...
static void SMALL_CYLINDER_SLIDER_get_config(CAN_msg_t *incoming_msg){
	...
	switch(incoming_msg->data.act_msg.act_data.config){
		case POSITION_CONFIG:
			...
			break;
		case TORQUE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.torque = AX12_get_speed_percentage(SMALL_CYLINDER_SLIDER_AX12_ID);
			break;
		case TEMPERATURE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.temperature = AX12_get_temperature(SMALL_CYLINDER_SLIDER_AX12_ID);
			break;
		case LOAD_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.load = AX12_get_load_percentage(SMALL_CYLINDER_SLIDER_AX12_ID);
			break;
		default:
			error = TRUE;
...
void SMALL_CYLINDER_SLIDER_init_pos(){
	...
	if(!AX12_set_position(SMALL_CYLINDER_SLIDER_AX12_ID, SMALL_CYLINDER_SLIDER_AX12_INIT_POS))
		debug_printf("   L'AX12 n°%d n'est pas là\n", SMALL_CYLINDER_SLIDER_AX12_ID);
	else
...
void SMALL_CYLINDER_SLIDER_run_command(queue_id_t queueId, bool_e init) {
	...
	if(QUEUE_get_act(queueId) == QUEUE_ACT_AX12_SMALL_CYLINDER_SLIDER) {    // Gestion des mouvements de SMALL_CYLINDER_SLIDER
		if(init)
...
static void SMALL_CYLINDER_SLIDER_command_init(queue_id_t queueId) {
	...
	*ax12_goalPosition = 0xFFFF;
	...
	switch(command) {
		...
		case ACT_SMALL_CYLINDER_SLIDER_ALMOST_OUT : *ax12_goalPosition = SMALL_CYLINDER_SLIDER_AX12_ALMOST_OUT_POS; break;
		case ACT_SMALL_CYLINDER_SLIDER_ALMOST_OUT_WITH_CYLINDER : *ax12_goalPosition = SMALL_CYLINDER_SLIDER_AX12_ALMOST_OUT_WITH_CYLINDER_POS; break;
	...
	AX12_reset_last_error(SMALL_CYLINDER_SLIDER_AX12_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
	if(!AX12_set_position(SMALL_CYLINDER_SLIDER_AX12_ID, *ax12_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que l'AX12 ne répond pas a cet envoi, on l'indique à la carte stratégie
		error_printf("AX12_set_position error: 0x%x\n", AX12_get_last_error(SMALL_CYLINDER_SLIDER_AX12_ID).error);
		QUEUE_next(queueId, ACT_SMALL_CYLINDER_SLIDER, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	//La commande a été envoyée et l'AX12 l'a bien reçu
	debug_printf("Move SMALL_CYLINDER_SLIDER ax12 to %d\n", *ax12_goalPosition);
}
...
static void SMALL_CYLINDER_SLIDER_command_run(queue_id_t queueId) {
	...
	Uint16 pos = AX12_get_position(SMALL_CYLINDER_SLIDER_AX12_ID);

	if(!run_now){
		if(ACTQ_check_status_ax12(queueId, SMALL_CYLINDER_SLIDER_AX12_ID, QUEUE_get_arg(queueId)->param, pos, SMALL_CYLINDER_SLIDER_AX12_ASSER_POS_EPSILON, SMALL_CYLINDER_SLIDER_AX12_ASSER_TIMEOUT, SMALL_CYLINDER_SLIDER_AX12_ASSER_POS_LARGE_EPSILON, &result, &errorCode, &line))
		...
		ACTQ_flush_queue_to_run_now(queueId, ACT_SMALL_CYLINDER_SLIDER);
		run_now = FALSE;
	}
...
static void SMALL_CYLINDER_SLIDER_set_warner(CAN_msg_t *msg){
	...
	warner.last_pos = AX12_get_position(SMALL_CYLINDER_SLIDER_AX12_ID);

	switch(msg->data.act_msg.act_data.warner_pos){
		/*case ACT_SMALL_CYLINDER_SLIDER_WARNER:
			warner.warner_pos = SMALL_CYLINDER_SLIDER_AX12_WARNER_POS;
			break;*/
		default:{
