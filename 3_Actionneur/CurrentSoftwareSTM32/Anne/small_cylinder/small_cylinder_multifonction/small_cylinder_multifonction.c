...
// 6) Un #define SMALL_CYLINDER_MULTIFONCTION_AX12_ID doit avoir été ajouté au fichier config_big/config_pin.h ou config_small/config_pin.h
...
static void SMALL_CYLINDER_MULTIFONCTION_get_position_config(ACT_order_e *pOrder, Uint16 *pPos){
	...
	}else if(position > SMALL_CYLINDER_MULTIFONCTION_AX12_UNLOCK_POS - epsilon && position < SMALL_CYLINDER_MULTIFONCTION_AX12_UNLOCK_POS + epsilon){
		order = ACT_SMALL_CYLINDER_MULTIFONCTION_UNLOCK;
...
bool_e SMALL_CYLINDER_MULTIFONCTION_CAN_process_msg(CAN_msg_t* msg) {
	if(msg->sid == ACT_SMALL_CYLINDER_MULTIFONCTION){
		...
		switch(msg->data.act_msg.order) {
			...
			case ACT_SMALL_CYLINDER_MULTIFONCTION_UNLOCK :
...
static void SMALL_CYLINDER_MULTIFONCTION_command_init(queue_id_t queueId) {
	...
	*ax12_goalPosition = 0xFFFF;
	...
	switch(command) {
		...
		case ACT_SMALL_CYLINDER_MULTIFONCTION_UNLOCK : *ax12_goalPosition = SMALL_CYLINDER_MULTIFONCTION_AX12_UNLOCK_POS; break;
	...
	AX12_reset_last_error(SMALL_CYLINDER_MULTIFONCTION_AX12_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
	if(!AX12_set_position(SMALL_CYLINDER_MULTIFONCTION_AX12_ID, *ax12_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que l'AX12 ne répond pas a cet envoi, on l'indique à la carte stratégie
