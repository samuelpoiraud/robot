...
#include "small_cylinder_poussix.h"
...
#include "small_cylinder_poussix_config.h"

// Les différents define pour le verbose sur uart
#define LOG_PREFIX "small_cylinder_poussix.c : "
#define LOG_COMPONENT OUTPUT_LOG_COMPONENT_SMALL_CYLINDER_POUSSIX
#include "../QS/QS_outputlog.h"

// Les fonctions internes au fonctionnement de l'actionneur
static void SMALL_CYLINDER_POUSSIX_command_run(queue_id_t queueId);
static void SMALL_CYLINDER_POUSSIX_initAX12();
static void SMALL_CYLINDER_POUSSIX_command_init(queue_id_t queueId);
static void SMALL_CYLINDER_POUSSIX_set_config(CAN_msg_t* msg);
static void SMALL_CYLINDER_POUSSIX_get_config(CAN_msg_t *incoming_msg);
static void SMALL_CYLINDER_POUSSIX_get_position_config(ACT_order_e *pOrder, Uint16 *pPos);
static void SMALL_CYLINDER_POUSSIX_set_warner(CAN_msg_t *msg);
static void SMALL_CYLINDER_POUSSIX_check_warner(Uint16 pos);
...
void SMALL_CYLINDER_POUSSIX_init() {
	...
	AX12_init();
	SMALL_CYLINDER_POUSSIX_initAX12();
}
...
static void SMALL_CYLINDER_POUSSIX_initAX12() {
	if(ax12_is_initialized == FALSE && AX12_is_ready(SMALL_CYLINDER_POUSSIX_AX12_ID) == TRUE) {
		ax12_is_initialized = TRUE;
		AX12_config_set_lowest_voltage(SMALL_CYLINDER_POUSSIX_AX12_ID, AX12_MIN_VOLTAGE);
		AX12_config_set_highest_voltage(SMALL_CYLINDER_POUSSIX_AX12_ID, AX12_MAX_VOLTAGE);
		AX12_set_torque_limit(SMALL_CYLINDER_POUSSIX_AX12_ID, SMALL_CYLINDER_POUSSIX_AX12_MAX_TORQUE_PERCENT);
		AX12_config_set_temperature_limit(SMALL_CYLINDER_POUSSIX_AX12_ID, SMALL_CYLINDER_POUSSIX_AX12_MAX_TEMPERATURE);

		AX12_config_set_maximal_angle(SMALL_CYLINDER_POUSSIX_AX12_ID, SMALL_CYLINDER_POUSSIX_AX12_MAX_VALUE);
		AX12_config_set_minimal_angle(SMALL_CYLINDER_POUSSIX_AX12_ID, SMALL_CYLINDER_POUSSIX_AX12_MIN_VALUE);
		AX12_set_move_to_position_speed(SMALL_CYLINDER_POUSSIX_AX12_ID, SMALL_CYLINDER_POUSSIX_AX12_SPEED);

		AX12_config_set_error_before_led(SMALL_CYLINDER_POUSSIX_AX12_ID, AX12_BEFORE_LED);
		AX12_config_set_error_before_shutdown(SMALL_CYLINDER_POUSSIX_AX12_ID, AX12_BEFORE_SHUTDOWN);
...
void SMALL_CYLINDER_POUSSIX_set_config(CAN_msg_t* msg){
	switch(msg->data.act_msg.act_data.act_config.sub_act_id){
		case DEFAULT_MONO_ACT : // Premier élement de l'actionneur
			ACTMGR_config_AX12(SMALL_CYLINDER_POUSSIX_AX12_ID, msg);
			break;
...
static void SMALL_CYLINDER_POUSSIX_get_config(CAN_msg_t *incoming_msg){
	...
	msg.data.act_get_config_answer.sid = 0xFF & ACT_SMALL_CYLINDER_POUSSIX;
	...
	switch(incoming_msg->data.act_msg.act_data.config){
		case POSITION_CONFIG:
			SMALL_CYLINDER_POUSSIX_get_position_config(&order, &pos);
			...
			break;
		case TORQUE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.torque = AX12_get_speed_percentage(SMALL_CYLINDER_POUSSIX_AX12_ID);
			break;
		case TEMPERATURE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.temperature = AX12_get_temperature(SMALL_CYLINDER_POUSSIX_AX12_ID);
			break;
		case LOAD_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.load = AX12_get_load_percentage(SMALL_CYLINDER_POUSSIX_AX12_ID);
			break;
		default:
			error = TRUE;
...
static void SMALL_CYLINDER_POUSSIX_get_position_config(ACT_order_e *pOrder, Uint16 *pPos){
	ACT_order_e order = ACT_SMALL_CYLINDER_POUSSIX_STOP;
	Uint16 position = AX12_get_position(SMALL_CYLINDER_POUSSIX_AX12_ID);
	Uint16 epsilon = SMALL_CYLINDER_POUSSIX_AX12_ASSER_POS_EPSILON;

	if(position > SMALL_CYLINDER_POUSSIX_AX12_DOWN_POS - epsilon && position < SMALL_CYLINDER_POUSSIX_AX12_DOWN_POS + epsilon){
...
void SMALL_CYLINDER_POUSSIX_init_pos(){
	...
	if(!AX12_set_position(SMALL_CYLINDER_POUSSIX_AX12_ID, SMALL_CYLINDER_POUSSIX_AX12_INIT_POS))
		debug_printf("   L'AX12 n°%d n'est pas là\n", SMALL_CYLINDER_POUSSIX_AX12_ID);
	else
...
void SMALL_CYLINDER_POUSSIX_stop(){
	AX12_set_torque_enabled(SMALL_CYLINDER_POUSSIX_AX12_ID, FALSE); //Stopper l'asservissement de l'AX12
}
...
bool_e SMALL_CYLINDER_POUSSIX_CAN_process_msg(CAN_msg_t* msg) {
	if(msg->sid == ACT_SMALL_CYLINDER_POUSSIX){
		SMALL_CYLINDER_POUSSIX_initAX12();
		switch(msg->data.act_msg.order) {
			...
			case ACT_WARNER:
				SMALL_CYLINDER_POUSSIX_set_warner(msg);
				break;

			case ACT_GET_CONFIG:
				SMALL_CYLINDER_POUSSIX_get_config(msg);
				break;

			case ACT_SET_CONFIG :
				SMALL_CYLINDER_POUSSIX_set_config(msg);
				break;
...
void SMALL_CYLINDER_POUSSIX_run_command(queue_id_t queueId, bool_e init) {
	...
	if(QUEUE_get_act(queueId) == QUEUE_ACT_AX12_SMALL_CYLINDER_POUSSIX) {    // Gestion des mouvements de SMALL_CYLINDER_POUSSIX
		if(init)
			SMALL_CYLINDER_POUSSIX_command_init(queueId);
		else
			SMALL_CYLINDER_POUSSIX_command_run(queueId);
	}
}
...
static void SMALL_CYLINDER_POUSSIX_command_init(queue_id_t queueId) {
	...
	*ax12_goalPosition = 0xFFFF;
	...
	switch(command) {
		...
		case ACT_SMALL_CYLINDER_POUSSIX_STOP :
			AX12_set_torque_enabled(SMALL_CYLINDER_POUSSIX_AX12_ID, FALSE); //Stopper l'asservissement de l'AX12
			QUEUE_next(queueId, ACT_SMALL_CYLINDER_POUSSIX, ACT_RESULT_DONE, ACT_RESULT_ERROR_OK, __LINE__);
			return;

		default: {
			error_printf("Invalid SMALL_CYLINDER_POUSSIX command: %u, code is broken !\n", command);
			QUEUE_next(queueId, ACT_SMALL_CYLINDER_POUSSIX, ACT_RESULT_NOT_HANDLED, ACT_RESULT_ERROR_LOGIC, __LINE__);
			return;
		}
	}
	...
	if(ax12_is_initialized == FALSE){
		...
		QUEUE_next(queueId, ACT_SMALL_CYLINDER_POUSSIX, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	...
	AX12_reset_last_error(SMALL_CYLINDER_POUSSIX_AX12_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
	if(!AX12_set_position(SMALL_CYLINDER_POUSSIX_AX12_ID, *ax12_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que l'AX12 ne répond pas a cet envoi, on l'indique à la carte stratégie
		error_printf("AX12_set_position error: 0x%x\n", AX12_get_last_error(SMALL_CYLINDER_POUSSIX_AX12_ID).error);
		QUEUE_next(queueId, ACT_SMALL_CYLINDER_POUSSIX, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	//La commande a été envoyée et l'AX12 l'a bien reçu
	debug_printf("Move SMALL_CYLINDER_POUSSIX ax12 to %d\n", *ax12_goalPosition);
}
...
static void SMALL_CYLINDER_POUSSIX_command_run(queue_id_t queueId) {
	...
	Uint16 pos = AX12_get_position(SMALL_CYLINDER_POUSSIX_AX12_ID);

	if(ACTQ_check_status_ax12(queueId, SMALL_CYLINDER_POUSSIX_AX12_ID, QUEUE_get_arg(queueId)->param, pos, SMALL_CYLINDER_POUSSIX_AX12_ASSER_POS_EPSILON, SMALL_CYLINDER_POUSSIX_AX12_ASSER_TIMEOUT, SMALL_CYLINDER_POUSSIX_AX12_ASSER_POS_LARGE_EPSILON, &result, &errorCode, &line))
		QUEUE_next(queueId, ACT_SMALL_CYLINDER_POUSSIX, result, errorCode, line);

    // On ne surveille le warner que si il est activé
	if(warner.activated)
		SMALL_CYLINDER_POUSSIX_check_warner(pos);
}

// Fonction permettant d'activer un warner sur une position de l'ax12
static void SMALL_CYLINDER_POUSSIX_set_warner(CAN_msg_t *msg){
	warner.activated = TRUE;
	warner.last_pos = AX12_get_position(SMALL_CYLINDER_POUSSIX_AX12_ID);

	switch(msg->data.act_msg.act_data.warner_pos){
		/*case ACT_SMALL_CYLINDER_POUSSIX_WARNER:
			warner.warner_pos = SMALL_CYLINDER_POUSSIX_AX12_WARNER_POS;
			break;*/
		default:{
...
static void SMALL_CYLINDER_POUSSIX_check_warner(Uint16 pos){
	...
	if( (warner.last_pos < pos && warner.last_pos < warner.warner_pos && pos > warner.warner_pos)
	 ...
	 || (warner.last_pos < AX12_MIN_WARNER && pos > AX12_MAX_WARNER	&& (warner.last_pos > warner.warner_pos || pos < warner.warner_pos))
		...
		msg.data.act_warner_answer.sid = 0xFF & ACT_SMALL_CYLINDER_POUSSIX;
		CAN_send(&msg);
