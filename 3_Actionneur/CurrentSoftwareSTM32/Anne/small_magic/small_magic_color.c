...
#include "small_magic_color.h"


// SMALL_MAGIC_COLOR d'un actionneur standart avec RX24
...
//		Formatage : QUEUE_ACT_RX24_SMALL_MAGIC_COLOR
...
// 5) Ajout du pilotage via terminal dans le fichier term_io.c dans le tableau terminal_motor du type : ACT_DECLARE(SMALL_MAGIC_COLOR)
// 6) Un #define SMALL_MAGIC_COLOR_RX24_ID doit avoir été ajouté au fichier config_big/config_pin.h ou config_small/config_pin.h
...
static void SMALL_MAGIC_COLOR_command_run(queue_id_t queueId);
...
void SMALL_MAGIC_COLOR_reset_config(){
...
void SMALL_MAGIC_COLOR_set_config(CAN_msg_t* msg){
	switch(msg->data.act_msg.act_data.act_config.sub_act_id){
		case DEFAULT_MONO_ACT : // Premier élement de l'actionneur
			ACTMGR_config_RX24(SMALL_MAGIC_COLOR_RX24_ID, msg);
			break;
...
static void SMALL_MAGIC_COLOR_get_config(CAN_msg_t *incoming_msg){
	...
	msg.data.act_get_config_answer.sid = 0xFF & ACT_SMALL_MAGIC_COLOR;
	...
	switch(incoming_msg->data.act_msg.act_data.config){
		...
		case TEMPERATURE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.temperature = RX24_get_temperature(SMALL_MAGIC_COLOR_RX24_ID);
			break;
		case LOAD_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.load = RX24_get_load_percentage(SMALL_MAGIC_COLOR_RX24_ID);
			break;
		default:
			error = TRUE;
...
/*static void SMALL_MAGIC_COLOR_get_position_config(ACT_order_e *pOrder, Uint16 *pPos){
	...
	if(position > SMALL_MAGIC_COLOR_RX24_LOCK_POS - epsilon && position < SMALL_MAGIC_COLOR_RX24_LOCK_POS + epsilon){
		order = ACT_SMALL_MAGIC_COLOR_LOCK;
	}else if(position > SMALL_MAGIC_COLOR_RX24_UNLOCK_POS - epsilon && position < SMALL_MAGIC_COLOR_RX24_UNLOCK_POS + epsilon){
		order = ACT_SMALL_MAGIC_COLOR_UNLOCK;
	}else if(position > SMALL_MAGIC_COLOR_RX24_IDLE_POS - epsilon && position < SMALL_MAGIC_COLOR_RX24_IDLE_POS + epsilon){
		order = ACT_SMALL_MAGIC_COLOR_IDLE;
	}
...
bool_e SMALL_MAGIC_COLOR_CAN_process_msg(CAN_msg_t* msg) {
	if(msg->sid == ACT_SMALL_MAGIC_COLOR){
		SMALL_MAGIC_COLOR_initRX24();
		switch(msg->data.act_msg.order) {
			...
			case ACT_WARNER:
				SMALL_MAGIC_COLOR_set_warner(msg);
				break;

			case ACT_GET_CONFIG:
				SMALL_MAGIC_COLOR_get_config(msg);
				break;

			case ACT_SET_CONFIG :
				SMALL_MAGIC_COLOR_set_config(msg);
				break;
...
void SMALL_MAGIC_COLOR_run_command(queue_id_t queueId, bool_e init) {
	if(QUEUE_has_error(queueId)) {
		QUEUE_behead(queueId);
		return;
	}

	if(QUEUE_get_act(queueId) == QUEUE_ACT_RX24_SMALL_MAGIC_COLOR) {    // Gestion des mouvements de SMALL_MAGIC_COLOR
		if(init)
			SMALL_MAGIC_COLOR_command_init(queueId);
		else
			SMALL_MAGIC_COLOR_command_run(queueId);
	}
}
...
static void SMALL_MAGIC_COLOR_command_init(queue_id_t queueId) {
	...
	*rx24_goalPosition = 0xFFFF;
	...
	switch(command) {
		...
		case ACT_SMALL_MAGIC_COLOR_STOP :
			RX24_set_torque_enabled(SMALL_MAGIC_COLOR_RX24_ID, FALSE); //Stopper l'asservissement du RX24
			QUEUE_next(queueId, ACT_SMALL_MAGIC_COLOR, ACT_RESULT_DONE, ACT_RESULT_ERROR_OK, __LINE__);
			return;

		default: {
			error_printf("Invalid SMALL_MAGIC_COLOR command: %u, code is broken !\n", command);
			QUEUE_next(queueId, ACT_SMALL_MAGIC_COLOR, ACT_RESULT_NOT_HANDLED, ACT_RESULT_ERROR_LOGIC, __LINE__);
			return;
		}
	}
...
static void SMALL_MAGIC_COLOR_set_warner(CAN_msg_t *msg){
	...
	warner.last_pos = RX24_get_position(SMALL_MAGIC_COLOR_RX24_ID);

	switch(msg->data.act_msg.act_data.warner_pos){
		/*case ACT_SMALL_MAGIC_COLOR_WARNER:
			warner.warner_pos = SMALL_MAGIC_COLOR_RX24_WARNER_POS;
			break;*/
		default:{
...
static void SMALL_MAGIC_COLOR_check_warner(Uint16 pos){
	...
	if( (warner.last_pos < pos && warner.last_pos < warner.warner_pos && pos > warner.warner_pos)
	 ...
	 || (warner.last_pos < RX24_MIN_WARNER && pos > RX24_MAX_WARNER	&& (warner.last_pos > warner.warner_pos || pos < warner.warner_pos))
		...
		msg.data.act_warner_answer.sid = 0xFF & ACT_SMALL_MAGIC_COLOR;
		CAN_send(&msg);
