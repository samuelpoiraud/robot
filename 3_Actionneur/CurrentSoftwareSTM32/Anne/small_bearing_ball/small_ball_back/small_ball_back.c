 ...
 *
 *	Fichier : SMALL_BALL_BACK_ax12.c
...
// 6) Un #define SMALL_BALL_BACK_AX12_ID doit avoir été ajouté au fichier config_big/config_pin.h ou config_small/config_pin.h
...
#include "small_ball_back_config.h"
...
static void SMALL_BALL_BACK_command_run(queue_id_t queueId);
static void SMALL_BALL_BACK_initAX12();
static void SMALL_BALL_BACK_command_init(queue_id_t queueId);
static void SMALL_BALL_BACK_set_config(CAN_msg_t* msg);
static void SMALL_BALL_BACK_get_position_config(ACT_order_e *pOrder, Uint16 *pPos);
static void SMALL_BALL_BACK_get_config(CAN_msg_t *incoming_msg);
static void SMALL_BALL_BACK_set_warner(CAN_msg_t *msg);
static void SMALL_BALL_BACK_check_warner(Uint16 pos);
...
static void SMALL_BALL_BACK_initAX12() {
	if(ax12_is_initialized == FALSE && AX12_is_ready(SMALL_BALL_BACK_AX12_ID) == TRUE) {
		ax12_is_initialized = TRUE;
		AX12_config_set_lowest_voltage(SMALL_BALL_BACK_AX12_ID, AX12_MIN_VOLTAGE);
		AX12_config_set_highest_voltage(SMALL_BALL_BACK_AX12_ID, AX12_MAX_VOLTAGE);
		AX12_set_torque_limit(SMALL_BALL_BACK_AX12_ID, SMALL_BALL_BACK_AX12_MAX_TORQUE_PERCENT);
		AX12_config_set_temperature_limit(SMALL_BALL_BACK_AX12_ID, SMALL_BALL_BACK_AX12_MAX_TEMPERATURE);

		AX12_config_set_maximal_angle(SMALL_BALL_BACK_AX12_ID, SMALL_BALL_BACK_AX12_MAX_VALUE);
		AX12_config_set_minimal_angle(SMALL_BALL_BACK_AX12_ID, SMALL_BALL_BACK_AX12_MIN_VALUE);
		AX12_set_move_to_position_speed(SMALL_BALL_BACK_AX12_ID, SMALL_BALL_BACK_AX12_SPEED);

		AX12_config_set_error_before_led(SMALL_BALL_BACK_AX12_ID, AX12_BEFORE_LED);
		AX12_config_set_error_before_shutdown(SMALL_BALL_BACK_AX12_ID, AX12_BEFORE_SHUTDOWN);
...
void SMALL_BALL_BACK_set_config(CAN_msg_t* msg){
	switch(msg->data.act_msg.act_data.act_config.sub_act_id){
		case DEFAULT_MONO_ACT : // Premier élement de l'actionneur
			ACTMGR_config_AX12(SMALL_BALL_BACK_AX12_ID, msg);
			break;
...
static void SMALL_BALL_BACK_get_config(CAN_msg_t *incoming_msg){
	...
	msg.data.act_get_config_answer.sid = 0xFF & ACT_SMALL_BALL_BACK;
	...
	switch(incoming_msg->data.act_msg.act_data.config){
		case POSITION_CONFIG:
			SMALL_BALL_BACK_get_position_config(&order, &pos);
			...
			break;
		case TORQUE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.torque = AX12_get_speed_percentage(SMALL_BALL_BACK_AX12_ID);
			break;
		case TEMPERATURE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.temperature = AX12_get_temperature(SMALL_BALL_BACK_AX12_ID);
			break;
		case LOAD_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.load = AX12_get_load_percentage(SMALL_BALL_BACK_AX12_ID);
			break;
		default:
			error = TRUE;
...
static void SMALL_BALL_BACK_get_position_config(ACT_order_e *pOrder, Uint16 *pPos){
	ACT_order_e order = ACT_SMALL_BALL_BACK_STOP;
	Uint16 position = AX12_get_position(SMALL_BALL_BACK_AX12_ID);
	Uint16 epsilon = SMALL_BALL_BACK_AX12_ASSER_POS_EPSILON;

	if(position > SMALL_BALL_BACK_AX12_UP_POS - epsilon && position < SMALL_BALL_BACK_AX12_UP_POS + epsilon){
		order = ACT_SMALL_BALL_BACK_UP;
	}else if(position > SMALL_BALL_BACK_AX12_DOWN_POS - epsilon && position < SMALL_BALL_BACK_AX12_DOWN_POS + epsilon){
		order = ACT_SMALL_BALL_BACK_DOWN;
	}else if(position > SMALL_BALL_BACK_AX12_IDLE_POS - epsilon && position < SMALL_BALL_BACK_AX12_IDLE_POS + epsilon){
		order = ACT_SMALL_BALL_BACK_IDLE;
	}
...
void SMALL_BALL_BACK_init_pos(){
	...
	if(!AX12_set_position(SMALL_BALL_BACK_AX12_ID, SMALL_BALL_BACK_AX12_INIT_POS))
		debug_printf("   L'AX12 n°%d n'est pas là\n", SMALL_BALL_BACK_AX12_ID);
	else
...
void SMALL_BALL_BACK_stop(){
	AX12_set_torque_enabled(SMALL_BALL_BACK_AX12_ID, FALSE); //Stopper l'asservissement de l'AX12
}

// fonction appellée à la réception d'un message CAN (via ActManager)
bool_e SMALL_BALL_BACK_CAN_process_msg(CAN_msg_t* msg) {
	if(msg->sid == ACT_SMALL_BALL_BACK){
		SMALL_BALL_BACK_initAX12();
		switch(msg->data.act_msg.order) {
			// Listing de toutes les positions de l'actionneur possible
			case ACT_SMALL_BALL_BACK_IDLE :
			case ACT_SMALL_BALL_BACK_DOWN :
			case ACT_SMALL_BALL_BACK_UP :
			case ACT_SMALL_BALL_BACK_STOP :
				run_now = msg->data.act_msg.act_data.act_order.run_now;
				ACTQ_push_operation_from_msg(msg, QUEUE_ACT_AX12_SMALL_BALL_BACK, &SMALL_BALL_BACK_run_command, 0,TRUE);
				break;

			case ACT_WARNER:
				SMALL_BALL_BACK_set_warner(msg);
				break;

			case ACT_GET_CONFIG:
				SMALL_BALL_BACK_get_config(msg);
				break;

			case ACT_SET_CONFIG :
				SMALL_BALL_BACK_set_config(msg);
				break;
...
void SMALL_BALL_BACK_run_command(queue_id_t queueId, bool_e init) {
	...
	if(QUEUE_get_act(queueId) == QUEUE_ACT_AX12_SMALL_BALL_BACK) {    // Gestion des mouvements de SMALL_BALL_BACK_LEFT
		if(init)
			SMALL_BALL_BACK_command_init(queueId);
		else
			SMALL_BALL_BACK_command_run(queueId);
	}
}
...
static void SMALL_BALL_BACK_command_init(queue_id_t queueId) {
	...
	*ax12_goalPosition = 0xFFFF;
	...
	switch(command) {
		...
		case ACT_SMALL_BALL_BACK_IDLE : *ax12_goalPosition = SMALL_BALL_BACK_AX12_IDLE_POS; break;
		case ACT_SMALL_BALL_BACK_DOWN : *ax12_goalPosition = SMALL_BALL_BACK_AX12_DOWN_POS; break;
		case ACT_SMALL_BALL_BACK_UP : *ax12_goalPosition = SMALL_BALL_BACK_AX12_UP_POS; break;

		case ACT_SMALL_BALL_BACK_STOP :
			AX12_set_torque_enabled(SMALL_BALL_BACK_AX12_ID, FALSE); //Stopper l'asservissement de l'AX12
			QUEUE_next(queueId, ACT_SMALL_BALL_BACK, ACT_RESULT_DONE, ACT_RESULT_ERROR_OK, __LINE__);
			return;

		default: {
			error_printf("Invalid SMALL_BALL_BACK command: %u, code is broken !\n", command);
			QUEUE_next(queueId, ACT_SMALL_BALL_BACK, ACT_RESULT_NOT_HANDLED, ACT_RESULT_ERROR_LOGIC, __LINE__);
			return;
		}
	}
	...
	if(ax12_is_initialized == FALSE){
		...
		QUEUE_next(queueId, ACT_SMALL_BALL_BACK, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	...
	AX12_reset_last_error(SMALL_BALL_BACK_AX12_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
	if(!AX12_set_position(SMALL_BALL_BACK_AX12_ID, *ax12_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que l'AX12 ne répond pas a cet envoi, on l'indique à la carte stratégie
		error_printf("AX12_set_position error: 0x%x\n", AX12_get_last_error(SMALL_BALL_BACK_AX12_ID).error);
		QUEUE_next(queueId, ACT_SMALL_BALL_BACK, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
...
static void SMALL_BALL_BACK_command_run(queue_id_t queueId) {
	...
	Uint16 pos = AX12_get_position(SMALL_BALL_BACK_AX12_ID);

	if(!run_now){
		if(ACTQ_check_status_ax12(queueId, SMALL_BALL_BACK_AX12_ID, QUEUE_get_arg(queueId)->param, pos, SMALL_BALL_BACK_AX12_ASSER_POS_EPSILON, SMALL_BALL_BACK_AX12_ASSER_TIMEOUT, SMALL_BALL_BACK_AX12_ASSER_POS_LARGE_EPSILON, &result, &errorCode, &line))
			QUEUE_next(queueId, ACT_SMALL_BALL_BACK, result, errorCode, line);
	}else{
	...
	if(warner.activated)
		SMALL_BALL_BACK_check_warner(pos);
}

static void SMALL_BALL_BACK_set_warner(CAN_msg_t *msg){
	warner.activated = TRUE;
	warner.last_pos = AX12_get_position(SMALL_BALL_BACK_AX12_ID);

	switch(msg->data.act_msg.act_data.warner_pos){
		/*case ACT_SMALL_BALL_BACK_WARNER:
			warner.warner_pos = SMALL_BALL_BACK_AX12_WARNER_POS;
			break;*/
		default:{
...
static void SMALL_BALL_BACK_check_warner(Uint16 pos){
	...
	if( (warner.last_pos < pos && warner.last_pos < warner.warner_pos && pos > warner.warner_pos)
	 ...
	 || (warner.last_pos < AX12_MIN_WARNER && pos > AX12_MAX_WARNER	&& (warner.last_pos > warner.warner_pos || pos < warner.warner_pos))
		...
		msg.data.act_warner_answer.sid = 0xFF & ACT_SMALL_BALL_BACK;
		CAN_send(&msg);
