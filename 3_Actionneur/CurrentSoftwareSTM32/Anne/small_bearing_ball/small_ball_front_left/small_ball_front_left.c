...
static ACT_order_e SMALL_BALL_FRONT_LEFT_get_position_config();
...
static void SMALL_BALL_FRONT_LEFT_check_warner();
...
static void SMALL_BALL_FRONT_LEFT_get_config(CAN_msg_t *incoming_msg){
	...
	switch(incoming_msg->data.act_msg.act_data.config){
		case POSITION_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.pos = SMALL_BALL_FRONT_LEFT_get_position_config();
			CAN_send(&msg); // Envoi du message CAN
			break;
		case SPEED_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.speed = AX12_get_speed_percentage(SMALL_BALL_FRONT_LEFT_AX12_ID);
			CAN_send(&msg); // Envoi du message CAN
			break;
		default:
...
static ACT_order_e SMALL_BALL_FRONT_LEFT_get_position_config(){
	ACT_order_e current_state = ACT_SMALL_BALL_FRONT_LEFT_STOP;
...
static void SMALL_BALL_FRONT_LEFT_command_run(queue_id_t queueId) {
	...
	if(warner.activated)
		SMALL_BALL_FRONT_LEFT_check_warner();
}
