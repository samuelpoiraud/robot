...
// 6) Un #define SMALL_BALL_FRONT_LEFT_AX12_ID doit avoir été ajouté au fichier config_big/config_pin.h ou config_small/config_pin.h
...
static void SMALL_BALL_FRONT_LEFT_set_config(CAN_msg_t* msg);
static void SMALL_BALL_FRONT_LEFT_get_position_config(ACT_order_e *pOrder, Uint16 *pPos);
static void SMALL_BALL_FRONT_LEFT_get_config(CAN_msg_t *incoming_msg);
static void SMALL_BALL_FRONT_LEFT_set_warner(CAN_msg_t *msg);
static void SMALL_BALL_FRONT_LEFT_check_warner(Uint16 pos);
...
static void SMALL_BALL_FRONT_LEFT_initAX12() {
	if(ax12_is_initialized == FALSE && AX12_is_ready(SMALL_BALL_FRONT_LEFT_AX12_ID) == TRUE) {
		...
		AX12_set_move_to_position_speed(SMALL_BALL_FRONT_LEFT_AX12_ID, SMALL_BALL_FRONT_LEFT_AX12_SPEED);
...
void SMALL_BALL_FRONT_LEFT_set_config(CAN_msg_t* msg){
...
static void SMALL_BALL_FRONT_LEFT_get_config(CAN_msg_t *incoming_msg){
	...
	msg.data.act_get_config_answer.sid = 0xFF & ACT_SMALL_BALL_FRONT_LEFT;
	...
	switch(incoming_msg->data.act_msg.act_data.config){
		case POSITION_CONFIG:
			SMALL_BALL_FRONT_LEFT_get_position_config(&order, &pos);
			...
			break;
		case TORQUE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.torque = AX12_get_speed_percentage(SMALL_BALL_FRONT_LEFT_AX12_ID);
			break;
		case TEMPERATURE_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.temperature = AX12_get_temperature(SMALL_BALL_FRONT_LEFT_AX12_ID);
			break;
		case LOAD_CONFIG:
			msg.data.act_get_config_answer.act_get_config_data.load = AX12_get_load_percentage(SMALL_BALL_FRONT_LEFT_AX12_ID);
			break;
		default:
			error = TRUE;
...
static void SMALL_BALL_FRONT_LEFT_get_position_config(ACT_order_e *pOrder, Uint16 *pPos){
	ACT_order_e order = ACT_SMALL_BALL_FRONT_LEFT_STOP;
	Uint16 position = AX12_get_position(SMALL_BALL_FRONT_LEFT_AX12_ID);
	Uint16 epsilon = SMALL_BALL_FRONT_LEFT_AX12_ASSER_POS_EPSILON;

	if(position > SMALL_BALL_FRONT_LEFT_AX12_UP_POS - epsilon && position < SMALL_BALL_FRONT_LEFT_AX12_UP_POS + epsilon){
		order = ACT_SMALL_BALL_FRONT_LEFT_UP;
	}else if(position > SMALL_BALL_FRONT_LEFT_AX12_DOWN_POS - epsilon && position < SMALL_BALL_FRONT_LEFT_AX12_DOWN_POS + epsilon){
		order = ACT_SMALL_BALL_FRONT_LEFT_DOWN;
	}else if(position > SMALL_BALL_FRONT_LEFT_AX12_IDLE_POS - epsilon && position < SMALL_BALL_FRONT_LEFT_AX12_IDLE_POS + epsilon){
		order = ACT_SMALL_BALL_FRONT_LEFT_IDLE;
	}
...
bool_e SMALL_BALL_FRONT_LEFT_CAN_process_msg(CAN_msg_t* msg) {
	if(msg->sid == ACT_SMALL_BALL_FRONT_LEFT){
		...
		switch(msg->data.act_msg.order) {
			...
			case ACT_SMALL_BALL_FRONT_LEFT_DOWN :
			case ACT_SMALL_BALL_FRONT_LEFT_UP :
			...
			case ACT_WARNER:
				SMALL_BALL_FRONT_LEFT_set_warner(msg);
				break;

			case ACT_GET_CONFIG:
				SMALL_BALL_FRONT_LEFT_get_config(msg);
				break;

			case ACT_SET_CONFIG :
				SMALL_BALL_FRONT_LEFT_set_config(msg);
				break;
...
static void SMALL_BALL_FRONT_LEFT_command_init(queue_id_t queueId) {
	...
	*ax12_goalPosition = 0xFFFF;
	...
	switch(command) {
		...
		case ACT_SMALL_BALL_FRONT_LEFT_DOWN : *ax12_goalPosition = SMALL_BALL_FRONT_LEFT_AX12_DOWN_POS; break;
		case ACT_SMALL_BALL_FRONT_LEFT_UP : *ax12_goalPosition = SMALL_BALL_FRONT_LEFT_AX12_UP_POS; break;
	...
	AX12_reset_last_error(SMALL_BALL_FRONT_LEFT_AX12_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
	if(!AX12_set_position(SMALL_BALL_FRONT_LEFT_AX12_ID, *ax12_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que l'AX12 ne répond pas a cet envoi, on l'indique à la carte stratégie
...
static void SMALL_BALL_FRONT_LEFT_command_run(queue_id_t queueId) {
	...
	Uint16 pos = AX12_get_position(SMALL_BALL_FRONT_LEFT_AX12_ID);

	if(!run_now){
		if(ACTQ_check_status_ax12(queueId, SMALL_BALL_FRONT_LEFT_AX12_ID, QUEUE_get_arg(queueId)->param, pos, SMALL_BALL_FRONT_LEFT_AX12_ASSER_POS_EPSILON, SMALL_BALL_FRONT_LEFT_AX12_ASSER_TIMEOUT, SMALL_BALL_FRONT_LEFT_AX12_ASSER_POS_LARGE_EPSILON, &result, &errorCode, &line))
		...
		ACTQ_flush_queue_to_run_now(queueId, ACT_SMALL_BALL_FRONT_LEFT);
	}

    // On ne surveille le warner que si il est activé
	if(warner.activated)
		SMALL_BALL_FRONT_LEFT_check_warner(pos);
}

static void SMALL_BALL_FRONT_LEFT_set_warner(CAN_msg_t *msg){
	warner.activated = TRUE;
	warner.last_pos = AX12_get_position(SMALL_BALL_FRONT_LEFT_AX12_ID);

	switch(msg->data.act_msg.act_data.warner_pos){
		/*case ACT_SMALL_BALL_FRONT_LEFT_WARNER:
			warner.warner_pos = SMALL_BALL_FRONT_LEFT_AX12_WARNER_POS;
			break;*/
		default:{
...
static void SMALL_BALL_FRONT_LEFT_check_warner(Uint16 pos){
	...
	if( (warner.last_pos < pos && warner.last_pos < warner.warner_pos && pos > warner.warner_pos)
	 ...
	 || (warner.last_pos < AX12_MIN_WARNER && pos > AX12_MAX_WARNER	&& (warner.last_pos > warner.warner_pos || pos < warner.warner_pos))
		...
		msg.data.act_warner_answer.sid = 0xFF & ACT_SMALL_BALL_FRONT_LEFT;
		CAN_send(&msg);
