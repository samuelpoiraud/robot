...
bool_e EXEMPLE_CAN_process_msg(CAN_msg_t* msg) {
	if(msg->sid == ACT_EXEMPLE){
		...
		/*SELFTEST_set_actions(&EXEMPLE_run_command,  3, (SELFTEST_action_t[]){
								 {ACT_EXEMPLE_IDLE,		0,  QUEUE_ACT_RX24_EXEMPLE},
								 {ACT_EXEMPLE_OPEN,       0,  QUEUE_ACT_RX24_EXEMPLE},
								 {ACT_EXEMPLE_IDLE,		0,  QUEUE_ACT_RX24_EXEMPLE}
							 });*/
	}
	return FALSE;
...
static void EXEMPLE_command_init(queue_id_t queueId) {
	...
	*rx24_goalPosition = 0xFFFF;
	...
	switch(command) {
		...
		case ACT_EXEMPLE_CLOSE : *rx24_goalPosition = EXEMPLE_RX24_CLOSE_POS; break;
		case ACT_EXEMPLE_OPEN : *rx24_goalPosition = EXEMPLE_RX24_OPEN_POS; break;
	...
	RX24_reset_last_error(EXEMPLE_RX24_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
	if(!RX24_set_position(EXEMPLE_RX24_ID, *rx24_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que le RX24 ne répond pas a cet envoi, on l'indique à la carte stratégie
