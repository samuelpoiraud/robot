...
*	Description : Gestion de la pompe POMPE_EXEMPLE
*	Auteur :
*	Version 2017
*	Robot : BIG
*/
...
// Un define POMPE_EXEMPLE_PIN doit avoir été ajouté au fichier config_big/config_pin.h // config_small/config_pin.h
...
#define LOG_PREFIX "pompe_exemple.c : "
...
static void POMPE_EXEMPLE_do_order(Uint8 command, Uint8 param);
...
static void POMPE_EXEMPLE_init_pwm() {
	...
	info_printf("Pompe POMPE_EXEMPLE initialisée\n");
}
...
static void POMPE_EXEMPLE_command_init(queue_id_t queueId) {
	...
	switch(command) {
		case ACT_POMPE_NORMAL:
		case ACT_POMPE_REVERSE:
			POMPE_EXEMPLE_do_order(command, param);
			break;
...
static void POMPE_EXEMPLE_do_order(Uint8 command, Uint8 param){
	if(command == ACT_POMPE_NORMAL)
		GPIO_SetBits(POMPE_EXEMPLE_SENS);
	else if(command == ACT_POMPE_REVERSE)
		GPIO_ResetBits(POMPE_EXEMPLE_SENS);
	else{
		debug_printf("commande envoyée à POMPE_EXEMPLE_do_order inconnue -> %d	%x\n", command, command);
		PWM_stop(POMPE_EXEMPLE_PWM_NUM);
		return;
	}

	param = (param > 100) ? 100 : param;
	PWM_run(param, POMPE_EXEMPLE_PWM_NUM);
}
