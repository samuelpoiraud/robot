...
// Formatage : QUEUE_ACT_POMPE_FRONT_RIGHT
...
static void POMPE_FRONT_RIGHT_run_command(queue_id_t queueId, bool_e init);
...
bool_e POMPE_FRONT_RIGHT_CAN_process_msg(CAN_msg_t* msg) {
	if(msg->sid == ACT_POMPE_FRONT_RIGHT) {
		...
		SELFTEST_set_actions(&POMPE_FRONT_RIGHT_run_command, 27, 3, (SELFTEST_action_t[]){
