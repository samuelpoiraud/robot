...
#define POMPE_BACK_RIGHT_PIN 0,0
...
#define LOG_PREFIX "pompe_back_right : "
...
static void POMPE_BACK_RIGHT_run_command(queue_id_t queueId, bool_e init);
	...
	if(msg->sid == ACT_POMPE_BACK_RIGHT) {
		switch(msg->data.act_msg.order) {
			...
			case ACT_POMPE_BACK_RIGHT_STOP:
				ACTQ_push_operation_from_msg(msg, QUEUE_ACT_POMPE_BACK_RIGHT, &POMPE_BACK_RIGHT_run_command, msg->data.act_msg.act_data.act_optionnal_data[0], TRUE);
				break;
...
static void POMPE_BACK_RIGHT_run_command(queue_id_t queueId, bool_e init) {
	...
	if(QUEUE_get_act(queueId) == QUEUE_ACT_POMPE_BACK_RIGHT) {
			if(init)
				POMPE_BACK_RIGHT_command_init(queueId);
			else
				POMPE_BACK_RIGHT_command_run(queueId);
	}
}
...
static void POMPE_BACK_RIGHT_command_init(queue_id_t queueId) {
	...
	switch(command) {
		case ACT_POMPE_BACK_RIGHT_NORMAL:
			POMPE_BACK_RIGHT_do_order(command);
			break;
		...
		default: {
			error_printf("Invalid POMPE_BACK_RIGHT command: %u, code is broken !\n", command);
			QUEUE_next(queueId, ACT_POMPE_BACK_RIGHT, ACT_RESULT_NOT_HANDLED, ACT_RESULT_ERROR_LOGIC, __LINE__);
			return;
		}
	}
}
...
static void POMPE_BACK_RIGHT_command_run(queue_id_t queueId){
	QUEUE_next(queueId, ACT_POMPE_BACK_RIGHT, ACT_RESULT_DONE, ACT_RESULT_ERROR_OK, 0);
}

static void POMPE_BACK_RIGHT_do_order(Uint8 command){
	if(command == ACT_POMPE_BACK_RIGHT_NORMAL)
		GPIO_SetBits(POMPE_BACK_RIGHT_PIN);
	else{
		debug_printf("commande envoyée à POMPE_BACK_RIGHT_do_order inconnue -> %d	0x%x\n", command, command);
		GPIO_ResetBits(POMPE_BACK_RIGHT_PIN);
		return;
	}
}
