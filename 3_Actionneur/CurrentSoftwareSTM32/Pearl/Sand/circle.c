...
// CIRCLE d'un actionneur standart avec AX12
...
// Formatage : QUEUE_ACT_AX12_CIRCLE
...
// Ajout du pilotage via terminal dans le fichier term_io.c dans le tableau terminal_motor du type : ACT_DECLARE(CIRCLE)
// Un define CIRCLE_AX12_ID doit avoir été ajouté au fichier config_big/config_pin.h // config_small/config_pin.h
...
#define LOG_PREFIX "CIRCLE.c : "
#define LOG_COMPONENT OUTPUT_LOG_COMPONENT_CIRCLE
#include "../QS/QS_outputlog.h"

// Les fonctions internes au fonctionnement de l'actionneur
static void CIRCLE_command_run(queue_id_t queueId);
static void CIRCLE_initAX12();
static void CIRCLE_command_init(queue_id_t queueId);
static void CIRCLE_config(CAN_msg_t* msg);
...
static void CIRCLE_initAX12() {
	if(ax12_is_initialized == FALSE && AX12_is_ready(CIRCLE_AX12_ID) == TRUE) {
		ax12_is_initialized = TRUE;
		AX12_config_set_lowest_voltage(CIRCLE_AX12_ID, AX12_MIN_VOLTAGE);
		AX12_config_set_highest_voltage(CIRCLE_AX12_ID, AX12_MAX_VOLTAGE);
		AX12_set_torque_limit(CIRCLE_AX12_ID, CIRCLE_AX12_MAX_TORQUE_PERCENT);
		AX12_config_set_temperature_limit(CIRCLE_AX12_ID, CIRCLE_AX12_MAX_TEMPERATURE);

		AX12_config_set_maximal_angle(CIRCLE_AX12_ID, CIRCLE_AX12_MAX_VALUE);
		AX12_config_set_minimal_angle(CIRCLE_AX12_ID, CIRCLE_AX12_MIN_VALUE);

		AX12_config_set_error_before_led(CIRCLE_AX12_ID, AX12_BEFORE_LED);
		AX12_config_set_error_before_shutdown(CIRCLE_AX12_ID, AX12_BEFORE_SHUTDOWN);
...
void CIRCLE_config(CAN_msg_t* msg){
	switch(msg->data.act_msg.act_data.act_config.sub_act_id){
		case DEFAULT_MONO_ACT : // Premier élement de l'actionneur
			ACTMGR_config_AX12(CIRCLE_AX12_ID, msg);
			break;
...
void CIRCLE_init_pos(){
	...
	if(!AX12_set_position(CIRCLE_AX12_ID, CIRCLE_AX12_INIT_POS))
...
void CIRCLE_stop(){
	AX12_set_torque_enabled(CIRCLE_AX12_ID, FALSE); //Stopper l'asservissement de l'AX12
}

// fonction appellée à la réception d'un message CAN (via ActManager)
bool_e CIRCLE_CAN_process_msg(CAN_msg_t* msg) {
	if(msg->sid == ACT_CIRCLE){
		CIRCLE_initAX12();
		switch(msg->data.act_msg.order) {
			// Listing de toutes les positions de l'actionneur possible
			case ACT_CIRCLE_IDLE :
			case ACT_CIRCLE_OPEN :
			case ACT_CIRCLE_CLOSE :
			case ACT_CIRCLE_STOP :
				ACTQ_push_operation_from_msg(msg, QUEUE_ACT_AX12_CIRCLE, &CIRCLE_run_command, 0,TRUE);
				break;
		...
		SELFTEST_set_actions(&CIRCLE_run_command, 3, 3, (SELFTEST_action_t[]){
								 {ACT_CIRCLE_IDLE,		0,  QUEUE_ACT_AX12_CIRCLE},
								 {ACT_CIRCLE_OPEN,       0,  QUEUE_ACT_AX12_CIRCLE},
								 {ACT_CIRCLE_IDLE,		0,  QUEUE_ACT_AX12_CIRCLE}
							 });
	}
	return FALSE;
...
void CIRCLE_run_command(queue_id_t queueId, bool_e init) {
	...
	if(QUEUE_get_act(queueId) == QUEUE_ACT_AX12_CIRCLE) {    // Gestion des mouvements de CIRCLE
		if(init)
			CIRCLE_command_init(queueId);
		else
			CIRCLE_command_run(queueId);
	}
}
...
static void CIRCLE_command_init(queue_id_t queueId) {
	...
	*ax12_goalPosition = 0xFFFF;
	...
	switch(command) {
		...
		case ACT_CIRCLE_IDLE : *ax12_goalPosition = CIRCLE_AX12_IDLE_POS; break;
		case ACT_CIRCLE_CLOSE : *ax12_goalPosition = CIRCLE_AX12_CLOSE_POS; break;
		case ACT_CIRCLE_OPEN : *ax12_goalPosition = CIRCLE_AX12_OPEN_POS; break;

		case ACT_CIRCLE_STOP :
			AX12_set_torque_enabled(CIRCLE_AX12_ID, FALSE); //Stopper l'asservissement de l'AX12
			QUEUE_next(queueId, ACT_CIRCLE, ACT_RESULT_DONE, ACT_RESULT_ERROR_OK, __LINE__);
			return;

		default: {
			error_printf("Invalid CIRCLE command: %u, code is broken !\n", command);
			QUEUE_next(queueId, ACT_CIRCLE, ACT_RESULT_NOT_HANDLED, ACT_RESULT_ERROR_LOGIC, __LINE__);
			return;
		}
	}
	...
	if(ax12_is_initialized == FALSE){
		...
		QUEUE_next(queueId, ACT_CIRCLE, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	...
	AX12_reset_last_error(CIRCLE_AX12_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
	if(!AX12_set_position(CIRCLE_AX12_ID, *ax12_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que l'AX12 ne répond pas a cet envoi, on l'indique à la carte stratégie
		error_printf("AX12_set_position error: 0x%x\n", AX12_get_last_error(CIRCLE_AX12_ID).error);
		QUEUE_next(queueId, ACT_CIRCLE, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	//La commande a été envoyée et l'AX12 l'a bien reçu
	debug_printf("Move CIRCLE ax12 to %d\n", *ax12_goalPosition);
}
...
static void CIRCLE_command_run(queue_id_t queueId) {
	...
	if(ACTQ_check_status_ax12(queueId, CIRCLE_AX12_ID, QUEUE_get_arg(queueId)->param, CIRCLE_AX12_ASSER_POS_EPSILON, CIRCLE_AX12_ASSER_TIMEOUT, CIRCLE_AX12_ASSER_POS_LARGE_EPSILON, &result, &errorCode, &line))
		QUEUE_next(queueId, ACT_CIRCLE, result, errorCode, line);
}
