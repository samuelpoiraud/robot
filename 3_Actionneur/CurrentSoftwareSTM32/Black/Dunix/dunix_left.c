...
#include "dunix_left.h"


// DUNIX_LEFT d'un actionneur standart avec RX24
...
// Formatage : QUEUE_ACT_RX24_DUNIX_LEFT
...
// Ajout du pilotage via terminal dans le fichier term_io.c dans le tableau terminal_motor du type : ACT_DECLARE(DUNIX_LEFT)
// Un define DUNIX_LEFT_RX24_ID doit avoir été ajouté au fichier config_big/config_pin.h // config_small/config_pin.h
...
#include "dunix_left_config.h"

// Les différents define pour le verbose sur uart
#define LOG_PREFIX "dunix_left.c : "
#define LOG_COMPONENT OUTPUT_LOG_COMPONENT_DUNIX_LEFT
#include "../QS/QS_outputlog.h"

// Les fonctions internes au fonctionnement de l'actionneur
static void DUNIX_LEFT_command_run(queue_id_t queueId);
static void DUNIX_LEFT_initRX24();
static void DUNIX_LEFT_command_init(queue_id_t queueId);
static void DUNIX_LEFT_config(CAN_msg_t* msg);
...
static void DUNIX_LEFT_initRX24() {
	if(rx24_is_initialized == FALSE && RX24_is_ready(DUNIX_LEFT_RX24_ID) == TRUE) {
		rx24_is_initialized = TRUE;
		RX24_config_set_lowest_voltage(DUNIX_LEFT_RX24_ID, RX24_MIN_VOLTAGE);
		RX24_config_set_highest_voltage(DUNIX_LEFT_RX24_ID, RX24_MAX_VOLTAGE);
		RX24_set_torque_limit(DUNIX_LEFT_RX24_ID, DUNIX_LEFT_RX24_MAX_TORQUE_PERCENT);
		RX24_config_set_temperature_limit(DUNIX_LEFT_RX24_ID, DUNIX_LEFT_RX24_MAX_TEMPERATURE);

		RX24_config_set_maximal_angle(DUNIX_LEFT_RX24_ID, DUNIX_LEFT_RX24_MAX_VALUE);
		RX24_config_set_minimal_angle(DUNIX_LEFT_RX24_ID, DUNIX_LEFT_RX24_MIN_VALUE);

		RX24_config_set_error_before_led(DUNIX_LEFT_RX24_ID, RX24_BEFORE_LED);
		RX24_config_set_error_before_shutdown(DUNIX_LEFT_RX24_ID, RX24_BEFORE_SHUTDOWN);
...
void DUNIX_LEFT_config(CAN_msg_t* msg){
	switch(msg->data.act_msg.act_data.act_config.sub_act_id){
		case DEFAULT_MONO_ACT : // Premier élement de l'actionneur
			ACTMGR_config_RX24(DUNIX_LEFT_RX24_ID, msg);
			break;
...
void DUNIX_LEFT_init_pos(){
	...
	if(!RX24_set_position(DUNIX_LEFT_RX24_ID, DUNIX_LEFT_RX24_INIT_POS))
...
void DUNIX_LEFT_run_command(queue_id_t queueId, bool_e init) {
	...
	if(QUEUE_get_act(queueId) == QUEUE_ACT_RX24_DUNIX_LEFT) {    // Gestion des mouvements de DUNIX_LEFT
		if(init)
			DUNIX_LEFT_command_init(queueId);
		else
			DUNIX_LEFT_command_run(queueId);
	}
}
...
static void DUNIX_LEFT_command_init(queue_id_t queueId) {
	...
	*rx24_goalPosition = 0xFFFF;
	...
	switch(command) {
		...
		case ACT_DUNIX_LEFT_STOP :
			RX24_set_torque_enabled(DUNIX_LEFT_RX24_ID, FALSE); //Stopper l'asservissement du RX24
			QUEUE_next(queueId, ACT_DUNIX_LEFT, ACT_RESULT_DONE, ACT_RESULT_ERROR_OK, __LINE__);
			return;

		default: {
			error_printf("Invalid DUNIX_LEFT command: %u, code is broken !\n", command);
			QUEUE_next(queueId, ACT_DUNIX_LEFT, ACT_RESULT_NOT_HANDLED, ACT_RESULT_ERROR_LOGIC, __LINE__);
			return;
		}
	}
	...
	if(rx24_is_initialized == FALSE){
		...
		QUEUE_next(queueId, ACT_DUNIX_LEFT, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	...
	RX24_reset_last_error(DUNIX_LEFT_RX24_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
	if(!RX24_set_position(DUNIX_LEFT_RX24_ID, *rx24_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que le RX24 ne répond pas a cet envoi, on l'indique à la carte stratégie
		error_printf("RX24_set_position error: 0x%x\n", RX24_get_last_error(DUNIX_LEFT_RX24_ID).error);
		QUEUE_next(queueId, ACT_DUNIX_LEFT, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	//La commande a été envoyée et le RX24 l'a bien reçu
	debug_printf("Move DUNIX_LEFT rx24 to %d\n", *rx24_goalPosition);
}
...
static void DUNIX_LEFT_command_run(queue_id_t queueId) {
	...
	if(ACTQ_check_status_rx24(queueId, DUNIX_LEFT_RX24_ID, QUEUE_get_arg(queueId)->param, DUNIX_LEFT_RX24_ASSER_POS_EPSILON, DUNIX_LEFT_RX24_ASSER_TIMEOUT, DUNIX_LEFT_RX24_ASSER_POS_LARGE_EPSILON, &result, &errorCode, &line))
		QUEUE_next(queueId, ACT_DUNIX_LEFT, result, errorCode, line);
}
