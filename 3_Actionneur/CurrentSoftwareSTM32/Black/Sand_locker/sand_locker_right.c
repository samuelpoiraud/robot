...
// SAND_LOCKER_RIGHT d'un actionneur standart avec RX24
...
// Formatage : QUEUE_ACT_RX24_SAND_LOCKER_RIGHT
...
// Ajout du pilotage via terminal dans le fichier term_io.c dans le tableau terminal_motor du type : ACT_DECLARE(SAND_LOCKER_RIGHT)
// Un define SAND_LOCKER_RIGHT_RX24_ID doit avoir été ajouté au fichier config_big/config_pin.h // config_small/config_pin.h
...
bool_e SAND_LOCKER_RIGHT_CAN_process_msg(CAN_msg_t* msg) {
	if(msg->sid == ACT_SAND_LOCKER_RIGHT){
		...
		switch(msg->data.act_msg.order) {
			...
			case ACT_SAND_LOCKER_RIGHT_OPEN :
			case ACT_SAND_LOCKER_RIGHT_CLOSE :
...
static void SAND_LOCKER_RIGHT_command_init(queue_id_t queueId) {
	...
	*rx24_goalPosition = 0xFFFF;
	...
	switch(command) {
		...
		case ACT_SAND_LOCKER_RIGHT_CLOSE : *rx24_goalPosition = SAND_LOCKER_RIGHT_RX24_CLOSE_POS; break;
		case ACT_SAND_LOCKER_RIGHT_OPEN : *rx24_goalPosition = SAND_LOCKER_RIGHT_RX24_OPEN_POS; break;
	...
	RX24_reset_last_error(SAND_LOCKER_RIGHT_RX24_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
	if(!RX24_set_position(SAND_LOCKER_RIGHT_RX24_ID, *rx24_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que le RX24 ne répond pas a cet envoi, on l'indique à la carte stratégie
