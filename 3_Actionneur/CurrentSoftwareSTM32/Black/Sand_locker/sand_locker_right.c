	...
	if(msg->sid == ACT_SAND_LOCKER_RIGHT){
		...
		switch(msg->data.act_msg.order) {
			...
			case ACT_SAND_LOCKER_RIGHT_IDLE :
				RX24_set_torque_enabled(SAND_LOCKER_RIGHT_RX24_ID, TRUE);
...
static void SAND_LOCKER_RIGHT_command_init(queue_id_t queueId) {
	...
	*rx24_goalPosition = 0xFFFF;
	...
	switch(command) {
		...
		case ACT_SAND_LOCKER_RIGHT_LOCK_BLOCK_SERRAGE : *rx24_goalPosition = SAND_LOCKER_RIGHT_RX24_LOCK_BLOCK_SERRAGE_POS; break;
