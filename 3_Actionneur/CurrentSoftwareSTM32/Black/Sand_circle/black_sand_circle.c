...
// BLACK_SAND_CIRCLE d'un actionneur standart avec RX24
...
// Formatage : QUEUE_ACT_RX24_BLACK_SAND_CIRCLE
...
// Ajout du pilotage via terminal dans le fichier term_io.c dans le tableau terminal_motor du type : ACT_DECLARE(BLACK_SAND_CIRCLE)
// Un define BLACK_SAND_CIRCLE_RX24_ID doit avoir été ajouté au fichier config_big/config_pin.h // config_small/config_pin.h
...
static void BLACK_SAND_CIRCLE_initRX24();
...
static void BLACK_SAND_CIRCLE_initRX24() {
	if(rx24_is_initialized == FALSE && RX24_is_ready(BLACK_SAND_CIRCLE_RX24_ID) == TRUE) {
		rx24_is_initialized = TRUE;
		RX24_config_set_lowest_voltage(BLACK_SAND_CIRCLE_RX24_ID, RX24_MIN_VOLTAGE);
		RX24_config_set_highest_voltage(BLACK_SAND_CIRCLE_RX24_ID, RX24_MAX_VOLTAGE);
		RX24_set_torque_limit(BLACK_SAND_CIRCLE_RX24_ID, BLACK_SAND_CIRCLE_RX24_MAX_TORQUE_PERCENT);
		RX24_config_set_temperature_limit(BLACK_SAND_CIRCLE_RX24_ID, BLACK_SAND_CIRCLE_RX24_MAX_TEMPERATURE);

		RX24_config_set_maximal_angle(BLACK_SAND_CIRCLE_RX24_ID, BLACK_SAND_CIRCLE_RX24_MAX_VALUE);
		RX24_config_set_minimal_angle(BLACK_SAND_CIRCLE_RX24_ID, BLACK_SAND_CIRCLE_RX24_MIN_VALUE);

		RX24_config_set_error_before_led(BLACK_SAND_CIRCLE_RX24_ID, RX24_BEFORE_LED);
		RX24_config_set_error_before_shutdown(BLACK_SAND_CIRCLE_RX24_ID, RX24_BEFORE_SHUTDOWN);
...
void BLACK_SAND_CIRCLE_config(CAN_msg_t* msg){
	switch(msg->data.act_msg.act_data.act_config.sub_act_id){
		case DEFAULT_MONO_ACT : // Premier élement de l'actionneur
			ACTMGR_config_RX24(BLACK_SAND_CIRCLE_RX24_ID, msg);
			break;
...
void BLACK_SAND_CIRCLE_init_pos(){
	...
	if(!RX24_set_position(BLACK_SAND_CIRCLE_RX24_ID, BLACK_SAND_CIRCLE_RX24_INIT_POS))
		debug_printf("   Le RX24 n°%d n'est pas là\n", BLACK_SAND_CIRCLE_RX24_ID);
	else
...
void BLACK_SAND_CIRCLE_stop(){
	RX24_set_torque_enabled(BLACK_SAND_CIRCLE_RX24_ID, FALSE); //Stopper l'asservissement du RX24
}
	...
	if(msg->sid == ACT_BLACK_SAND_CIRCLE){
		...
		switch(msg->data.act_msg.order) {
			...
			case ACT_BLACK_SAND_CIRCLE_STOP :
				ACTQ_push_operation_from_msg(msg, QUEUE_ACT_RX24_BLACK_SAND_CIRCLE, &BLACK_SAND_CIRCLE_run_command, 0,TRUE);
				break;
		...
		/*SELFTEST_set_actions(&BLACK_SAND_CIRCLE_run_command,  3, (SELFTEST_action_t[]){
								 ...
								 {ACT_BLACK_SAND_CIRCLE_OPEN,       0,  QUEUE_ACT_RX24_BLACK_SAND_CIRCLE},
								 {ACT_BLACK_SAND_CIRCLE_IDLE,		0,  QUEUE_ACT_RX24_BLACK_SAND_CIRCLE}
							 });*/
	}
	return FALSE;
...
void BLACK_SAND_CIRCLE_run_command(queue_id_t queueId, bool_e init) {
	...
	if(QUEUE_get_act(queueId) == QUEUE_ACT_RX24_BLACK_SAND_CIRCLE) {    // Gestion des mouvements de BLACK_SAND_CIRCLE
		if(init)
...
static void BLACK_SAND_CIRCLE_command_init(queue_id_t queueId) {
	...
	*rx24_goalPosition = 0xFFFF;
	...
	switch(command) {
		...
		case ACT_BLACK_SAND_CIRCLE_IDLE : *rx24_goalPosition = BLACK_SAND_CIRCLE_RX24_IDLE_POS; break;
		case ACT_BLACK_SAND_CIRCLE_LOCK : *rx24_goalPosition = BLACK_SAND_CIRCLE_RX24_LOCK_POS; break;
		case ACT_BLACK_SAND_CIRCLE_UNLOCK : *rx24_goalPosition = BLACK_SAND_CIRCLE_RX24_UNLOCK_POS; break;
	...
	RX24_reset_last_error(BLACK_SAND_CIRCLE_RX24_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
	if(!RX24_set_position(BLACK_SAND_CIRCLE_RX24_ID, *rx24_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que le RX24 ne répond pas a cet envoi, on l'indique à la carte stratégie
		error_printf("RX24_set_position error: 0x%x\n", RX24_get_last_error(BLACK_SAND_CIRCLE_RX24_ID).error);
		QUEUE_next(queueId, ACT_BLACK_SAND_CIRCLE, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	//La commande a été envoyée et le RX24 l'a bien reçu
	debug_printf("Move BLACK_SAND_CIRCLE rx24 to %d\n", *rx24_goalPosition);
}
...
static void BLACK_SAND_CIRCLE_command_run(queue_id_t queueId) {
	...
	if(ACTQ_check_status_rx24(queueId, BLACK_SAND_CIRCLE_RX24_ID, QUEUE_get_arg(queueId)->param, BLACK_SAND_CIRCLE_RX24_ASSER_POS_EPSILON, BLACK_SAND_CIRCLE_RX24_ASSER_TIMEOUT, BLACK_SAND_CIRCLE_RX24_ASSER_POS_LARGE_EPSILON, &result, &errorCode, &line))
