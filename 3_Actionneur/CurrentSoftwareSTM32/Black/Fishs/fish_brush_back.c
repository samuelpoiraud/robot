...
static void FISH_BRUSH_BACK_initRX24() {
	if(rx24_is_initialized == FALSE && AX12_is_ready(FISH_BRUSH_BACK_AX12_RX24_ID) == TRUE) {
		rx24_is_initialized = TRUE;
		AX12_config_set_lowest_voltage(FISH_BRUSH_BACK_AX12_RX24_ID, AX12_MIN_VOLTAGE);
		AX12_config_set_highest_voltage(FISH_BRUSH_BACK_AX12_RX24_ID, AX12_MAX_VOLTAGE);
		AX12_set_torque_limit(FISH_BRUSH_BACK_AX12_RX24_ID, FISH_BRUSH_BACK_AX12_RX24_MAX_TORQUE_PERCENT);
		AX12_config_set_temperature_limit(FISH_BRUSH_BACK_AX12_RX24_ID, FISH_BRUSH_BACK_AX12_RX24_MAX_TEMPERATURE);

		AX12_config_set_maximal_angle(FISH_BRUSH_BACK_AX12_RX24_ID, FISH_BRUSH_BACK_AX12_RX24_MAX_VALUE);
		AX12_config_set_minimal_angle(FISH_BRUSH_BACK_AX12_RX24_ID, FISH_BRUSH_BACK_AX12_RX24_MIN_VALUE);

		AX12_config_set_error_before_led(FISH_BRUSH_BACK_AX12_RX24_ID, AX12_BEFORE_LED);
		AX12_config_set_error_before_shutdown(FISH_BRUSH_BACK_AX12_RX24_ID, AX12_BEFORE_SHUTDOWN);
...
void FISH_BRUSH_BACK_config(CAN_msg_t* msg){
	switch(msg->data.act_msg.act_data.act_config.sub_act_id){
		case DEFAULT_MONO_ACT : // Premier élement de l'actionneur
			ACTMGR_config_AX12(FISH_BRUSH_BACK_AX12_RX24_ID, msg);
			break;
...
void FISH_BRUSH_BACK_init_pos(){
	...
	if(!AX12_set_position(FISH_BRUSH_BACK_AX12_RX24_ID, FISH_BRUSH_BACK_AX12_RX24_INIT_POS))
		debug_printf("   Le RX24 n°%d n'est pas là\n", FISH_BRUSH_BACK_AX12_RX24_ID);
	else
...
void FISH_BRUSH_BACK_stop(){
	AX12_set_torque_enabled(FISH_BRUSH_BACK_AX12_RX24_ID, FALSE); //Stopper l'asservissement du RX24
}
	...
	if(msg->sid == ACT_FISH_BRUSH_BACK){
		...
		switch(msg->data.act_msg.order) {
			...
			case ACT_FISH_BRUSH_BACK_STOP :
				ACTQ_push_operation_from_msg(msg, QUEUE_ACT_AX12_RX24_FISH_BRUSH_BACK, &FISH_BRUSH_BACK_run_command, 0,TRUE);
				break;
		...
		SELFTEST_set_actions(&FISH_BRUSH_BACK_run_command, 3, 3, (SELFTEST_action_t[]){
								 {ACT_FISH_BRUSH_BACK_IDLE,		0,  QUEUE_ACT_AX12_RX24_FISH_BRUSH_BACK},
								 {ACT_FISH_BRUSH_BACK_OPEN,       0,  QUEUE_ACT_AX12_RX24_FISH_BRUSH_BACK},
								 {ACT_FISH_BRUSH_BACK_IDLE,		0,  QUEUE_ACT_AX12_RX24_FISH_BRUSH_BACK}
							 });
	}
	return FALSE;
...
void FISH_BRUSH_BACK_run_command(queue_id_t queueId, bool_e init) {
	...
	if(QUEUE_get_act(queueId) == QUEUE_ACT_AX12_RX24_FISH_BRUSH_BACK) {    // Gestion des mouvements de FISH_BRUSH_BACK
		if(init)
...
static void FISH_BRUSH_BACK_command_init(queue_id_t queueId) {
	...
	*rx24_goalPosition = 0xFFFF;
	...
	switch(command) {
		...
		case ACT_FISH_BRUSH_BACK_IDLE : *rx24_goalPosition = FISH_BRUSH_BACK_AX12_RX24_IDLE_POS; break;
		case ACT_FISH_BRUSH_BACK_CLOSE : *rx24_goalPosition = FISH_BRUSH_BACK_AX12_RX24_CLOSE_POS; break;
		case ACT_FISH_BRUSH_BACK_OPEN : *rx24_goalPosition = FISH_BRUSH_BACK_AX12_RX24_OPEN_POS; break;
	...
	AX12_reset_last_error(FISH_BRUSH_BACK_AX12_RX24_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
	if(!AX12_set_position(FISH_BRUSH_BACK_AX12_RX24_ID, *rx24_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que le RX24 ne répond pas a cet envoi, on l'indique à la carte stratégie
		error_printf("AX12_set_position error: 0x%x\n", AX12_get_last_error(FISH_BRUSH_BACK_AX12_RX24_ID).error);
		QUEUE_next(queueId, ACT_FISH_BRUSH_BACK, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
...
static void FISH_BRUSH_BACK_command_run(queue_id_t queueId) {
	...
	if(ACTQ_check_status_ax12(queueId, FISH_BRUSH_BACK_AX12_RX24_ID, QUEUE_get_arg(queueId)->param, FISH_BRUSH_BACK_AX12_RX24_ASSER_POS_EPSILON, FISH_BRUSH_BACK_AX12_RX24_ASSER_TIMEOUT, FISH_BRUSH_BACK_AX12_RX24_ASSER_POS_LARGE_EPSILON, &result, &errorCode, &line))
