...
#include "bottom_dune_right_config.h"

// Les différents define pour le verbose sur uart
#define LOG_PREFIX "BOTTOM_DUNE_RIGHT_rx24.c : "
#define LOG_COMPONENT OUTPUT_LOG_COMPONENT_BOTTOM_DUNE_RIGHT
#include "../QS/QS_outputlog.h"

// Les fonctions internes au fonctionnement de l'actionneur
static void BOTTOM_DUNE_RIGHT_command_run(queue_id_t queueId);
static void BOTTOM_DUNE_RIGHT_initRX24();
static void BOTTOM_DUNE_RIGHT_command_init(queue_id_t queueId);
static void BOTTOM_DUNE_RIGHT_config(CAN_msg_t* msg);
...
static void BOTTOM_DUNE_RIGHT_initRX24() {
	if(rx24_is_initialized == FALSE && RX24_is_ready(BOTTOM_DUNE_RIGHT_RX24_ID) == TRUE) {
		...
		RX24_set_torque_limit(BOTTOM_DUNE_RIGHT_RX24_ID, BOTTOM_DUNE_RIGHT_RX24_MAX_TORQUE_PERCENT);
		RX24_config_set_temperature_limit(BOTTOM_DUNE_RIGHT_RX24_ID, BOTTOM_DUNE_RIGHT_RX24_MAX_TEMPERATURE);

		RX24_config_set_maximal_angle(BOTTOM_DUNE_RIGHT_RX24_ID, BOTTOM_DUNE_RIGHT_RX24_MAX_VALUE);
		RX24_config_set_minimal_angle(BOTTOM_DUNE_RIGHT_RX24_ID, BOTTOM_DUNE_RIGHT_RX24_MIN_VALUE);
...
void BOTTOM_DUNE_RIGHT_config(CAN_msg_t* msg){
...
void BOTTOM_DUNE_RIGHT_init_pos(){
	...
	if(!RX24_set_position(BOTTOM_DUNE_RIGHT_RX24_ID, BOTTOM_DUNE_RIGHT_RX24_INIT_POS))
...
bool_e BOTTOM_DUNE_RIGHT_CAN_process_msg(CAN_msg_t* msg) {
	if(msg->sid == ACT_BOTTOM_DUNE_RIGHT){
		BOTTOM_DUNE_RIGHT_initRX24();
...
void BOTTOM_DUNE_RIGHT_run_command(queue_id_t queueId, bool_e init) {
	...
	if(QUEUE_get_act(queueId) == QUEUE_ACT_RX24_BOTTOM_DUNE_RIGHT) {    // Gestion des mouvements de BOTTOM_DUNE_RIGHT
		if(init)
			BOTTOM_DUNE_RIGHT_command_init(queueId);
		else
			BOTTOM_DUNE_RIGHT_command_run(queueId);
	}
}
...
static void BOTTOM_DUNE_RIGHT_command_init(queue_id_t queueId) {
	...
	*rx24_goalPosition = 0xFFFF;
	...
	switch(command) {
		...
		case ACT_BOTTOM_DUNE_RIGHT_STOP :
			RX24_set_torque_enabled(BOTTOM_DUNE_RIGHT_RX24_ID, FALSE); //Stopper l'asservissement du RX24
			QUEUE_next(queueId, ACT_BOTTOM_DUNE_RIGHT, ACT_RESULT_DONE, ACT_RESULT_ERROR_OK, __LINE__);
			return;

		default: {
			error_printf("Invalid BOTTOM_DUNE_RIGHT command: %u, code is broken !\n", command);
			QUEUE_next(queueId, ACT_BOTTOM_DUNE_RIGHT, ACT_RESULT_NOT_HANDLED, ACT_RESULT_ERROR_LOGIC, __LINE__);
			return;
		}
	}
	...
	if(rx24_is_initialized == FALSE){
		...
		QUEUE_next(queueId, ACT_BOTTOM_DUNE_RIGHT, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	...
	if(!RX24_set_position(BOTTOM_DUNE_RIGHT_RX24_ID, *rx24_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que le RX24 ne répond pas a cet envoi, on l'indique à la carte stratégie
		error_printf("RX24_set_position error: 0x%x\n", RX24_get_last_error(BOTTOM_DUNE_RIGHT_RX24_ID).error);
		QUEUE_next(queueId, ACT_BOTTOM_DUNE_RIGHT, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	//La commande a été envoyée et le RX24 l'a bien reçu
	debug_printf("Move BOTTOM_DUNE_RIGHT rx24 to %d\n", *rx24_goalPosition);
}
...
static void BOTTOM_DUNE_RIGHT_command_run(queue_id_t queueId) {
	...
	if(ACTQ_check_status_rx24(queueId, BOTTOM_DUNE_RIGHT_RX24_ID, QUEUE_get_arg(queueId)->param, BOTTOM_DUNE_RIGHT_RX24_ASSER_POS_EPSILON, BOTTOM_DUNE_RIGHT_RX24_ASSER_TIMEOUT, BOTTOM_DUNE_RIGHT_RX24_ASSER_POS_LARGE_EPSILON, &result, &errorCode, &line))
		QUEUE_next(queueId, ACT_BOTTOM_DUNE_RIGHT, result, errorCode, line);
}
#endif
