...
static void BRUSH_DUNE_command_init(queue_id_t queueId) {
	...
	RX24_reset_last_error(BRUSH_DUNE_LEFT_RX24_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
	RX24_reset_last_error(BRUSH_DUNE_RIGHT_RX24_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.

	debug_printf("result = %d\n", result);

	if(!RX24_set_position(BRUSH_DUNE_LEFT_RX24_ID, rx24_goalPosition_left)) {	//Si la commande n'a pas été envoyée correctement et/ou que le RX24 ne répond pas a cet envoi, on l'indique à la carte stratégie
	...
	if(!RX24_set_position(BRUSH_DUNE_RIGHT_RX24_ID, rx24_goalPosition_right)) {	//Si la commande n'a pas été envoyée correctement et/ou que le RX24 ne répond pas a cet envoi, on l'indique à la carte stratégie
		error_printf("RX24_set_position error: 0x%x\n", RX24_get_last_error(BRUSH_DUNE_LEFT_RX24_ID).error);
		result = FALSE;
	}else{
