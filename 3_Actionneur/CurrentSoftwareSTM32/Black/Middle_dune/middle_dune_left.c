...
#include "middle_dune_left.h"


// MIDDLE_DUNE_LEFT d'un actionneur standart avec RX24
...
// Formatage : QUEUE_ACT_RX24_MIDDLE_DUNE_LEFT
...
// Ajout du pilotage via terminal dans le fichier term_io.c dans le tableau terminal_motor du type : ACT_DECLARE(MIDDLE_DUNE_LEFT)
// Un define MIDDLE_DUNE_LEFT_RX24_ID doit avoir été ajouté au fichier config_big/config_pin.h // config_small/config_pin.h
...
#include "middle_dune_left_config.h"

// Les différents define pour le verbose sur uart
#define LOG_PREFIX "middle_dune_left.c : "
#define LOG_COMPONENT OUTPUT_LOG_COMPONENT_MIDDLE_DUNE_LEFT
#include "../QS/QS_outputlog.h"

// Les fonctions internes au fonctionnement de l'actionneur
static void MIDDLE_DUNE_LEFT_command_run(queue_id_t queueId);
static void MIDDLE_DUNE_LEFT_initRX24();
static void MIDDLE_DUNE_LEFT_command_init(queue_id_t queueId);
static void MIDDLE_DUNE_LEFT_config(CAN_msg_t* msg);
...
static void MIDDLE_DUNE_LEFT_initRX24() {
	if(rx24_is_initialized == FALSE && RX24_is_ready(MIDDLE_DUNE_LEFT_RX24_ID) == TRUE) {
		...
		RX24_set_torque_limit(MIDDLE_DUNE_LEFT_RX24_ID, MIDDLE_DUNE_LEFT_RX24_MAX_TORQUE_PERCENT);
		RX24_config_set_temperature_limit(MIDDLE_DUNE_LEFT_RX24_ID, MIDDLE_DUNE_LEFT_RX24_MAX_TEMPERATURE);

		RX24_config_set_maximal_angle(MIDDLE_DUNE_LEFT_RX24_ID, MIDDLE_DUNE_LEFT_RX24_MAX_VALUE);
		RX24_config_set_minimal_angle(MIDDLE_DUNE_LEFT_RX24_ID, MIDDLE_DUNE_LEFT_RX24_MIN_VALUE);
...
void MIDDLE_DUNE_LEFT_config(CAN_msg_t* msg){
...
void MIDDLE_DUNE_LEFT_init_pos(){
	...
	if(!RX24_set_position(MIDDLE_DUNE_LEFT_RX24_ID, MIDDLE_DUNE_LEFT_RX24_INIT_POS))
...
bool_e MIDDLE_DUNE_LEFT_CAN_process_msg(CAN_msg_t* msg) {
	if(msg->sid == ACT_MIDDLE_DUNE_LEFT){
		MIDDLE_DUNE_LEFT_initRX24();
...
void MIDDLE_DUNE_LEFT_run_command(queue_id_t queueId, bool_e init) {
	...
	if(QUEUE_get_act(queueId) == QUEUE_ACT_RX24_MIDDLE_DUNE_LEFT) {    // Gestion des mouvements de MIDDLE_DUNE_LEFT
		if(init)
			MIDDLE_DUNE_LEFT_command_init(queueId);
		else
			MIDDLE_DUNE_LEFT_command_run(queueId);
	}
}
...
static void MIDDLE_DUNE_LEFT_command_init(queue_id_t queueId) {
	...
	*rx24_goalPosition = 0xFFFF;
	...
	switch(command) {
		...
		case ACT_MIDDLE_DUNE_LEFT_STOP :
			RX24_set_torque_enabled(MIDDLE_DUNE_LEFT_RX24_ID, FALSE); //Stopper l'asservissement du RX24
			QUEUE_next(queueId, ACT_MIDDLE_DUNE_LEFT, ACT_RESULT_DONE, ACT_RESULT_ERROR_OK, __LINE__);
			return;

		default: {
			error_printf("Invalid MIDDLE_DUNE_LEFT command: %u, code is broken !\n", command);
			QUEUE_next(queueId, ACT_MIDDLE_DUNE_LEFT, ACT_RESULT_NOT_HANDLED, ACT_RESULT_ERROR_LOGIC, __LINE__);
			return;
		}
	}
	...
	if(rx24_is_initialized == FALSE){
		...
		QUEUE_next(queueId, ACT_MIDDLE_DUNE_LEFT, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	...
	if(!RX24_set_position(MIDDLE_DUNE_LEFT_RX24_ID, *rx24_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que le RX24 ne répond pas a cet envoi, on l'indique à la carte stratégie
	...
	debug_printf("Move MIDDLE_DUNE_LEFT rx24 to %d\n", *rx24_goalPosition);
}
...
static void MIDDLE_DUNE_LEFT_command_run(queue_id_t queueId) {
	...
	if(ACTQ_check_status_rx24(queueId, MIDDLE_DUNE_LEFT_RX24_ID, QUEUE_get_arg(queueId)->param, MIDDLE_DUNE_LEFT_RX24_ASSER_POS_EPSILON, MIDDLE_DUNE_LEFT_RX24_ASSER_TIMEOUT, MIDDLE_DUNE_LEFT_RX24_ASSER_POS_LARGE_EPSILON, &result, &errorCode, &line))
		QUEUE_next(queueId, ACT_MIDDLE_DUNE_LEFT, result, errorCode, line);
}
