 ...
 *	BIG
 *
 *	Fichier : middle_dune_right.c
 *	Package : Carte actionneur
 *	Description : Gestion de l'actionneur MIDDLE_DUNE_RIGHT
 *  Auteur : Valentin
 *  Version 2016
 *  Robot : BIG
 */

#include "middle_dune_right.h"


// MIDDLE_DUNE_RIGHT d'un actionneur standart avec RX24
...
// Formatage : QUEUE_ACT_RX24_MIDDLE_DUNE_RIGHT
...
// Ajout du pilotage via terminal dans le fichier term_io.c dans le tableau terminal_motor du type : ACT_DECLARE(MIDDLE_DUNE_RIGHT)
// Un define MIDDLE_DUNE_RIGHT_RX24_ID doit avoir été ajouté au fichier config_big/config_pin.h // config_small/config_pin.h
...
#include "middle_dune_right_config.h"

// Les différents define pour le verbose sur uart
#define LOG_PREFIX "MIDDLE_DUNE_RIGHT.c : "
#define LOG_COMPONENT OUTPUT_LOG_COMPONENT_MIDDLE_DUNE_RIGHT
#include "../QS/QS_outputlog.h"

// Les fonctions internes au fonctionnement de l'actionneur
static void MIDDLE_DUNE_RIGHT_command_run(queue_id_t queueId);
static void MIDDLE_DUNE_RIGHT_initRX24();
static void MIDDLE_DUNE_RIGHT_command_init(queue_id_t queueId);
static void MIDDLE_DUNE_RIGHT_config(CAN_msg_t* msg);
...
static void MIDDLE_DUNE_RIGHT_initRX24() {
	if(rx24_is_initialized == FALSE && RX24_is_ready(MIDDLE_DUNE_RIGHT_RX24_ID) == TRUE) {
		...
		RX24_set_torque_limit(MIDDLE_DUNE_RIGHT_RX24_ID, MIDDLE_DUNE_RIGHT_RX24_MAX_TORQUE_PERCENT);
		RX24_config_set_temperature_limit(MIDDLE_DUNE_RIGHT_RX24_ID, MIDDLE_DUNE_RIGHT_RX24_MAX_TEMPERATURE);

		RX24_config_set_maximal_angle(MIDDLE_DUNE_RIGHT_RX24_ID, MIDDLE_DUNE_RIGHT_RX24_MAX_VALUE);
		RX24_config_set_minimal_angle(MIDDLE_DUNE_RIGHT_RX24_ID, MIDDLE_DUNE_RIGHT_RX24_MIN_VALUE);
...
void MIDDLE_DUNE_RIGHT_config(CAN_msg_t* msg){
...
void MIDDLE_DUNE_RIGHT_init_pos(){
	...
	if(!RX24_set_position(MIDDLE_DUNE_RIGHT_RX24_ID, MIDDLE_DUNE_RIGHT_RX24_INIT_POS))
...
bool_e MIDDLE_DUNE_RIGHT_CAN_process_msg(CAN_msg_t* msg) {
	if(msg->sid == ACT_MIDDLE_DUNE_RIGHT){
		MIDDLE_DUNE_RIGHT_initRX24();
		switch(msg->data.act_msg.order) {
			// Listing de toutes les positions de l'actionneur possible
			case ACT_MIDDLE_DUNE_RIGHT_IDLE :
			case ACT_MIDDLE_DUNE_RIGHT_OPEN :
			case ACT_MIDDLE_DUNE_RIGHT_CLOSE :
			case ACT_MIDDLE_DUNE_RIGHT_STOP :
				ACTQ_push_operation_from_msg(msg, QUEUE_ACT_RX24_MIDDLE_DUNE_RIGHT, &MIDDLE_DUNE_RIGHT_run_command, 0,TRUE);
				break;

			case ACT_CONFIG :
				MIDDLE_DUNE_RIGHT_config(msg);
				break;
...
void MIDDLE_DUNE_RIGHT_run_command(queue_id_t queueId, bool_e init) {
	...
	if(QUEUE_get_act(queueId) == QUEUE_ACT_RX24_MIDDLE_DUNE_RIGHT) {    // Gestion des mouvements de MIDDLE_DUNE_RIGHT
		if(init)
			MIDDLE_DUNE_RIGHT_command_init(queueId);
		else
			MIDDLE_DUNE_RIGHT_command_run(queueId);
	}
}
...
static void MIDDLE_DUNE_RIGHT_command_init(queue_id_t queueId) {
	...
	*rx24_goalPosition = 0xFFFF;
	...
	switch(command) {
		...
		case ACT_MIDDLE_DUNE_RIGHT_IDLE : *rx24_goalPosition = MIDDLE_DUNE_RIGHT_RX24_IDLE_POS; break;
		case ACT_MIDDLE_DUNE_RIGHT_CLOSE : *rx24_goalPosition = MIDDLE_DUNE_RIGHT_RX24_CLOSE_POS; break;
		case ACT_MIDDLE_DUNE_RIGHT_OPEN : *rx24_goalPosition = MIDDLE_DUNE_RIGHT_RX24_OPEN_POS; break;

		case ACT_MIDDLE_DUNE_RIGHT_STOP :
			RX24_set_torque_enabled(MIDDLE_DUNE_RIGHT_RX24_ID, FALSE); //Stopper l'asservissement du RX24
			QUEUE_next(queueId, ACT_MIDDLE_DUNE_RIGHT, ACT_RESULT_DONE, ACT_RESULT_ERROR_OK, __LINE__);
			return;

		default: {
			error_printf("Invalid MIDDLE_DUNE_RIGHT command: %u, code is broken !\n", command);
			QUEUE_next(queueId, ACT_MIDDLE_DUNE_RIGHT, ACT_RESULT_NOT_HANDLED, ACT_RESULT_ERROR_LOGIC, __LINE__);
			return;
		}
	}
	...
	if(rx24_is_initialized == FALSE){
		...
		QUEUE_next(queueId, ACT_MIDDLE_DUNE_RIGHT, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	...
	if(!RX24_set_position(MIDDLE_DUNE_RIGHT_RX24_ID, *rx24_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que le RX24 ne répond pas a cet envoi, on l'indique à la carte stratégie
		error_printf("RX24_set_position error: 0x%x\n", RX24_get_last_error(MIDDLE_DUNE_RIGHT_RX24_ID).error);
		QUEUE_next(queueId, ACT_MIDDLE_DUNE_RIGHT, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	//La commande a été envoyée et le RX24 l'a bien reçu
	debug_printf("Move MIDDLE_DUNE_RIGHT rx24 to %d\n", *rx24_goalPosition);
}
...
static void MIDDLE_DUNE_RIGHT_command_run(queue_id_t queueId) {
	...
	if(ACTQ_check_status_rx24(queueId, MIDDLE_DUNE_RIGHT_RX24_ID, QUEUE_get_arg(queueId)->param, MIDDLE_DUNE_RIGHT_RX24_ASSER_POS_EPSILON, MIDDLE_DUNE_RIGHT_RX24_ASSER_TIMEOUT, MIDDLE_DUNE_RIGHT_RX24_ASSER_POS_LARGE_EPSILON, &result, &errorCode, &line))
		QUEUE_next(queueId, ACT_MIDDLE_DUNE_RIGHT, result, errorCode, line);
}
