...
#include "shift_cylinder_config.h"

// Les différents define pour le verbose sur uart
#define LOG_PREFIX "shift_cylinder.c : "
#define LOG_COMPONENT OUTPUT_LOG_COMPONENT_SHIFT_CYLINDER
#include "../QS/QS_outputlog.h"

// Les fonctions internes au fonctionnement de l'actionneur
static void SHIFT_CYLINDER_command_run(queue_id_t queueId);
static void SHIFT_CYLINDER_initRX24();
static void SHIFT_CYLINDER_command_init(queue_id_t queueId);
static void SHIFT_CYLINDER_config(CAN_msg_t* msg);
...
static void SHIFT_CYLINDER_initRX24() {
	if(rx24_is_initialized == FALSE && RX24_is_ready(SHIFT_CYLINDER_RX24_ID) == TRUE) {
		rx24_is_initialized = TRUE;
		RX24_config_set_lowest_voltage(SHIFT_CYLINDER_RX24_ID, RX24_MIN_VOLTAGE);
		RX24_config_set_highest_voltage(SHIFT_CYLINDER_RX24_ID, RX24_MAX_VOLTAGE);
		RX24_set_torque_limit(SHIFT_CYLINDER_RX24_ID, SHIFT_CYLINDER_RX24_MAX_TORQUE_PERCENT);
		RX24_config_set_temperature_limit(SHIFT_CYLINDER_RX24_ID, SHIFT_CYLINDER_RX24_MAX_TEMPERATURE);

		RX24_config_set_maximal_angle(SHIFT_CYLINDER_RX24_ID, SHIFT_CYLINDER_RX24_MAX_VALUE);
		RX24_config_set_minimal_angle(SHIFT_CYLINDER_RX24_ID, SHIFT_CYLINDER_RX24_MIN_VALUE);

		RX24_config_set_error_before_led(SHIFT_CYLINDER_RX24_ID, RX24_BEFORE_LED);
		RX24_config_set_error_before_shutdown(SHIFT_CYLINDER_RX24_ID, RX24_BEFORE_SHUTDOWN);
...
void SHIFT_CYLINDER_config(CAN_msg_t* msg){
	switch(msg->data.act_msg.act_data.act_config.sub_act_id){
		case DEFAULT_MONO_ACT : // Premier élement de l'actionneur
			ACTMGR_config_RX24(SHIFT_CYLINDER_RX24_ID, msg);
			break;
...
void SHIFT_CYLINDER_init_pos(){
	...
	if(!RX24_set_position(SHIFT_CYLINDER_RX24_ID, SHIFT_CYLINDER_RX24_INIT_POS))
		debug_printf("   Le RX24 n°%d n'est pas là\n", SHIFT_CYLINDER_RX24_ID);
	else
...
void SHIFT_CYLINDER_stop(){
	RX24_set_torque_enabled(SHIFT_CYLINDER_RX24_ID, FALSE); //Stopper l'asservissement du RX24
}

// fonction appellée à la réception d'un message CAN (via ActManager)
bool_e SHIFT_CYLINDER_CAN_process_msg(CAN_msg_t* msg) {
	if(msg->sid == ACT_SHIFT_CYLINDER){
		SHIFT_CYLINDER_initRX24();
		switch(msg->data.act_msg.order) {
			// Listing de toutes les positions de l'actionneur possible
			case ACT_SHIFT_CYLINDER_IDLE :
			case ACT_SHIFT_CYLINDER_OPEN :
			case ACT_SHIFT_CYLINDER_CLOSE :
			case ACT_SHIFT_CYLINDER_STOP :
				ACTQ_push_operation_from_msg(msg, QUEUE_ACT_RX24_SHIFT_CYLINDER, &SHIFT_CYLINDER_run_command, 0,TRUE);
				break;

			case ACT_CONFIG :
				SHIFT_CYLINDER_config(msg);
				break;
...
void SHIFT_CYLINDER_run_command(queue_id_t queueId, bool_e init) {
	...
	if(QUEUE_get_act(queueId) == QUEUE_ACT_RX24_SHIFT_CYLINDER) {    // Gestion des mouvements de SHIFT_CYLINDER
		if(init)
			SHIFT_CYLINDER_command_init(queueId);
		else
			SHIFT_CYLINDER_command_run(queueId);
	}
}
...
static void SHIFT_CYLINDER_command_init(queue_id_t queueId) {
	...
	*rx24_goalPosition = 0xFFFF;
	...
	switch(command) {
		...
		case ACT_SHIFT_CYLINDER_IDLE : *rx24_goalPosition = SHIFT_CYLINDER_RX24_IDLE_POS; break;
		case ACT_SHIFT_CYLINDER_CLOSE : *rx24_goalPosition = SHIFT_CYLINDER_RX24_CLOSE_POS; break;
		case ACT_SHIFT_CYLINDER_OPEN : *rx24_goalPosition = SHIFT_CYLINDER_RX24_OPEN_POS; break;

		case ACT_SHIFT_CYLINDER_STOP :
			RX24_set_torque_enabled(SHIFT_CYLINDER_RX24_ID, FALSE); //Stopper l'asservissement du RX24
			QUEUE_next(queueId, ACT_SHIFT_CYLINDER, ACT_RESULT_DONE, ACT_RESULT_ERROR_OK, __LINE__);
			return;

		default: {
			error_printf("Invalid SHIFT_CYLINDER command: %u, code is broken !\n", command);
			QUEUE_next(queueId, ACT_SHIFT_CYLINDER, ACT_RESULT_NOT_HANDLED, ACT_RESULT_ERROR_LOGIC, __LINE__);
			return;
		}
	}
	...
	if(rx24_is_initialized == FALSE){
		...
		QUEUE_next(queueId, ACT_SHIFT_CYLINDER, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	...
	RX24_reset_last_error(SHIFT_CYLINDER_RX24_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
	if(!RX24_set_position(SHIFT_CYLINDER_RX24_ID, *rx24_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que le RX24 ne répond pas a cet envoi, on l'indique à la carte stratégie
		error_printf("RX24_set_position error: 0x%x\n", RX24_get_last_error(SHIFT_CYLINDER_RX24_ID).error);
		QUEUE_next(queueId, ACT_SHIFT_CYLINDER, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	//La commande a été envoyée et le RX24 l'a bien reçu
	debug_printf("Move SHIFT_CYLINDER rx24 to %d\n", *rx24_goalPosition);
}
...
static void SHIFT_CYLINDER_command_run(queue_id_t queueId) {
	...
	if(ACTQ_check_status_rx24(queueId, SHIFT_CYLINDER_RX24_ID, QUEUE_get_arg(queueId)->param, SHIFT_CYLINDER_RX24_ASSER_POS_EPSILON, SHIFT_CYLINDER_RX24_ASSER_TIMEOUT, SHIFT_CYLINDER_RX24_ASSER_POS_LARGE_EPSILON, &result, &errorCode, &line))
		QUEUE_next(queueId, ACT_SHIFT_CYLINDER, result, errorCode, line);
}
