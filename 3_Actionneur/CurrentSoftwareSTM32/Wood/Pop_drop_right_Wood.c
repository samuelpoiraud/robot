...
#include "Pop_drop_right_Wood.h"

// POP_DROP_RIGHT_WOOD d'un actionneur standart avec AX12
...
// Formatage : QUEUE_ACT_AX12_POP_DROP_RIGHT_WOOD
...
// Un define POP_DROP_RIGHT_WOOD_AX12_ID doit avoir été ajouté au fichier config_big/config_pin.h // config_small/config_pin.h
...
#include "Pop_drop_right_Wood_config.h"

// Les différents define pour le verbose sur uart
#define LOG_PREFIX "Pop_drop_right_Wood.c : "
#define LOG_COMPONENT OUTPUT_LOG_COMPONENT_POP_DROP_RIGHT_WOOD
#include "../QS/QS_outputlog.h"

// Les fonctions internes au fonctionnement de l'actionneur
static void POP_DROP_RIGHT_WOOD_command_run(queue_id_t queueId);
static void POP_DROP_RIGHT_WOOD_initAX12();
static void POP_DROP_RIGHT_WOOD_command_init(queue_id_t queueId);
static void POP_DROP_RIGHT_WOOD_config(CAN_msg_t* msg);
...
static void POP_DROP_RIGHT_WOOD_initAX12() {
	if(ax12_is_initialized == FALSE && AX12_is_ready(POP_DROP_RIGHT_WOOD_AX12_ID) == TRUE) {
		ax12_is_initialized = TRUE;
		AX12_config_set_highest_voltage(POP_DROP_RIGHT_WOOD_AX12_ID, 136);
		AX12_config_set_lowest_voltage(POP_DROP_RIGHT_WOOD_AX12_ID, 70);
		AX12_config_set_maximum_torque_percentage(POP_DROP_RIGHT_WOOD_AX12_ID, POP_DROP_RIGHT_WOOD_AX12_MAX_TORQUE_PERCENT);

		AX12_config_set_maximal_angle(POP_DROP_RIGHT_WOOD_AX12_ID, POP_DROP_RIGHT_WOOD_AX12_MAX_VALUE);
		AX12_config_set_minimal_angle(POP_DROP_RIGHT_WOOD_AX12_ID, POP_DROP_RIGHT_WOOD_AX12_MIN_VALUE);

		AX12_config_set_error_before_led(POP_DROP_RIGHT_WOOD_AX12_ID, AX12_ERROR_ANGLE | AX12_ERROR_CHECKSUM | AX12_ERROR_INSTRUCTION | AX12_ERROR_OVERHEATING | AX12_ERROR_OVERLOAD | AX12_ERROR_RANGE);
		AX12_config_set_error_before_shutdown(POP_DROP_RIGHT_WOOD_AX12_ID, AX12_ERROR_OVERHEATING);
...
void POP_DROP_RIGHT_WOOD_config(CAN_msg_t* msg){
	switch(msg->data[1]){
		case 0 : // Premier élement de l'actionneur
			ACTMGR_config_AX12(POP_DROP_RIGHT_WOOD_AX12_ID, msg);
			break;
...
void POP_DROP_RIGHT_WOOD_init_pos(){
	...
	if(!AX12_set_position(POP_DROP_RIGHT_WOOD_AX12_ID, POP_DROP_RIGHT_WOOD_AX12_INIT_POS))
		debug_printf("   L'AX12 n°%d n'est pas là\n", POP_DROP_RIGHT_WOOD_AX12_ID);
	else
...
bool_e POP_DROP_RIGHT_WOOD_CAN_process_msg(CAN_msg_t* msg) {
	if(msg->sid == ACT_POP_DROP_RIGHT_WOOD){
		POP_DROP_RIGHT_WOOD_initAX12();
		switch(msg->data[0]) {
			// Listing de toutes les positions de l'actionneur possible
			case ACT_POP_DROP_RIGHT_WOOD_IDLE :
			...
			case ACT_CONFIG :
				POP_DROP_RIGHT_WOOD_config(msg);
				break;
		...
		SELFTEST_set_actions(&POP_DROP_RIGHT_WOOD_run_command, 12, 3, (SELFTEST_action_t[]){
								 {ACT_POP_DROP_RIGHT_WOOD_CLOSED,		0,  QUEUE_ACT_AX12_POP_DROP_RIGHT_WOOD},
								 {ACT_POP_DROP_RIGHT_WOOD_OPEN,		0,  QUEUE_ACT_AX12_POP_DROP_RIGHT_WOOD},
								 {ACT_POP_DROP_RIGHT_WOOD_CLOSED,		0,  QUEUE_ACT_AX12_POP_DROP_RIGHT_WOOD}
							 });
	}
	return FALSE;
...
void POP_DROP_RIGHT_WOOD_run_command(queue_id_t queueId, bool_e init) {
	...
	if(QUEUE_get_act(queueId) == QUEUE_ACT_AX12_POP_DROP_RIGHT_WOOD) {    // Gestion des mouvements de POP_DROP_RIGHT_WOOD
		if(init)
			POP_DROP_RIGHT_WOOD_command_init(queueId);
		else
			POP_DROP_RIGHT_WOOD_command_run(queueId);
	}
}
...
static void POP_DROP_RIGHT_WOOD_command_init(queue_id_t queueId) {
	...
	*ax12_goalPosition = 0xFFFF;
	...
	switch(command) {
		...
		case ACT_POP_DROP_RIGHT_WOOD_IDLE : *ax12_goalPosition = POP_DROP_RIGHT_WOOD_AX12_IDLE_POS; break;
		case ACT_POP_DROP_RIGHT_WOOD_CLOSED : *ax12_goalPosition = POP_DROP_RIGHT_WOOD_AX12_IDLE_POS; break;
		case ACT_POP_DROP_RIGHT_WOOD_OPEN : *ax12_goalPosition = POP_DROP_RIGHT_WOOD_AX12_DEPLOYED_POS; break;

		case ACT_POP_DROP_RIGHT_WOOD_STOP :
			AX12_set_torque_enabled(POP_DROP_RIGHT_WOOD_AX12_ID, FALSE); //Stopper l'asservissement de l'AX12
			QUEUE_next(queueId, ACT_POP_DROP_RIGHT_WOOD, ACT_RESULT_DONE, ACT_RESULT_ERROR_OK, __LINE__);
			return;

		default: {
			error_printf("Invalid POP_DROP_RIGHT_WOOD command: %u, code is broken !\n", command);
			QUEUE_next(queueId, ACT_POP_DROP_RIGHT_WOOD, ACT_RESULT_NOT_HANDLED, ACT_RESULT_ERROR_LOGIC, __LINE__);
			return;
		}
	}
	...
	if(ax12_is_initialized == FALSE){
		...
		QUEUE_next(queueId, ACT_POP_DROP_RIGHT_WOOD, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
	...
	AX12_reset_last_error(POP_DROP_RIGHT_WOOD_AX12_ID); //Sécurité anti terroriste. Nous les parano on aime pas voir des erreurs là ou il n'y en a pas.
	if(!AX12_set_position(POP_DROP_RIGHT_WOOD_AX12_ID, *ax12_goalPosition)) {	//Si la commande n'a pas été envoyée correctement et/ou que l'AX12 ne répond pas a cet envoi, on l'indique à la carte stratégie
		error_printf("AX12_set_position error: 0x%x\n", AX12_get_last_error(POP_DROP_RIGHT_WOOD_AX12_ID).error);
		QUEUE_next(queueId, ACT_POP_DROP_RIGHT_WOOD, ACT_RESULT_FAILED, ACT_RESULT_ERROR_NOT_HERE, __LINE__);
		return;
	}
...
static void POP_DROP_RIGHT_WOOD_command_run(queue_id_t queueId) {
	...
	if(ACTQ_check_status_ax12(queueId, POP_DROP_RIGHT_WOOD_AX12_ID, QUEUE_get_arg(queueId)->param, POP_DROP_RIGHT_WOOD_AX12_ASSER_POS_EPSILON, POP_DROP_RIGHT_WOOD_AX12_ASSER_TIMEOUT, POP_DROP_RIGHT_WOOD_AX12_ASSER_POS_LARGE_EPSILON, &result, &errorCode, &line))
		QUEUE_next(queueId, ACT_POP_DROP_RIGHT_WOOD, result, errorCode, line);
}
