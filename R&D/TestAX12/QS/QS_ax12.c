...
#define AX12_instruction_queue_is_full() (INC_WITH_MOD(AX12_special_instruction_buffer.end_index, AX12_INSTRUCTION_REAL_NEEDED_BUFFER_SIZE) == AX12_special_instruction_buffer.start_index)
...
{
	while(!AX12_instruction_queue_is_empty()) Nop();
}
...
static void AX12_state_machine(event_e event) {
	unsigned int interrupt_enable_copy;
	...
	{
		case READY_TO_SEND:
			//Désactivation des ITs pour eviter les acces concurrent lorsque cette fonction est appelée hors IT (car il n'y avait rien a faire avant)
			interrupt_enable_copy = IEC0;
			IEC0=0;
			...
			//Réactivation de l'IT Tx de l'uart 2, pour pouvoir recevoir l'IT Tx (sinon on risque de le louper)
			EnableIntU2TX;
